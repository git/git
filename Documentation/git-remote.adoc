...
git remote add [-t <branch>] [-m <master>] [-f] [--[no-]tags] [--mirror=(fetch|push)] [--retries=<n>] <name> <URL>
...
git remote [-v | --verbose] update [-p | --prune] [--retries=<n>] [(<group> | <remote>)...]
...
+
With `--retries[=<n>|inf|never]` option, the fetch run by `-f` retries
the given number of times (or forever with `inf`, or not at all with
`never`) when it fails because of a transient error reported by the
transport.  When the option is not given, the `GIT_FETCH_RETRIES`
environment variable is consulted; when that is not set either, no
...
+
With `--retries[=<n>|inf|never]`, forward the given retry policy to each
`git fetch` invocation.  With `inf` (or `infinite`, `forever`), keep
...
`GIT_REMOTE_UPDATE_RETRIES` is consulted; when that is not set either,
no retrying is performed.
