...
+
If a `branch.<branch>.pushRemote` value is the same URL and cannot be
interpreted as a remote name, it is replaced with _<name>_ so that pushes
can be associated with the remote-tracking branches.
+
