...
git clone [--template=<template-directory>]
	  ...
	  [--[no-]retries[=<n>|inf|never]] [--] <repository>
	  [<directory>]
...
`--[no-]retries[=<n>|inf|never]`::
	Retry the clone up to _<n>_ times when the clone fails because
	of a transient error reported by the transport, waiting a short
	while between attempts.  With `inf` (or `infinite`, `forever`),
	keep retrying until the clone succeeds.  With `never` (or `none`),
	do not retry.  A bare `--retries` uses an internal default.
	`--no-retries` is the same as `--retries=never`.
+
When the option is not given, note that the environment variable
`GIT_CLONE_RETRIES` is consulted; when that is not set either, no
retrying is performed.
