...
[[commit]]
commit::
    A commit object contains these required fields
    (though there are other optional fields):
+
1. The *tree ID* of the repo's top-level directory. The tree object referred
   to by that id contains the full directory structure of all the files at
   that moment in time and each file's contents.
...
3. An *author* (the person who wrote the code) and the time the commit was
   authored.
4. A *committer* (the person committing the code) and the time the commit
   was committed. The author and committer are often the same person.
5. A *commit message*
+
...
[[tree]]
tree::
    In Git, a tree object represents a directory.
...
3. The <<object-id,*object ID*>> created from the contents of the file,
   directory, or gitlink.
+
...
with 1000 files, that commit will create 2 new blobs in the corresponding
tree object and leave the other 998 files blob IDs unchanged.
