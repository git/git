...
+
If not set, the default behavior in Git 2.x is equivalent to `@{upstream}`,
which compares against the configured upstream tracking branch. Starting
with Git 3.0 the default is equivalent to `@{upstream} @{push}`, which also
compares against the push tracking branch.
+
