...
`default`;;
`myers`;;
	The basic greedy diff algorithm. This is the default in Git 2.x.
`minimal`;;
...
`histogram`;;
	...
	low-occurrence common elements". This is the default starting
	with Git 3.0.
--
+
...
ifndef::git-diff[]
	...
	set to `false`, moved lines are not colored. Defaults to `false` in
	Git 2.x. Defaults to `zebra` starting with Git 3.0.
