...
+
The `--retries=<n>` option of `git maintenance run` (or the
`GIT_FETCH_RETRIES` environment variable) makes each prefetch `git fetch`
retry up to _<n>_ times when it fails because of a transient error
reported by the transport; `inf` (or `infinite`, `forever`) retries until
it succeeds, and `never` (or `none`) never retries.
