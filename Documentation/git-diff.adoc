...
`--include-untracked`::
	Show untracked files as new files, as if they had been marked
	with `git add --intent-to-add`, without changing the index.
	Files ignored by `.gitignore` and friends are not shown.  Can be
	used only when the working tree is one side of the comparison,
	i.e. not with `--cached` or when comparing two commits.
