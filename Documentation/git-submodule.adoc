...
`update [--init] [--remote] [-N | --no-fetch] [--[no-]recommend-shallow] [-f | --force] [--checkout | --rebase | --merge] [--reference=<repository>] [--ref-format=<format>] [--depth=<depth>] [--recursive] [--jobs <n>] [--[no-]single-branch] [--filter=<filter-spec>] [--retries=<n>] [--] [<path>...]`::
+
--
...
`--retries=<n>`::
	Retry a `git fetch` run in a submodule up to _<n>_ times when it
	fails because of a transient error reported by the transport; `inf`
	(or `infinite`, `forever`) retries until it succeeds, and `never`
	(or `none`) never retries.  A bare `--retries` uses an internal
	default, and `--no-retries` is the same as `--retries=never`.
	This option is only valid for the `update` command, and defaults to
	the `GIT_FETCH_RETRIES` environment variable when not given.
