...
Updating a ref can leave an older value in `packed-refs`.  Manually
removing only the loose file would make that older value visible again.
Use `git update-ref -d` to delete the ref from both locations instead
of removing its file directly; see linkgit:git-update-ref[1].
