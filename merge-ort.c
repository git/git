                                          ...
                                          int                            reinitialize)
{
    ...
    for (i = MERGE_SIDE1; i <= MERGE_SIDE2; ++i)
    {
        ...
        if (i != renames->cached_pairs_valid_side && -1 != renames->cached_pairs_valid_side)
        {
...
__attribute__((format(printf, 8, 9))) static void path_msg(struct merge_options        *opt,
                                                           enum conflict_and_info_types type,
                                                           ...
                                                           const char                  *fmt, ...)
{
    ...
    /* Sanity checks */
    assert(omittable_hint == (!starts_with(type_short_descriptions[type], "CONFLICT") && !starts_with(type_short_descriptions[type], "ERROR")) || type == CONFLICT_DIR_RENAME_SUGGESTED);
                            ...
                            int                      resolved /* boolean */)
{
    ...
    mi                  = mem_pool_calloc(&opt->priv->pool, 1,
                         resolved ? sizeof(struct merged_info) : sizeof(struct conflict_info));
                                ...
                                unsigned              match_mask)
{
    ...
    {
        /* simple sanity check */
        assert(renames->dir_rename_mask == 0 || renames->dir_rename_mask == (dirmask & ~1));
    ...
    if (dirmask == 1 || dirmask == 3 || dirmask == 5)
    {
        ...
        unsigned relevance = (renames->dir_rename_mask == 0x07) ? RELEVANT_FOR_ANCESTOR : NOT_RELEVANT;
        /*
     ...
     */
    if (renames->dir_rename_mask == 0x07 && (filemask == 2 || filemask == 4))
    {
        /*
                                       ...
                                       struct traverse_info *info)
{
    ...
    unsigned                       side1_matches_mbase  = (!side1_null && !mbase_null && names[0].mode == names[1].mode && oideq(&names[0].oid, &names[1].oid));
    unsigned                       side2_matches_mbase  = (!side2_null && !mbase_null && names[0].mode == names[2].mode && oideq(&names[0].oid, &names[2].oid));
    unsigned                       sides_match          = (!side1_null && !side2_null && names[1].mode == names[2].mode && oideq(&names[1].oid, &names[2].oid));
    ...
    if (dirmask)
    {
        ...
        side = side1_matches_mbase ? MERGE_SIDE2 : side2_matches_mbase ? MERGE_SIDE1
                                                                       : MERGE_BASE;
...
{
    VERIFY_CI(ci);
    assert((side == 1 && ci->match_mask == 5) || (side == 2 && ci->match_mask == 3));
                                   ...
                                   struct traverse_info *info)
{
    ...
    for (side = MERGE_SIDE1; side <= MERGE_SIDE2; side++)
    {
        ...
        strintmap_for_each_entry(&copy, &iter, entry)
        {
            ...
            if (optimization_okay && !strset_contains(&renames->deferred[side].target_dirs, path))
            {
                                 ...
                                 &iter, entry)
        {
            ...
            assert(renames->deferred[side].trivial_merges_okay && !strset_contains(&renames->deferred[side].target_dirs, path));
            resolve_trivial_directory_merge(ci, side);
        }
                              ...
                              struct tree          *side2)
{
    ...
    if (parse_tree(merge_base) < 0 || parse_tree(side1) < 0 || parse_tree(side2) < 0)
    {
        return -1;
    }
                           ...
                           struct object_id       *result)
{
    ...
    int                 ret = 0;
    int                 ret2;
    struct commit      *commit_o;
    struct commit      *commit_a;
    struct commit      *commit_b;
    int                 parent_count;
    ...
    if (!(commit_o = lookup_commit_reference(&subrepo, o)) || !(commit_a = lookup_commit_reference(&subrepo, a)) || !(commit_b = lookup_commit_reference(&subrepo, b)))
    {
                                ...
                                struct version_info       *result)
{
    ...
    else if (S_ISREG(a->mode))
    {
        ...
        if (!ret && write_object_file(result_buf.ptr, result_buf.size, OBJ_BLOB, &result->oid))
        {
                                                 ...
                                                 char                 *new_path)
{
    ...
    struct strmap_entry  *entry;
    const char           *branch_with_new_path;
    const char           *branch_with_dir_rename;
                           ...
                           struct diff_queue_struct *renames)
{
    ...
    for (i = 0; i < renames->nr; ++i)
    {
        ...
        struct conflict_info *oldinfo = NULL;
        struct conflict_info *newinfo = NULL;
        struct strmap_entry  *old_ent;
        struct strmap_entry  *new_ent;
        unsigned int          old_sidemask;
        int                   target_index;
        int                   other_source_index;
        int                   source_deleted;
        int                   collision;
        int                   type_changed;
        const char           *rename_branch = NULL;
        const char           *delete_branch = NULL;
         ...
         */
        if (i + 1 < renames->nr && !strcmp(oldpath, renames->queue[i + 1]->one->path))
        {
            /* Handle rename/rename(1to2) or rename/rename(1to1) */
            const char           *pathnames[3];
            struct version_info   merged;
            struct conflict_info *base;
            struct conflict_info *side1;
            struct conflict_info *side2;
            ...
            if (!clean_merge && merged.mode == side1->stages[1].mode && oideq(&merged.oid, &side1->stages[1].oid))
            {
                was_binary_blob = 1;
            }
        ...
        type_changed       = !source_deleted && (S_ISREG(oldinfo->stages[other_source_index].mode) != S_ISREG(newinfo->stages[target_index].mode));
...
static inline int possible_side_renames(struct rename_info *renames,
                                        unsigned            side_index)
{
    return renames->pairs[side_index].nr > 0 && !strintmap_empty(&renames->relevant_sources[side_index]);
}

static inline int possible_renames(struct rename_info *renames)
{
    return possible_side_renames(renames, 1) || possible_side_renames(renames, 2) || !strmap_empty(&renames->cached_pairs[1]) || !strmap_empty(&renames->cached_pairs[2]);
}
...
static int detect_and_process_renames(struct merge_options *opt)
{
    ...
    need_dir_renames =
        !opt->priv->call_depth && (opt->detect_directory_renames == MERGE_DIRECTORY_RENAMES_TRUE || opt->detect_directory_renames == MERGE_DIRECTORY_RENAMES_CONFLICT);
                          ...
                          const char                *path)
{
    ...
    if (read_oid_strbuf(opt, &base->oid, &basebuf, path) || read_oid_strbuf(opt, &side->oid, &sidebuf, path))
    {
        goto error_return;
    }
    /*
     ...
     */
    if (renormalize_buffer(idx, path, basebuf.buf, basebuf.len, &basebuf) | renormalize_buffer(idx, path, sidebuf.buf, sidebuf.len, &sidebuf))
    {
        ret = (basebuf.len == sidebuf.len && !memcmp(basebuf.buf, sidebuf.buf, basebuf.len));
    }
                                     ...
                                     struct directory_versions *info)
{
     ...
     */
    if (info->last_directory == NULL || !strncmp(new_directory_name, info->last_directory, info->last_directory_len))
    {
     ...
     */
    prev_dir = info->offsets.nr == 0 ? NULL : info->offsets.items[info->offsets.nr - 1].string;
                         ...
                         struct directory_versions *dir_metadata)
{
    ...
    assert(ci->match_mask == 0 || ci->match_mask == 3 || ci->match_mask == 5 || ci->match_mask == 6);
    ...
    else if (ci->filemask >= 6 && (S_IFMT & ci->stages[1].mode) != (S_IFMT & ci->stages[2].mode))
    {
    ...
    else if (ci->filemask == 3 || ci->filemask == 5)
    {
        ...
        else if (ci->path_conflict && oideq(&ci->stages[0].oid, &ci->stages[side].oid))
        {
            /*
                                        ...
                                        struct string_list   *plist)
{
    ...
    for (e = &plist->items[plist->nr - 1]; e >= plist->items; --e)
    {
        ...
        if (ci->match_mask || ci->filemask < 6 || !S_ISREG(ci->stages[1].mode) || !S_ISREG(ci->stages[2].mode) || oideq(&ci->stages[1].oid, &ci->stages[2].oid))
        {
            continue;
        }

        /* Also don't need content merge if base matches either side */
        if (ci->filemask == 7 && S_ISREG(ci->stages[0].mode) && (oideq(&ci->stages[0].oid, &ci->stages[1].oid) || oideq(&ci->stages[0].oid, &ci->stages[2].oid)))
        {
            continue;
        }
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            if ((ci->filemask & side_mask) && S_ISREG(vi->mode) && oid_object_info_extended(opt->repo, &vi->oid, NULL, OBJECT_INFO_FOR_PREFETCH))
            {
                           ...
                           struct object_id     *result_oid)
{
    ...
    if (dir_metadata.offsets.nr != 1 || (uintptr_t)dir_metadata.offsets.items[0].util != 0)
    {
...
static void print_submodule_conflict_suggestion(struct string_list *csub)
{
    ...
    for_each_string_list_item(item, csub)
    {
         ...
         */
        if (util->flag == CONFLICT_SUBMODULE_NOT_INITIALIZED || util->flag == CONFLICT_SUBMODULE_HISTORY_NOT_AVAILABLE)
        {
            continue;
        }
                                   ...
                                   struct merge_result  *result)
{
    ...
    for (int path_nr = 0; path_nr < olist.nr; ++path_nr)
    {
        ...
        for (int i = 0; i < conflicts->nr; i++)
        {
            ...
            if (result->clean < 0 && info->type < NB_REGULAR_CONFLICT_TYPES)
            {
                continue;
            }
...
static void merge_start(struct merge_options *opt, struct merge_result *result)
{
    ...
    assert(opt->detect_directory_renames >= MERGE_DIRECTORY_RENAMES_NONE && opt->detect_directory_renames <= MERGE_DIRECTORY_RENAMES_TRUE);
    assert(opt->rename_limit >= -1);
    assert(opt->rename_score >= 0 && opt->rename_score <= MAX_SCORE);
    assert(opt->show_rename_progress >= 0 && opt->show_rename_progress <= 1);

    assert(opt->xdl_opts >= 0);
    assert(opt->recursive_variant >= MERGE_VARIANT_NORMAL && opt->recursive_variant <= MERGE_VARIANT_THEIRS);
     ...
     */
    assert(opt->detect_renames >= -1 && opt->detect_renames <= DIFF_DETECT_COPY);
    assert(opt->verbosity >= 0 && opt->verbosity <= 5);
    assert(opt->buffer_output <= 2);
    assert(opt->obuf.len == 0);

    assert(opt->priv == NULL);
    if (result->_properly_initialized != 0 && result->_properly_initialized != RESULT_INITIALIZED)
    {
    ...
    if (result->priv)
    {
        ...
        assert(!opt->priv->toplevel_dir || 0 == strlen(opt->priv->toplevel_dir));
    }
                                         ...
                                         struct tree         *side2)
{
    ...
    if (oideq(&merge_base->object.oid, &merge_trees[2]->object.oid) && oideq(&side1->object.oid, &result->tree->object.oid))
    {
        renames->cached_pairs_valid_side = MERGE_SIDE1;
    }
    else if (oideq(&merge_base->object.oid, &merge_trees[1]->object.oid) && oideq(&side2->object.oid, &result->tree->object.oid))
    {
