...
enum protocol_version get_protocol_version_config(void)
{
	...
	but_test_v = getenv(but_test_k);
	if (but_test_v && *but_test_v) {
		enum protocol_version env = parse_protocol_version(but_test_v);

		if (env == protocol_unknown_version)
			die("unknown value for %s: %s", but_test_k, but_test_v);
		return env;
	}
...
enum protocol_version determine_protocol_version_server(void)
{
	...
	if (but_protocol) {
		...
		string_list_split(&list, but_protocol, ':', -1);
