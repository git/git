                                 ...
                                 uint32_t             graph_pos)
{
    ...
    {
        warning(
            "ignoring decreasing changed-path index offsets"
            " (%" PRIuMAX " > %" PRIuMAX
            ") for positions"
    ...
    filter->data    = (unsigned char *)(g->chunk_bloom_data + sizeof(unsigned char) * start_index + BLOOMDATA_CHUNK_HEADER_SIZE);
