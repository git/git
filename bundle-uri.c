                              ...
                              struct bundle_list *list)
{
    ...
    if (!subsection_len)
    {
        ...
        if (!strcmp(subkey, "heuristic"))
        {
            ...
            for (i = 0; i < BUNDLE_HEURISTIC__COUNT; i++)
            {
                if (heuristics[i].heuristic && heuristics[i].name && !strcmp(value, heuristics[i].name))
                {
                                  ...
                                  struct bundle_list *list)
{
    ...
    uint64_t                   maxCreationToken = 0;
    uint64_t                   newMaxCreationToken = 0;
    ...
    if (!repo_config_get_value(r,
                               ...
                               &creationTokenStr)
        && sscanf(creationTokenStr, "%" PRIu64, &maxCreationToken) == 1 && bundles.items[0]->creationToken <= maxCreationToken)
    {
        free(bundles.items);
        return 0;
    }
