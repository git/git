                                           ...
                                           int imin, int depth)
{
    ...
    if (emitthis)
    {
        ...
        for (i = 0; i < nparent; ++i)
        {
            ...
            p->parent[i].status =
                !t ? DIFF_STATUS_DELETED : tpi_valid ? DIFF_STATUS_MODIFIED
                                                     : DIFF_STATUS_ADDED;
        ...
        if (cmp == 0)
        {
            ...
            if (!opt->flags.find_copies_harder)
            {
                for (i = 0; i < nparent; ++i)
                {
                    ...
                    /* diff(t,pi) != ø */
                    if (!oideq(&t.entry.oid, &tp[i].entry.oid) || (t.entry.mode != tp[i].entry.mode))
                    {
                        continue;
                    }
...
{
    return diff_queued_diff.nr == 1 && !DIFF_FILE_VALID(diff_queued_diff.queue[0]->one);
}
                                  ...
                                  struct strbuf *base, struct diff_options *opt)
{
    ...
    for (i = 0; i < q->nr; i++)
    {
         ...
         * the future!
         */
        if ((p->status == 'R' || p->status == 'C') && !strcmp(p->two->path, opt->pathspec.items[0].match))
        {
            const char *path[2];
...
static void ll_diff_tree_oid(const struct object_id *old_oid,
                             ...
                             struct strbuf *base, struct diff_options *opt)
{
    struct combine_diff_path  phead;
    struct combine_diff_path *p;
