...
static void add_pushurl_alias(struct remote_state *remote_state, struct remote *remote, const char *url)
{
...
static void add_url_alias(struct remote_state *remote_state, struct remote *remote, const char *url)
{
...
static struct remote *make_remote(struct remote_state *remote_state, const char *name, int len)
{
	struct remote *ret;
...
static struct branch *make_branch(struct remote_state *remote_state, const char *name, size_t len)
{
	struct branch *ret;
	int i;
...
static void read_remotes_file(struct remote_state *remote_state, struct remote *remote)
{
	...
	while (strbuf_getline(&buf, f) != EOF) {
		...
		if (skip_prefix(buf.buf, "URL:", &v))
			add_url_alias(remote_state, remote, xstrdup(skip_spaces(v)));
...
static void read_branches_file(struct remote_state *remote_state, struct remote *remote)
{
	char *frag;
	struct strbuf buf = STRBUF_INIT;
	...
	if (startup_info->have_repository) {
		...
		if (head_ref && (flag & REF_ISSYMREF) &&
			...
			remote_state->current_branch =
				make_branch(remote_state, head_ref, strlen(head_ref));
		}
	}
				  ...
				  struct branch *branch, int for_push)
{
	if (branch) {
		if (!for_push) {
			...
			const char *dst, *remote_name = repo_pushremote_for_branch(
						 repo, branch, NULL);
			struct remote *remote = repo_remote_get(repo, remote_name);
...
static const char *repo_branch_get_push_1(struct repository *repo, struct branch *branch,
				     struct strbuf *err)
{
	struct remote *remote;

	remote = repo_remote_get(repo, repo_pushremote_for_branch(repo, branch, NULL));
	if (!remote)
		return error_buf(err,
			 ...
			 enum ahead_behind_flags abf)
{
	...
	sti = repo_stat_tracking_info(repo, branch, &ours, &theirs, &full_base, 0, abf);
	if (sti < 0) {
		if (!full_base)
			return 0;
		upstream_is_gone = 1;
