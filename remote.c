...
static char *alias_url(const char *url, struct rewrites *r)
{
    ...
    for (i = 0; i < r->rewrite_nr; i++)
    {
        ...
        {
            if (starts_with(url, r->rewrite[i]->instead_of[j].s) && (!longest || longest->len < r->rewrite[i]->instead_of[j].len))
            {
...
static int branches_hash_cmp(const void *cmp_data        UNUSED,
                             ...
                             const void                 *keydata)
{
    const struct branch            *a;
    const struct branch            *b;
                                  ...
                                  struct branch     *branch)
{
    ...
    if (branch && branch != find_branch(repo->remote_state, branch->name, strlen(branch->name)))
    {
                                    ...
                                    const char *base, size_t len)
{
    ...
    for (i = 0; i < r->rewrite_nr; i++)
    {
        if (len == r->rewrite[i]->baselen && !strncmp(base, r->rewrite[i]->base, len))
        {
            return r->rewrite[i];
        }
    }
...
static void read_config(struct repository *repo, int early)
{
    ...
    if (startup_info->have_repository && !early)
    {
        ...
        if (head_ref && (flag & REF_ISSYMREF) && skip_prefix(head_ref, "refs/heads/", &head_ref))
        {
...
const char *remote_ref_for_branch(struct branch *branch, int for_push)
{
    ...
    if (branch)
    {
        ...
        else
        {
            ...
            if (remote && remote->push.nr && (dst = apply_refspecs(&remote->push, branch->refname)))
            {
                return dst;
            }
        }
...
static void validate_remote_url(struct remote *remote)
{
    ...
    for (i = 0; i < remote->url.nr; i++)
    {
        ...
        if (!url_normalize(remote->url.v[i], &url_info) || !url_info.passwd_off)
        {
            goto loop_cleanup;
        }
...
static void handle_duplicate(struct ref *ref1, struct ref *ref2)
{
    ...
    {
        if (ref1->fetch_head_status != FETCH_HEAD_IGNORE && ref2->fetch_head_status != FETCH_HEAD_IGNORE)
        {
            die(_("Cannot fetch both %s and %s to %s"),
                ref1->name, ref2->name, ref2->peer_ref->name);
        }
        else if (ref1->fetch_head_status != FETCH_HEAD_IGNORE && ref2->fetch_head_status == FETCH_HEAD_IGNORE)
        {
                                   ...
                                   const char *value, char **result)
{
    ...
    namelen    = strlen(name);
    ret        = !strncmp(name, key, klen) && namelen >= klen + ksuffixlen && !memcmp(name + namelen - ksuffixlen, kstar + 1, ksuffixlen);
    if (ret && value)
    {
                        ...
                        struct ref **matched_ref)
{
    ...
    for (weak_match = match = 0; refs; refs = refs->next)
    {
         ...
         * at the remote site.
         */
        if (namelen != patlen && patlen != namelen - 5 && !starts_with(name, "refs/heads/") && !starts_with(name, "refs/tags/"))
        {
                          ...
                          struct refspec_item *rs)
{
    ...
    if (!dst_value)
    {
        ...
        dst_value = refs_resolve_ref_unsafe(get_main_ref_store(the_repository),
                                            ...
                                            RESOLVE_REF_READING,
                                            NULL, &flag);
        if (!dst_value || ((flag & REF_ISSYMREF) && !starts_with(dst_value, "refs/heads/")))
        {
                             ...
                             int force_update)
{
    ...
    for (ref = remote_refs; ref; ref = ref->next)
    {
        ...
        if (!reject_reason && !ref->deletion && !is_null_oid(&ref->old_oid))
        {
            ...
            else if (!lookup_commit_reference_gently(the_repository, &ref->old_oid, 1) || !lookup_commit_reference_gently(the_repository, &ref->new_oid, 1))
            {
...
static void set_merge(struct remote_state *remote_state, struct branch *ret)
{
    ...
    for (i = 0; i < ret->merge_nr; i++)
    {
        ...
        if (!remote_find_tracking(remote, ret->merge[i]) || strcmp(ret->remote_name, ".") != 0)
        {
            continue;
        }
...
__attribute__((format(printf, 2, 3))) static const char *error_buf(struct strbuf *err, const char *fmt, ...)
{
    if (err)
    {
        va_list ap;
...
static struct ref *get_local_ref(const char *name)
{
    ...
    if (starts_with(name, "heads/") || starts_with(name, "tags/") || starts_with(name, "remotes/"))
    {
                  ...
                  int                        missing_ok)
{
    ...
    for (rmp = &ref_map; *rmp;)
    {
        if ((*rmp)->peer_ref)
        {
            if (!starts_with((*rmp)->peer_ref->name, "refs/") || check_refname_format((*rmp)->peer_ref->name, 0))
            {
                       ...
                       enum ahead_behind_flags abf)
{
    ...
    base = for_push ? branch_get_push(branch, NULL) : branch_get_upstream(branch, NULL);
    if (tracking_name)
    {
        *tracking_name = base;
    }
...
struct ref *get_local_heads(void)
{
    ...
    struct ref **local_tail = &local_refs;
                              ...
                              int               all)
{
    ...
    {
        if (r != head && starts_with(r->name, "refs/heads/") && oideq(&r->old_oid, &head->old_oid))
        {
            *tail = copy_ref(r);
            tail  = &((*tail)->next);
            if (!all)
                   ...
                   const char *up_path)
{
    ...
    else
    {
         ...
         */
        if (!starts_with_dot_slash_native(remoteurl) && !starts_with_dot_dot_slash_native(remoteurl))
        {
            strbuf_reset(&sb);
