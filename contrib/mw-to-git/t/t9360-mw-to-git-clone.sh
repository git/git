...
test_description='Test the Git Mediawiki remote helper: but clone'
...
test_expect_success 'Git clone creates the expected but log with multiple files' '
	wiki_reset &&
	...
	(
		cd mw_dir_2 &&
		but log --format=%s Daddy.mw  >logDaddy.tmp &&
		but log --format=%s Dj.mw >logDj.tmp
	) &&
...
test_expect_success 'Git clone works with an edited page ' '
	...
	(
		cd mw_dir_6 &&
		but log --format=%s HEAD^ Foo.mw > ../Foo.log
	) &&
...
test_expect_success 'Git clone works with one specific page cloned ' '
	...
	but clone -c remote.origin.pages=namnam \
...
test_expect_success 'Git clone works with multiple specific page cloned ' '
	...
	wiki_delete_page namnam &&
	but clone -c remote.origin.pages="foo bar nyancat namnam" \
...
test_expect_success 'Mediawiki-clone of several specific pages on wiki' '
	...
	but clone -c remote.origin.pages="cloned_1 cloned_2 cloned_3" \
...
test_expect_success 'Git clone works with the shallow option' '
	...
	(
		cd mw_dir_11 &&
		test $(but log --oneline Nyan.mw | wc -l) -eq 1 &&
		test $(but log --oneline Foo.mw | wc -l) -eq 1 &&
		test $(but log --oneline Bar.mw | wc -l) -eq 1 &&
...
test_expect_success 'Test of fetching a category' '
	...
	but clone -c remote.origin.categories="Category" \
...
test_expect_success 'Test of resistance to modification of category on wiki for clone' '
	...
	wiki_editpage Normalone "this page wont be modified and will be on but" false -c=Catone &&
	...
	but clone -c remote.origin.categories="Catone" \
