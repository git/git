...
# generated by the 'make' commands above with 'but diff <version1> <version2>'
...
diff_lines () {
	perl -e '
		...
		while (<>) {
			...
			# Have we seen a hunk?  Ignore "diff --but" etc.
...
files=$(but diff --name-only "$V1" "$V2" -- \*.c)
...
(
	...
	do
		but log --no-decorate --pretty=format:'%an      %h: %s' -1 $cummit
		echo
	done
) | sort
