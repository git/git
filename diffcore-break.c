...
void diffcore_break(struct repository *r, int break_score)
{
    ...
    for (i = 0; i < q->nr; i++)
    {
         ...
         */
        if (DIFF_FILE_VALID(p->one) && DIFF_FILE_VALID(p->two) && object_type(p->one->mode) == OBJ_BLOB && object_type(p->two->mode) == OBJ_BLOB && !strcmp(p->one->path, p->two->path))
        {
    ...
    for (i = 0; i < q->nr; i++)
    {
        ...
        else if (p->broken_pair && !strcmp(p->one->path, p->two->path))
        {
            ...
            for (j = i + 1; j < q->nr; j++)
            {
                ...
                if (pp->broken_pair && !strcmp(pp->one->path, pp->two->path) && !strcmp(p->one->path, pp->two->path))
                {
