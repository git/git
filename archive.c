                                    ...
                                    unsigned long              *sizep)
{
    ...
    init_checkout_metadata(&meta, args->refname,
                           args->commit_oid ? args->commit_oid : (args->tree ? &args->tree->object.oid : NULL), oid);
                               ...
                               void *context)
{
    ...
    /* Stream it? */
    if (S_ISREG(mode) && !args->convert && oid_object_info(args->repo, oid, &size) == OBJ_BLOB && size > big_file_threshold)
...
static int write_directory(struct archiver_context *c)
{
    ...
    ret =
        write_directory(c) || write_archive_entry(&d->oid, d->path, d->baselen, d->path + d->baselen, d->mode, c) != READ_TREE_RECURSIVE;
    free(d);
    return ret ? -1 : 0;
}
                                        ...
                                        unsigned mode, void *context)
{
    ...
    while (c->bottom && !(base->len >= c->bottom->len && !strncmp(base->buf, c->bottom->path, c->bottom->len)))
    {
...
static int path_exists(struct archiver_args *args, const char *path)
{
    ...
    if (args->prefix && read_tree(args->repo, args->tree, &ctx.pathspec, reject_outside, args))
