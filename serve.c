...
{
    if (advertise_object_info == -1 && repo_config_get_bool(r, "transfer.advertiseobjectinfo", &advertise_object_info))
    {
        /* disabled by default */
...
static int process_request(void)
{
    ...
    packet_reader_init(&reader, 0, NULL, 0,
                       PACKET_READ_CHOMP_NEWLINE | PACKET_READ_GENTLE_ON_EOF | PACKET_READ_DIE_ON_ERR_PACKET);
