...
/*
 * define_cummit_slab(slabname, elemtype) creates boilerplate code to define
 * a new struct (struct slabname) that is used to associate a piece of data
 * of elemtype to cummits, and a few functions to use that struct.
 *
 * After including this header file, using:
 *
 * define_cummit_slab(indegree, int);
 *
 * will let you call the following functions:
 *
 * - int *indegree_at(struct indegree *, struct cummit *);
 *
 *   This function locates the data associated with the given cummit in
 *   the indegree slab, and returns the pointer to it.  The location to
 *   store the data is allocated as necessary.
 *
 * - int *indegree_peek(struct indegree *, struct cummit *);
 *
 *   This function is similar to indegree_at(), but it will return NULL
 *   if the location to store the data associated with the given cummit
 *   has not been allocated yet.
 *   Note that the location to store the data might have already been
 *   allocated even if no indegree_at() call has been made for that cummit
 ...
 *   to each cummit. 'stride' specifies how big each array is.  The slab
 *   that is initialized by the variant without "_with_stride" associates
 *   each cummit with an array of one integer.
 *
...
#define define_cummit_slab(slabname, elemtype) \
