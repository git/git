...
int xdl_change_compact(xdfile_t *xdf, xdfile_t *xdfo, long flags) {
	...
	while (1) {
		 ...
		 * matching lines. Re-diff the group to mark these lines as
		 * unchanged and remove from the diff output.
		 *
		 * Only do this for histogram diff as its LCS algorithm makes
		 * this scenario possible. In contrast, patience diff finds LCS
		 * of unique lines where lines cannot be shifted across.
		 ...
		 * XDF_NEED_MINIMAL, Myer's isn't technically guaranteed to
		 * be minimal, but it should be so most of the time)
		 */
