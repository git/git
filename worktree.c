...
static struct worktree *get_main_worktree(int skip_reading_head)
{
     ...
     */
    worktree->is_bare    = (is_bare_repository_cfg == 1) || is_bare_repository();
                                                ...
                                                const char       *suffix)
{
    ...
    for (; *list && nr_found < 2; list++)
    {
        ...
        if ((!start || (start > 0 && is_dir_sep(path[start - 1]))) && !fspathcmp(suffix, path + start))
        {
            found = *list;
            nr_found++;
        }
                              ...
                              const char            *target)
{
    ...
    found_rebase = wt_status_check_rebase(wt, &state) && (state.rebase_in_progress || state.rebase_interactive_in_progress) && state.branch && skip_prefix(target, "refs/heads/", &target) && !strcmp(state.branch, target);
                               ...
                               const char            *target)
{
    ...
    memset(&state, 0, sizeof(state));
    found_bisect = wt_status_check_bisect(wt, &state) && state.bisecting_from && skip_prefix(target, "refs/heads/", &target) && !strcmp(state.bisecting_from, target);
    wt_status_state_free_buffers(&state);
    return found_bisect;
}
                     ...
                     const char *target)
{
    ...
    symref_target = refs_resolve_ref_unsafe(refs, symref, 0,
                                            NULL, &flags);
    if ((flags & REF_ISSYMREF) && symref_target && !strcmp(symref_target, target))
    {
        return 1;
    }
...
void strbuf_worktree_ref(const struct worktree *wt,
                         ...
                         const char            *refname)
{
    if (parse_worktree_ref(refname, NULL, NULL, NULL) == REF_WORKTREE_CURRENT && wt && !wt->is_current)
    {
        if (is_main_worktree(wt))
        {
...
int should_prune_worktree(const char *id, struct strbuf *reason, char **wtpath, timestamp_t expire)
{
    ...
    *wtpath = NULL;
    ...
    fd = open(git_path("worktrees/%s/gitdir", id), O_RDONLY | O_CLOEXEC);
    if (fd < 0)
    {
    ...
    if (!file_exists(path))
    {
        if (stat(git_path("worktrees/%s/index", id), &st) || st.st_mtime <= expire)
        {
