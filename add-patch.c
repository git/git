...
static int parse_diff(struct add_p_state *s, const struct pathspec *ps)
{
    ...
    if (s->revision)
    {
        ...
        strvec_push(&args,
                    ...
                    !strcmp("HEAD", s->revision) && repo_get_oid(the_repository, "HEAD", &oid) ? empty_tree_oid_hex(the_repository->hash_algo) : s->revision);
    }
    ...
    while (p != pend)
    {
        ...
        else if (hunk == &file_diff->head && starts_with(p, "new file"))
        {
            file_diff->added = 1;
        }
    ...
    if (hunk->header.old_offset != 0 || hunk->header.new_offset != 0)
    {
        ...
        else if (header->suppress_colored_line_range)
        {
            strbuf_add(out,
                       ...
                       header->colored_extra_end - header->colored_extra_start);
                             ...
                             struct file_diff   *file_diff)
{
    ...
    for (;;)
    {
        ...
        if (ch == 'y')
        {
            hunk->use = USE_HUNK;
        soft_increment:
            hunk_index = undecided_next < 0 ? file_diff->hunk_nr : undecided_next;
        }
        else if (ch == 'n')
        {
            hunk->use = SKIP_HUNK;
        ...
        else if (s->answer.buf[0] == 'g')
        {
            ...
            while (s->answer.len == 0)
            {
                ...
                printf("%s", i < file_diff->hunk_nr ? _("go to which hunk (<ret> to see "
                                                        "more)? ")
                                                    : _("go to which hunk? "));
                fflush(stdout);
              ...
              const char *revision, const struct pathspec *ps)
{
    ...
    if (repo_read_index(r) < 0 || (!s.mode->index_only && repo_refresh_and_write_index(r, REFRESH_QUIET, 0, 1, NULL, NULL, NULL) < 0) || parse_diff(&s, ps) < 0)
    {
        add_p_state_clear(&s);
        return -1;
    }
