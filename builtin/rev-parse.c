...
static int cmd_parseopt(int argc, const char **argv, const char *prefix)
{
    ...
    argc = parse_options(argc, argv, prefix, opts, usage.v,
                         (keep_dashdash ? PARSE_OPT_KEEP_DASHDASH : 0) | (stop_at_non_option ? PARSE_OPT_STOP_AT_NON_OPTION : 0) | PARSE_OPT_SHELL_EVAL);
...
static void print_path(const char *path, const char *prefix, enum format_type format, enum default_type def)
{
    ...
    else if (format == FORMAT_RELATIVE || (format == FORMAT_DEFAULT && def == DEFAULT_RELATIVE))
    {
        /*
...
int cmd_rev_parse(int argc, const char **argv, const char *prefix)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if (!seen_end_of_options && *arg == '-')
        {
            ...
            if (!strcmp(arg, "--git-dir") || !strcmp(arg, "--absolute-git-dir"))
            {
