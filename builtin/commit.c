                             ...
                             struct strbuf    *author_ident)
{
    ...
    if (use_editor && include_status)
    {
        ...
        const char        *hint_cleanup_space = allow_empty_message ? _(
                                                    "Please enter the commit message for your changes."
                                                           " Lines starting\n"
     ...
     */
    if (!committable && whence != FROM_MERGE && !allow_empty && !(amend && is_a_merge(current_head)))
    {
    ...
    if (!no_verify && run_commit_hook(use_editor, index_file, NULL, "commit-msg", git_path_commit_editmsg(), NULL))
    {
        return 0;
    }
...
{
    int use_deferred_config = (status_format != STATUS_FORMAT_PORCELAIN && status_format != STATUS_FORMAT_PORCELAIN_V2 && !s->null_termination);
                                      ...
                                      struct wt_status    *s)
{
    ...
    if ((is_from_cherry_pick(whence) || whence == FROM_REBASE_PICK) && !renew_authorship)
    {
    ...
    if (fixup_message)
    {
        ...
        if (p > fixup_message && *p == ':')
        {
            *p           = '\0';
            fixup_commit = p + 1;
            if (!strcmp("amend", fixup_message) || !strcmp("reword", fixup_message))
            {
                fixup_prefix = "amend";
                allow_empty  = 1;
                if (*fixup_message == 'r')
                             ...
                             const struct config_context *ctx, void *cb)
{
    ...
    if (skip_prefix(k, "status.color.", &slot_name) || skip_prefix(k, "color.status.", &slot_name))
    {
...
int cmd_commit(int argc, const char **argv, const char *prefix)
{
    ...
    if (fixup_message && starts_with(sb.buf, "amend! ") && !allow_empty_message)
    {
