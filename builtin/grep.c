                      ...
                      const struct pathspec *pathspec, int cached)
{
    ...
    for (nr = 0; nr < repo->index->cache_nr; nr++)
    {
        ...
        else if (S_ISREG(ce->ce_mode) && match_pathspec(repo->index, pathspec, name.buf, name.len, 0, NULL, S_ISDIR(ce->ce_mode) || S_ISGITLINK(ce->ce_mode)))
        {
            /*
        ...
        else if (recurse_submodules && S_ISGITLINK(ce->ce_mode) && submodule_path_match(repo->index, pathspec, name.buf, NULL))
        {
        ...
        if (ce_stage(ce))
        {
            ...
            {
                nr++;
            } while (nr < repo->index->cache_nr && !strcmp(ce->name, repo->index->cache[nr]->name));
...
int cmd_grep(int argc, const char **argv, const char *prefix)
{
    ...
    argc = parse_options(argc, argv, prefix, options, grep_usage,
                         PARSE_OPT_KEEP_DASHDASH | PARSE_OPT_STOP_AT_NON_OPTION);
    ...
    parse_pathspec(&pathspec, 0,
                   PATHSPEC_PREFER_CWD | (opt.max_depth != -1 ? PATHSPEC_MAXDEPTH_VALID : 0),
                   prefix, argv + i);
    ...
    if (num_threads > 1)
    {
        ...
        if (!(opt.name_only || opt.unmatch_name_only || opt.count)
            && (opt.pre_context || opt.post_context || opt.file_break || opt.funcbody))
        {
            skip_first_line = 1;
        }
