...
static void init_terms(struct bisect_terms *terms)
{
	terms->term_good = NULL;
	terms->term_bad = NULL;
}
...
static int bisect_reset(const char *commit)
{
	...
	if (commit) {
		...
		if (get_oid_commit(commit, &oid)) {
			error(_("'%s' is not a valid commit"), commit);
			return 1;
		}
...
static int check_and_set_terms(struct bisect_terms *terms, const char *cmd)
{
	...
	if (has_term_file && strcmp(cmd, terms->term_bad) &&
		...
		// Although this can be changed once it is not exposed
		// to cmd_bisect__helper.
		error(_("Invalid command: you're currently in a "
...
int cmd_bisect__helper(int argc, const char **argv, const char *prefix)
{
	...
	case BISECT_WRITE:
		if (argc != 4 && argc != 5) {
			error(_("--bisect-write requires either 4 or 5 arguments"));
			return 1;
		}
