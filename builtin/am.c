...
{
	assert(state->msg);
	int ret = run_hook_le(NULL, "applypatch-msg",
...
static int get_mail_commit_oid(struct object_id *commit_id, const char *mail)
{
	...
	const char *x;
	int ret = (strbuf_getline_lf(&sb, fp) ||
...
static int run_apply(const struct am_state *state, const char *index_file)
{
	...
	if (apply_parse_options(apply_opts.argc, apply_opts.argv, &apply_state,
				&force_apply, &options, NULL) != 0)
	...
	int res = apply_all_patches(&apply_state, apply_paths.argc,
...
static int show_patch(struct am_state *state)
{
	...
	if (!is_null_oid(&state->orig_commit)) {
		...
		int ret = run_command_v_opt(av, RUN_GIT_CMD);
		free(new_oid_str);
		return ret;
	}

	patch_path = am_path(state, msgnum(state));
	if (strbuf_read_file(&sb, patch_path, 0) < 0)
