...
static int refresh(int verbose, const struct pathspec *pathspec)
{
    ...
    for (i = 0; i < pathspec->nr; i++)
    {
        if (!seen[i])
        {
            ...
            if (matches_skip_worktree(pathspec, i, &skip_worktree_seen) || !path_in_sparse_checkout(path, the_repository->index))
            {
...
static int add_files(struct dir_struct *dir, int flags)
{
    ...
    for (i = 0; i < dir->nr; i++)
    {
        if (!include_sparse && !path_in_sparse_checkout(dir->entries[i]->name, the_repository->index))
        {
...
int cmd_add(int argc, const char **argv, const char *prefix)
{
    ...
    if (pathspec.nr)
    {
        ...
        GUARD_PATHSPEC(&pathspec,
                       PATHSPEC_FROMTOP | PATHSPEC_LITERAL | PATHSPEC_GLOB | PATHSPEC_ICASE | PATHSPEC_EXCLUDE | PATHSPEC_ATTR);
