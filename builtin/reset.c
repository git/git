                       ...
                       const char **rev_ret)
{
    ...
    if (argv[0])
    {
         ...
         * can not be a tree
         */
        else if ((!argv[1] && !repo_get_oid_committish(the_repository, argv[0], &unused)) || (argv[1] && !repo_get_oid_treeish(the_repository, argv[0], &unused)))
        {
            /*
    ...
    *rev_ret = !strcmp("@", rev) ? "HEAD" : rev;
    ...
    parse_pathspec(pathspec, 0,
                   PATHSPEC_PREFER_FULL | (patch_mode ? PATHSPEC_PREFIX_ORIGIN : 0),
                   prefix, argv);
}
...
int cmd_reset(int argc, const char **argv, const char *prefix)
{
    ...
    unborn = !strcmp(rev, "HEAD") && repo_get_oid(the_repository, "HEAD", &oid);
    if (unborn)
    {
