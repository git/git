...
#define OPT_CALLBACK_VALUE(s, l, v, h, i)                                                                    \
    {                                                                                                        \
        OPTION_CALLBACK, (s), (l), (v), NULL, (h), PARSE_OPT_NOARG | PARSE_OPT_NONEG, option_parse_type, (i) \
    }
...
static int cmd_config_actions(int argc, const char **argv, const char *prefix)
{
    ...
    if (display_opts.show_origin && !(actions & (ACTION_GET | ACTION_GET_ALL | ACTION_GET_REGEXP | ACTION_LIST)))
    {
        error(_(
            "--show-origin is only applicable to --get, --get-all, "
            "--get-regexp, and --list"));
        exit(129);
    }
    ...
    if (comment_arg && !(actions & (ACTION_ADD | ACTION_SET | ACTION_SET_ALL | ACTION_REPLACE_ALL)))
    {
