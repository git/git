...
int cmd_mv(int argc, const char **argv, const char *prefix)
{
    ...
    else if (!path_in_sparse_checkout(dst_w_slash, the_repository->index) && empty_dir_has_sparse_contents(dst_w_slash))
    {
        internal_prefix_pathspec(&destinations, dst_w_slash, argv, argc, DUP_BASENAME);
    ...
    for (i = 0; i < argc; i++)
    {
        ...
        if (lstat(src, &st) < 0)
        {
            ...
            {
                char *src_w_slash = add_slash(src);
                if (!path_in_sparse_checkout(src_w_slash, the_repository->index) && empty_dir_has_sparse_contents(src))
                {
                    free(src_w_slash);
        ...
        if (!strncmp(src, dst, length) && (dst[length] == 0 || dst[length] == '/'))
        {
    ...
    dir_check:
        ...
        if (ignore_sparse && (dst_mode & (SKIP_WORKTREE_DIR | SPARSE)) && index_entry_exists(the_repository->index, dst, strlen(dst)))
        {
         ...
         */
        if (!ignore_sparse && !path_in_sparse_checkout(src, the_repository->index))
        {
            string_list_append(&only_match_skip_worktree, src);
            skip_sparse = 1;
        }
        if (!ignore_sparse && !path_in_sparse_checkout(dst, the_repository->index))
        {
    ...
    for (i = 0; i < argc; i++)
    {
        ...
        if (ignore_sparse && core_apply_sparse_checkout && core_sparse_checkout_cone)
        {
             ...
             */
            if ((mode & SPARSE) && path_in_sparse_checkout(dst, the_repository->index))
            {
