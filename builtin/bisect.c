...
static GIT_PATH_FUNC(git_path_bisect_terms, "BISECT_TERMS") static GIT_PATH_FUNC(git_path_bisect_ancestors_ok, "BISECT_ANCESTORS_OK") static GIT_PATH_FUNC(git_path_bisect_start, "BISECT_START") static GIT_PATH_FUNC(git_path_bisect_log, "BISECT_LOG") static GIT_PATH_FUNC(git_path_bisect_names, "BISECT_NAMES") static GIT_PATH_FUNC(git_path_bisect_first_parent, "BISECT_FIRST_PARENT") static GIT_PATH_FUNC(git_path_bisect_run, "BISECT_RUN")
...
{
    return !res || res == BISECT_INTERNAL_SUCCESS_1ST_BAD_FOUND || res == BISECT_INTERNAL_SUCCESS_MERGE_BASE;
}
...
static int write_in_file(const char *path, const char *mode, const char *format, va_list args)
{
    ...
    if (strcmp(mode, "w") != 0 && strcmp(mode, "a"))
    {
...
__attribute__((format(printf, 2, 3))) static int write_to_file(const char *path, const char *format, ...)
{
    int     res;
    va_list args;
...
__attribute__((format(printf, 2, 3))) static int append_to_file(const char *path, const char *format, ...)
{
    int     res;
    va_list args;
...
static int check_term_format(const char *term, const char *orig_term)
{
    ...
    if ((strcmp(orig_term, "bad") != 0 && one_of(term, "bad", "new", NULL)) || (strcmp(orig_term, "good") != 0 && one_of(term, "good", "old", NULL)))
    {
...
static int check_and_set_terms(struct bisect_terms *terms, const char *cmd)
{
    ...
    if (has_term_file && strcmp(cmd, terms->term_bad) != 0 && strcmp(cmd, terms->term_good) != 0)
    {
                       ...
                       int missing_bad)
{
    ...
    if (missing_good && !missing_bad && !strcmp(current_term, terms->term_good))
    {
        char *yesno;
        /*
...
__attribute__((format(printf, 1, 2))) static void bisect_log_printf(const char *fmt, ...)
{
                                      ...
                                      const char **argv)
{
    ...
    for (i = 0; i < argc; i++)
    {
        ...
        else if (skip_prefix(arg, "--term-good=", &arg) || skip_prefix(arg, "--term-old=", &arg))
        {
            must_write_terms = 1;
            free((void *)terms->term_good);
        ...
        else if (skip_prefix(arg, "--term-bad=", &arg) || skip_prefix(arg, "--term-new=", &arg))
        {
            must_write_terms = 1;
            free((void *)terms->term_bad);
    ...
    else
    {
        ...
        if (!repo_get_oid(the_repository, head, &head_oid) && !starts_with(head, "refs/heads/"))
        {
            strbuf_reset(&start_head);
            strbuf_addstr(&start_head, oid_to_hex(&head_oid));
        }
        else if (!repo_get_oid(the_repository, head, &head_oid) && skip_prefix(head, "refs/heads/", &head))
        {
    ...
    if (must_write_terms && write_terms(terms->term_bad, terms->term_good))
    {
        res = BISECT_FAILED;
        goto finish;
    }
...
static int bisect_autostart(struct bisect_terms *terms)
{
    ...
    yesno = git_prompt(_("Do you want me to do it for you "
                         "[Y/n]? "),
                       PROMPT_ECHO);
    res   = tolower(*yesno) == 'n' ? -1 : bisect_start(terms, 0, empty_strvec);
                                      ...
                                      const char **argv)
{
    ...
    state = argv[0];
    if (check_and_set_terms(terms, state) || !one_of(state, terms->term_good, terms->term_bad, "skip", NULL))
    {
        return BISECT_FAILED;
    }
...
static int process_replay_line(struct bisect_terms *terms, struct strbuf *line)
{
    ...
    if ((!skip_prefix(p, "git bisect", &p) && !skip_prefix(p, "git-bisect", &p)) || !isspace(*p))
    {
        return 0;
    }
    p += strspn(p, " \t");
                            ...
                            const char **argv)
{
    ...
    if (!argc)
    {
        if ((getenv("DISPLAY") || getenv("SESSIONNAME") || getenv("MSYSTEM") || getenv("SECURITYSESSIONID")) && exists_in_PATH("gitk"))
        {
    ...
    else
    {
        ...
        else if (strcmp(argv[0], "tig") != 0 && !starts_with(argv[0], "git"))
        {
            cmd.git_cmd = 1;
        }
...
static int bisect_run(struct bisect_terms *terms, int argc, const char **argv)
{
    ...
    while (1)
    {
        ...
        temporary_stdout_fd = open(git_path_bisect_run(), O_CREAT | O_WRONLY | O_TRUNC | O_CLOEXEC, 0666);
        ...
        fflush(stdout);
        saved_stdout = fcntl(1, F_DUPFD_CLOEXEC);
