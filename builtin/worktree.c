...
static void copy_sparse_checkout(const char *worktree_git_dir)
{
    ...
    {
        if (safe_create_leading_directories(to_file) || copy_file(to_file, from_file, 0666))
        {
...
static void copy_filtered_worktree_config(const char *worktree_git_dir)
{
    ...
    if (file_exists(from_file))
    {
        ...
        if (!git_configset_get_bool(&cs, "core.bare", &bare) && bare && git_config_set_multivar_in_file_gently(to_file, "core.bare", NULL, "true", NULL, 0))
        {
            error(_("failed to unset '%s' in '%s'"),
                  "core.bare", to_file);
        }
        if (!git_configset_get(&cs, "core.worktree") && git_config_set_in_file_gently(to_file, "core.worktree", NULL, NULL))
        {
            error(_("failed to unset '%s' in '%s'"),
...
{
    struct strbuf     sb_git   = STRBUF_INIT;
    struct strbuf     sb_repo  = STRBUF_INIT;
    struct strbuf     sb       = STRBUF_INIT;
    ...
    if (!opts->detach && !strbuf_check_branch_ref(&symref, refname) && refs_ref_exists(get_main_ref_store(the_repository), symref.buf))
    {
        is_branch = 1;
        if (!opts->force)
        {
    ...
    if (opts->orphan && (ret = make_worktree_orphan(refname, opts, &child_env)))
    {
        goto done;
    }

    if (opts->checkout && (ret = checkout_worktree(opts, &child_env)))
    {
        goto done;
    }
...
static int can_use_local_refs(const struct add_opts *opts)
{
    ...
    if (refs_for_each_branch_ref(get_main_ref_store(the_repository), first_valid_ref, NULL))
    {
        if (!opts->quiet)
        {
...
static int can_use_remote_refs(const struct add_opts *opts)
{
    ...
    if (refs_for_each_remote_ref(get_main_ref_store(the_repository), first_valid_ref, NULL))
    {
        return 1;
    }
...
static char *dwim_branch(const char *path, char **new_branch)
{
    ...
    branch_exists = !strbuf_check_branch_ref(&ref, branchname) && refs_ref_exists(get_main_ref_store(the_repository), ref.buf);
    strbuf_release(&ref);
    if (branch_exists)
    {
        return branchname;
...
static int add(int ac, const char **av, const char *prefix)
{
    ...
    if (new_branch_force)
    {
        ...
        if (!opts.force && !strbuf_check_branch_ref(&symref, new_branch) && refs_ref_exists(get_main_ref_store(the_repository), symref.buf))
        {
            die_if_checked_out(symref.buf, 0);
        }
        strbuf_release(&symref);
