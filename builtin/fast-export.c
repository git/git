...
static enum tag_of_filtered_mode
{
    TAG_FILTERING_ABORT,
    DROP,
    REWRITE
} tag_of_filtered_mode = TAG_FILTERING_ABORT;
static enum reencode_mode
{
    REENCODE_ABORT,
    REENCODE_YES,
    REENCODE_NO
...
static int anonymized_entry_cmp(const void *cmp_data        UNUSED,
                                ...
                                const void                 *keydata)
{
    const struct anonymized_entry *a;
    const struct anonymized_entry *b;
            ...
            case DIFF_STATUS_ADDED:
                ...
                if (no_data || S_ISGITLINK(spec->mode))
                {
                    printf("M %06o %s ", spec->mode,
                           anonymize ? anonymize_oid(oid_to_hex(&spec->oid)) : oid_to_hex(&spec->oid));
                }
                else
                {
...
static void anonymize_ident_line(const char **beg, const char **end)
{
    ...
    if (!split_ident_line(&split, end_of_header, *end - end_of_header) && split.date_begin)
    {
        const char *ident;
        size_t      len;
                          ...
                          struct string_list *paths_of_changed_objects)
{
    ...
    if (commit->parents && (get_object_mark(&commit->parents->item->object) != 0 || reference_excluded_commits) && !full_tree)
    {
...
int cmd_fast_export(int argc, const char **argv, const char *prefix)
{
    ...
    char              *import_filename_if_exists = NULL;
