                        ...
                        int unknown_type)
{
    ...
    unsigned              get_oid_flags =
        GET_OID_RECORD_PATH | GET_OID_ONLY_TO_DIE | GET_OID_HASH_ANY;
                             ...
                             struct expand_data   *data)
{
    ...
    int                   flags =
        GET_OID_HASH_ANY | (opt->follow_symlinks ? GET_OID_FOLLOW_SYMLINKS : 0);
                                  ...
                                  struct expand_data   *data)
{
    ...
    if (opt->buffer_output && nr && !git_env_bool("GIT_TEST_CAT_FILE_NO_FLUSH_ON_EXIT", 0))
    {
        dispatch_calls(opt, output, data, queued_cmd, nr);
        free_cmds(queued_cmd, &nr);
    }
