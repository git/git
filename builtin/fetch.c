                            ...
                            const struct config_context *ctx, void *cb)
{
    ...
    {
        int r                            = git_config_bool(k, v) ? RECURSE_SUBMODULES_ON : RECURSE_SUBMODULES_OFF;
                                ...
                                struct ref           ***tail)
{
    ...
    for (ref = refs; ref; ref = ref->next)
    {
        ...
        {
            if (item && !repo_has_object_file_with_flags(the_repository, &ref->old_oid, quick_flags) && !oidset_contains(&fetch_oids, &ref->old_oid) && !repo_has_object_file_with_flags(the_repository, &item->oid, quick_flags) && !oidset_contains(&fetch_oids, &item->oid))
            {
                clear_item(item);
            }
            item = NULL;
         ...
         * fetch.
         */
        if (item && !repo_has_object_file_with_flags(the_repository, &item->oid, quick_flags) && !oidset_contains(&fetch_oids, &item->oid))
        {
            clear_item(item);
        }
        ...
        if (refname_hash_exists(&remote_refs, ref->name) || refname_hash_exists(&existing_refs, ref->name))
        {
            continue;
        }
...
static void filter_prefetch_refspec(struct refspec *rs)
{
    ...
    for (i = 0; i < rs->nr; i++)
    {
        ...
        if (!rs->items[i].dst || (rs->items[i].src && starts_with(rs->items[i].src, ref_namespace[NAMESPACE_TAGS].ref)))
        {
            int j;
                               ...
                               int tags, int *autotags)
{
    ...
    struct ref  *orefs     = NULL;
    struct ref **oref_tail = &orefs;
    ...
    else
    {
                       ...
                       (has_merge && !strcmp(branch->remote_name, remote->name))))
        {
            for (i = 0; i < remote->fetch.nr; i++)
            {
                ...
                if (remote->fetch.items[i].dst && remote->fetch.items[i].dst[0])
                {
                    *autotags = 1;
                }
                if (!i && !has_merge && ref_map && !remote->fetch.items[0].pattern)
                {
...
static int refcol_width(const struct ref *ref_map, int compact_format)
{
    ...
    for (ref = ref_map; ref; ref = ref->next)
    {
        ...
        if (ref->status == REF_STATUS_REJECT_SHALLOW || !ref->peer_ref || !strcmp(ref->name, "HEAD"))
        {
            continue;
        }
...
{
    struct commit *current = NULL;
    struct commit *updated;
    int            fast_forward = 0;
    ...
    if (!update_head_ok && !is_null_oid(&ref->old_oid) && branch_checked_out(ref->name))
    {
        /*
    ...
    if (!is_null_oid(&ref->old_oid) && starts_with(ref->name, "refs/tags/"))
    {
        if (force || ref->force)
        {
            int r;
                              ...
                              const struct fetch_config *config)
{
         ...
         want_status++)
    {
        for (rm = ref_map; rm; rm = rm->next)
        {
            ...
            if (config->recurse_submodules != RECURSE_SUBMODULES_OFF && (!rm->peer_ref || !oideq(&ref->old_oid, &ref->new_oid)))
            {
                      ...
                      struct ref             *ref_map)
{
    int           result = 0;
    struct ref   *ref;
    struct ref   *stale_refs   = get_stale_heads(rs, ref_map);
    struct strbuf err          = STRBUF_INIT;
...
static void check_not_current_branch(struct ref *ref_map)
{
    ...
    {
        if (ref_map->peer_ref && starts_with(ref_map->peer_ref->name, "refs/heads/") && (path = branch_checked_out(ref_map->peer_ref->name)))
        {
                    ...
                    const struct fetch_config *config)
{
    ...
    else
    {
        ...
        if (branch_has_merge_config(branch) && !strcmp(branch->remote_name, transport->remote->name))
        {
            int i;
            for (i = 0; i < branch->merge_nr; i++)
            {
    ...
    if (tags == TAGS_DEFAULT && autotags)
    {
        struct ref  *tags_ref_map = NULL;
        struct ref **tail         = &tags_ref_map;
...
int cmd_fetch(int argc, const char **argv, const char *prefix)
{
    ...
    if (!git_config_get_string_tmp("fetch.bundleuri", &bundle_uri) && fetch_bundle_uri(the_repository, bundle_uri, NULL))
    {
    ...
    else
    {
        ...
        {
            die(_(
                "--atomic can only be used when fetching "
                "from one remote"));
        }
        ...
        {
            die(_(
                "--stdin can only be used when fetching "
                "from one remote"));
        }
