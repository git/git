                             ...
                             unsigned int limit)
{
    ...
    if (regexp)
    {
        ...
        while (sz && !regexec_buf(regexp, data, sz, 1, &regmatch, flags))
        {
            flags |= REG_NOTBOL;
            data += regmatch.rm_eo;
            sz -= regmatch.rm_eo;
                         ...
                         regex_t *regexp, kwset_t kws, pickaxe_fn fn)
{
    ...
    if (o->objfind)
    {
        return (DIFF_FILE_VALID(p->one) && oidset_contains(o->objfind, &p->one->oid)) || (DIFF_FILE_VALID(p->two) && oidset_contains(o->objfind, &p->two->oid));
    }
    ...
    if ((o->pickaxe_opts & DIFF_PICKAXE_KIND_G) && !o->flags.text && ((!textconv_one && diff_filespec_is_binary(o->repo, p->one)) || (!textconv_two && diff_filespec_is_binary(o->repo, p->two))))
        return 0;
...
void diffcore_pickaxe(struct diff_options *o)
{
    ...
    {
        if (o->pickaxe_opts & DIFF_PICKAXE_IGNORE_CASE && has_non_ascii(needle))
        {
