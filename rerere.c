...
static int check_one_conflict(struct index_state *istate, int i, int *type)
{
    ...
    *type = PUNTED;
    ...
    if (i + 1 < istate->cache_nr)
    {
        ...
        if (ce_stage(e2) == 2 && ce_stage(e3) == 3 && ce_same_name(e, e3) && S_ISREG(e2->ce_mode) && S_ISREG(e3->ce_mode))
        {
            *type = THREE_STAGED;
        }
    }
...
static int merge(struct index_state *istate, const struct rerere_id *id, const char *path)
{
     ...
     */
    if ((handle_file(istate, path, NULL, rerere_path(id, "thisimage")) < 0) || read_mmfile(&cur, rerere_path(id, "thisimage")))
    {
        ret = 1;
        goto out;
    }
                        ...
                        const char *path, unsigned char *hash, const char *output)
{
    ...
    while (pos < istate->cache_nr)
    {
        ...
        ce = istate->cache[pos++];
        if (ce_namelen(ce) != len || memcmp(ce->name, path, len) != 0)
        {
            break;
        }
        i = ce_stage(ce) - 1;
