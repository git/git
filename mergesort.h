...
#define DEFINE_LIST_SORT_INTERNAL(scope, name, type)                          \
 scope void name(type **listp, int (*compare_fn)(const type *, const type *)) \
 {                                                                            \
  type *list = *listp;                                                        \
  ...
  for (;;) {                                                                  \
   ...
   for (i = 0, m = n;; i++, m >>= 1) {                                        \
    if (m & 1) {                                                              \
     list = name##__merge(ranks[i], list, compare_fn);                        \
    } else if (next) {                                                        \
     break;                                                                   \
    } else if (!m) {                                                          \
     *listp = list;                                                           \
...
#define DEFINE_LIST_SORT_DEBUG(scope, name, type, next_member, on_get_next, \
			       on_set_next)                                 \
                                                                            \
