...
static int verify_cache(struct index_state *istate, int flags)
{
    ...
    for (i = 0; i + 1 < istate->cache_nr; i++)
    {
        ...
        if (this_len < ce_namelen(next_ce) && next_name[this_len] == '/' && strncmp(this_name, next_name, this_len) == 0)
        {
            if (10 < ++funny)
            {
                fprintf(stderr, "...\n");
                      ...
                      int                  flags)
{
    ...
    *skip_count = 0;
    ...
    if (entries > 0)
    {
        ...
        if (S_ISSPARSEDIR(ce->ce_mode) && ce->ce_namelen == baselen && !strncmp(ce->name, base, baselen))
        {
            it->entry_count = 1;
    ...
    while (i < entries)
    {
        ...
        if (pathlen <= baselen || memcmp(base, path, baselen) != 0)
        {
    ...
    while (i < entries)
    {
        ...
        if (is_null_oid(oid) || (!ce_missing_ok && !repo_has_object_file(the_repository, oid)))
        {
    ...
    else if (dryrun)
    {
        hash_object_file(the_hash_algo, buffer.buf, buffer.len,
                         OBJ_TREE, &it->oid);
                                     ...
                                     &it->oid, NULL, flags & WRITE_TREE_SILENT ? HASH_SILENT : 0))
    {
        strbuf_release(&buffer);
        return -1;
    }
...
struct tree *write_in_core_index_as_tree(struct repository *repo)
{
    ...
    was_valid                       = index_state->cache_tree && cache_tree_fully_valid(index_state->cache_tree);
...
int write_index_as_tree(struct object_id *oid, struct index_state *index_state, const char *index_path, int flags, const char *prefix)
{
    ...
    was_valid = !(flags & WRITE_TREE_IGNORE_CACHE_TREE) && index_state->cache_tree && cache_tree_fully_valid(index_state->cache_tree);
                                 ...
                                 struct strbuf     *tree_path)
{
    ...
    while (tree_entry(&desc, &entry))
    {
        ...
        else
        {
             ...
             * as normal.
             */
            if (r->index->sparse_index && index_entry_exists(r->index, tree_path->buf, tree_path->len))
            {
