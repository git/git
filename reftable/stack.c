                                             ...
                                             int                    reuse_open)
{
    ...
    while (1)
    {
        ...
        fd = open(st->list_file, O_RDONLY | O_CLOEXEC);
        if (fd < 0)
        {
            if (errno != ENOENT)
            {
...
out:
     ...
     */
    if (!err && fd >= 0 && !fstat(fd, &st->list_st) && st->list_st.st_dev && st->list_st.st_ino)
    {
        st->list_fd = fd;
        fd          = -1;
    }
...
static int stack_uptodate(struct reftable_stack *st)
{
    ...
    if (st->list_fd >= 0)
    {
         ...
         * directly.
         */
        if (st->list_st.st_dev == list_st.st_dev && st->list_st.st_ino == list_st.st_ino)
        {
            return 0;
        }
    }
    ...
    for (i = 0; i < st->readers_len; i++)
    {
        ...
        if (strcmp(st->readers[i]->name, names[i]) != 0)
        {
            err = 1;
            goto done;
        }
...
uint64_t reftable_stack_next_update_index(struct reftable_stack *st)
{
    ...
    if (sz > 0)
    {
        return reftable_reader_max_update_index(st->readers[sz - 1]) + 1;
    }
    return 1;
}
                                ...
                                struct tempfile                  **tab_file_out)
{
    ...
    if (st->opts.default_permissions && chmod(get_tempfile_path(tab_file), st->opts.default_permissions) < 0)
    {
        err = REFTABLE_IO_ERROR;
        goto done;
    }
                               ...
                               unsigned int                       flags)
{
    ...
    int               err              = 0;
    size_t            first_to_replace;
    size_t            last_to_replace;
    size_t            i;
    size_t            nlocks = 0;
    char            **names  = NULL;
    ...
    for (i = last + 1; i > first; i--)
    {
        ...
        if (err < 0)
        {
             ...
             */
            if (errno == EEXIST && last - (i - 1) >= 2 && flags & STACK_COMPACT_RANGE_BEST_EFFORT)
            {
                err = 0;
                /*
    ...
    if (err > 0)
    {
        ...
        {
            if (strcmp(names[i], st->readers[first]->name) != 0)
            {
                continue;
            }
            ...
            {
                const char *old = first + j < st->merged->stack_len ? st->readers[first + j]->name : NULL;
                            ...
                            struct reftable_log_record *log)
{
    ...
    if (strcmp(log->refname, refname) != 0 || reftable_log_record_is_deletion(log))
    {
        err = 1;
        goto done;
    }
