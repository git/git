                            ...
                            struct reftable_ref_record *ref)
{
    ...
    if (ref->update_index < w->min_update_index || ref->update_index > w->max_update_index)
        return REFTABLE_API_ERROR;
                                            ...
                                            struct reftable_log_record *log)
{
    ...
    if (w->block_writer && block_writer_type(w->block_writer) == BLOCK_TYPE_REF)
    {
                            ...
                            struct reftable_log_record *log)
{
    ...
    if (!w->opts.exact_log_message && log->value.update.message)
    {
        ...
        while (cleaned_message.len && cleaned_message.buf[cleaned_message.len - 1] == '\n')
