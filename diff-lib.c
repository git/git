                         ...
                         int cached, int match_missing)
{
    ...
    assert(S_ISSPARSEDIR(old_entry->ce_mode) == S_ISSPARSEDIR(new_entry->ce_mode));
                           ...
                           const struct cache_entry    *tree)
{
     ...
     */
    if (o->index_only && revs->diffopt.ita_invisible_in_index && idx && ce_intent_to_add(idx))
    {
        idx = NULL;
        if (!tree)
        {
    ...
    cached = o->index_only || (idx && ((idx->ce_flags & CE_VALID) || ce_skip_worktree(idx)));
                      ...
                      int                     cached)
{
    ...
    opts.diff_index_cached   = (cached && !revs->diffopt.flags.find_copies_harder);
    opts.merge               = 1;
    opts.fn                  = oneway_diff;
