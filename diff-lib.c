...
void run_diff_files(struct rev_info *revs, unsigned int option)
{
	...
	for (i = 0; i < entries; i++) {
		...
		if (ce_stage(ce)) {
			...
			if (!changed)
				wt_mode = ce_mode_from_stat(ce, st.st_mode, istate->repo);
			else {
				if (changed < 0) {
					perror(ce->name);
					continue;
			...
			while (i < entries) {
				...
				if (2 <= stage) {
					...
					oidcpy(&dpath->parent[stage - 2].oid,
					       &nce->oid);
					dpath->parent[stage-2].mode = ce_mode_from_stat(nce, mode, istate->repo);
		...
		if (ce->ce_flags & (CE_VALID | CE_FSMONITOR_VALID)) {
			...
			if (changed) {
				...
				continue;
				   ...
				   ce_intent_to_add(ce)) {
				newmode = ce_mode_from_stat(ce, st.st_mode, istate->repo);
			 ...
			 unsigned *dirty_submodule, struct diff_options *diffopt)
{
	...
	if (!cached && !ce_uptodate(ce)) {
		...
		changed = match_stat_with_submodule(diffopt, ce, &st,
						    0, dirty_submodule);
		if (changed) {
			mode = ce_mode_from_stat(ce, st.st_mode, diffopt->repo);
