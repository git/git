...
struct replay_opts {
	...
	int drop_redundant_cummits;
	int keep_redundant_cummits;
	int verbose;
	int quiet;
	...
	char *gpg_sign;
	enum cummit_msg_cleanup_mode default_msg_cleanup;
...
enum todo_command {
	...
	TODO_FIXUP,
	TODO_SQUASH,
	/* commands that do something else than handling a single cummit */
	TODO_EXEC,
	TODO_BREAK,
	TODO_LABEL,
	TODO_RESET,
...
/*
 * When rebasing merges, cummits that do have the base cummit as ancestor
 * ("cousins") are *not* rebased onto the new base by default. If those
 * cummits should be rebased onto the new base, this flag needs to be passed.
 */
#define TODO_LIST_REBASE_COUSINS (1U << 4)
#define TODO_LIST_APPEND_TODO_HELP (1U << 5)
/*
 * When generating a script that rebases merges with `--root` *and* with
 * `--onto`, we do not want to re-generate the root cummits.
 */
...
/*
 * Append a signoff to the cummit message in "msgbuf". The ignore_footer
...
void cummit_post_rewrite(struct repository *r,
			 const struct cummit *current_head,
			 const struct object_id *new_head);
