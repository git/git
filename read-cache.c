...
{
	if (!repo_has_symlinks && S_ISREG(mode) &&
	    ce && S_ISLNK(ce->ce_mode))
		return ce->ce_mode;
	if (!repo_trust_executable_bit && S_ISREG(mode)) {
...
static unsigned int st_mode_from_ce(const struct cache_entry *ce)
{
	...
	case S_IFLNK:
		return repo_has_symlinks ? S_IFLNK : (S_IFREG | 0644);
	case S_IFREG:
		return (ce->ce_mode & (repo_trust_executable_bit ? 0755 : 0644)) | S_IFREG;
	case S_IFGITLINK:
		return S_IFDIR | 0755;
	case S_IFDIR:
		return ce->ce_mode;
...
static int ce_match_stat_basic(const struct cache_entry *ce, struct stat *st)
{
	...
	case S_IFREG:
		...
		if (0100 & (ce->ce_mode ^ st->st_mode)) {
			if (repo_trust_executable_bit)
				changed |= MODE_CHANGED;
		}
		break;
	case S_IFLNK:
		if (!S_ISLNK(st->st_mode) &&
		    (repo_has_symlinks || !S_ISREG(st->st_mode)))
...
int add_to_index(struct index_state *istate, const char *path, struct stat *st, int flags)
{
	...
	if (repo_has_symlinks && repo_trust_executable_bit) {
