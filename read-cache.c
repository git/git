...
{
    struct cache_entry *old_entry = istate->cache[nr];
    struct cache_entry *new_entry;
    struct cache_entry *refreshed;
...
static int ce_match_stat_basic(const struct cache_entry *ce, struct stat *st)
{
    ...
    switch (ce->ce_mode & S_IFMT)
    {
        case S_IFREG:
             ...
             * "mode changes"
             */
            if (trust_executable_bit && (0100 & (ce->ce_mode ^ st->st_mode)))
            {
                        ...
                        const struct stat_data   *sd)
{
    return (istate->timestamp.sec &&
            ...
            (istate->timestamp.sec < sd->sd_mtime.sec || (istate->timestamp.sec == sd->sd_mtime.sec && istate->timestamp.nsec <= sd->sd_mtime.nsec))
#else
...
int is_racy_timestamp(const struct index_state *istate,
                      const struct cache_entry *ce)
{
    return (!S_ISGITLINK(ce->ce_mode) && is_racy_stat(istate, &ce->ce_stat_data));
}
                ...
                struct stat *st, unsigned int options)
{
     ...
     * then we know it is.
     */
    if ((changed & DATA_CHANGED) && (S_ISGITLINK(ce->ce_mode) || ce->ce_stat_data.sd_size != 0))
    {
        return changed;
    }
                                ...
                                enum index_search_mode search_mode)
{
    ...
    if (search_mode == EXPAND_SPARSE && istate->sparse_index && first > 0)
    {
         ...
         */
        if (S_ISSPARSEDIR(ce->ce_mode) && ce_namelen(ce) < namelen && !strncmp(name, ce->name, ce_namelen(ce)))
        {
...
{
    return namelen != ce_namelen(ce) || memcmp(path, ce->name, namelen) != 0;
}
                                        ...
                                        const char *path, int namelen)
{
    ...
    /* maybe unmerged? */
    pos = -1 - pos;
    if (pos >= istate->cache_nr || compare_name((ce = istate->cache[pos]), path, namelen))
    {
        return -1;
    }

    /* order of preference: stage 2, 1, 3 */
    if (ce_stage(ce) == 1 && pos + 1 < istate->cache_nr && ce_stage((ce = istate->cache[pos + 1])) == 2 && !compare_name(ce, path, namelen))
    {
        pos++;
    }
    return pos;
...
static int different_name(struct cache_entry *ce, struct cache_entry *alias)
{
    ...
    return ce_namelen(alias) != len || memcmp(ce->name, alias->name, len) != 0;
}
...
static enum verify_path_result verify_path_internal(const char * /*path*/, unsigned /*mode*/);
...
static int verify_dotfile(const char *rest, unsigned mode)
{
    ...
    switch (*rest)
    {
        ...
        case 'G':
            ...
            {
                rest += 3;
                if (skip_iprefix(rest, "modules", &rest) && (*rest == '\0' || is_dir_sep(*rest)))
                {
                    return 0;
                }
            }
                                                    ...
                                                    unsigned    mode)
{
    ...
    for (;;)
    {
        ...
        inside:
            ...
            c = *path++;
            if ((c == '.' && !verify_dotfile(path, mode)) || is_dir_sep(c))
            {
                return PATH_INVALID;
            }
            /*
            ...
            if (c == '\0')
            {
                return S_ISDIR(mode) ? PATH_DIR_WITH_SEP : PATH_INVALID;
            }
        }
                        ...
                        const struct cache_entry *ce, int pos, int ok_to_replace)
{
    ...
    for (;;)
    {
        ...
        {
            struct cache_entry *p = istate->cache[pos];
            if ((ce_namelen(p) <= len) || (p->name[len] != '/') || memcmp(p->name, name, len) != 0)
            {
...
static int add_index_entry_with_check(struct index_state *istate, struct cache_entry *ce, int option)
{
     ...
     */
    if (istate->cache_nr > 0 && strcmp(ce->name, istate->cache[istate->cache_nr - 1]->name) > 0)
    {
    ...
    if (!skip_df_check && check_file_directory_conflict(istate, ce, pos, ok_to_replace))
    {
        if (!ok_to_replace)
        {
                  ...
                  char *seen, const char *header_msg)
{
    ...
    unsigned int     options              = (CE_MATCH_REFRESH | (really ? CE_MATCH_IGNORE_VALID : 0) | (not_new ? CE_MATCH_IGNORE_MISSING : 0));
    ...
    for (i = 0; i < istate->cache_nr; i++)
    {
        ...
        if (ce_stage(ce))
        {
            while ((i < istate->cache_nr) && !strcmp(istate->cache[i]->name, ce->name))
            {
                i++;
            }
            i--;
...
static void set_new_index_sparsity(struct index_state *istate)
{
    ...
    if (!istate->repo->settings.command_requires_full_index && is_sparse_index_allowed(istate, 0))
    {
...
void validate_cache_entries(const struct index_state *istate)
{
    ...
    for (i = 0; i < istate->cache_nr; i++)
    {
        ...
        else if (!istate->ce_mem_pool || !mem_pool_contains(istate->ce_mem_pool, istate->cache[i]))
        {
            if (!istate->split_index || !istate->split_index->base || !istate->split_index->base->ce_mem_pool || !mem_pool_contains(istate->split_index->base->ce_mem_pool, istate->cache[i]))
            {
                BUG("cache entry is not allocated from expected memory pool");
            }
        }
                          ...
                          struct strbuf *previous_name, struct ondisk_cache_entry *ondisk)
{
    ...
    else
    {
        ...
        for (common = 0;
             (ce->name[common] && common < previous_name->len && ce->name[common] == previous_name->buf[common]);
             common++)
        {
            ; /* still matching */
        }
...
{
    if ((repo->index->cache_changed || has_racy_timestamp(repo->index)) && repo_verify_index(repo))
    {
                          ...
                          enum write_extensions write_extensions, unsigned flags)
{
    ...
    if (write_extensions & WRITE_SPLIT_INDEX_EXTENSION && istate->split_index)
    {
        strbuf_reset(&sb);
        ...
        err = write_link_extension(&sb, istate) < 0 || write_index_ext_header(f, eoie_c, CACHE_EXT_LINK, sb.len) < 0;
        hashwrite(f, sb.buf, sb.len);
        if (err)
        {
            ret = -1;
    ...
    if (write_extensions & WRITE_CACHE_TREE_EXTENSION && !drop_cache_tree && istate->cache_tree)
    {
        strbuf_reset(&sb);
    ...
    if (write_extensions & WRITE_RESOLVE_UNDO_EXTENSION && istate->resolve_undo)
    {
        strbuf_reset(&sb);
    ...
    if (write_extensions & WRITE_FSMONITOR_EXTENSION && istate->fsmonitor_last_update)
    {
        strbuf_reset(&sb);
...
static int clean_shared_index_files(const char *current_hex)
{
    ...
    while ((de = readdir(dir)) != NULL)
    {
        ...
        if (should_delete_shared_index(shared_index_path) > 0 && unlink(shared_index_path))
        {
...
int write_locked_index(struct index_state *istate, struct lock_file *lock,
                       unsigned flags)
{
    int                 new_shared_index;
    int                 ret;
    int                 test_split_index_env;
    struct split_index *si = istate->split_index;
    ...
    if ((!si && !test_split_index_env) || alternate_index_output || (istate->cache_changed & ~EXTMASK))
    {
                                ...
                                const char *path, unsigned long *size)
{
    ...
    if (pos < 0)
    {
        ...
        int i;
        for (i = -pos - 1;
             (pos < 0 && i < istate->cache_nr && !strcmp(istate->cache[i]->name, path));
             i++)
        {
