...
static int is_multipart_boundary(struct mailinfo *mi, const struct strbuf *line)
{
    ...
    return ((content_top->len <= line->len) && !memcmp(line->buf, content_top->buf, content_top->len));
}
                        ...
                        struct strbuf *hdr_data[], int overwrite)
{
    ...
    for (i = 0; header[i]; i++)
    {
        if ((!hdr_data[i] || overwrite) && parse_header(line, header[i], mi, &sb))
        {
...
static int is_scissors_line(const char *line)
{
    ...
    for (c = line; *c; c++)
    {
        ...
        if (starts_with(c, ">8") || starts_with(c, "8<") || starts_with(c, ">%") || starts_with(c, "%<"))
        {
            in_perforation = 1;
            perforation += 2;
            scissors += 2;
...
{
    if (mi->inbody_header_accum.len && (line->buf[0] == ' ' || line->buf[0] == '\t'))
    {
...
again:
    if (line->len >= (*(mi->content_top))->len + 2 && !memcmp(line->buf + (*(mi->content_top))->len, "--", 2))
    {
                                 ...
                                 struct strbuf *prev)
{
    ...
    if (!mi->format_flowed)
    {
        if (len >= 2 && line->buf[len - 2] == '\r' && line->buf[len - 1] == '\n')
        {
            mi->have_quoted_cr = 1;
