...
static int load_one_loose_object_map(struct repository *repo, struct object_directory *dir)
{
    ...
    errno = 0;
    if (strbuf_getwholeline(&buf, fp, '\n') || strcmp(buf.buf, loose_object_header) != 0)
    {
        goto err;
    }
    while (!strbuf_getline_lf(&buf, fp))
    {
        ...
        if (parse_oid_hex_algop(buf.buf, &oid, &p, repo->hash_algo) || *p++ != ' ' || parse_oid_hex_algop(p, &compat_oid, &p, repo->compat_hash_algo) || p != buf.buf + buf.len)
        {
            goto err;
        }
...
int repo_write_loose_object_map(struct repository *repo)
{
    ...
    for (; iter != kh_end(map); iter++)
    {
        ...
        {
            if (oideq(&kh_key(map, iter), the_hash_algo->empty_tree) || oideq(&kh_key(map, iter), the_hash_algo->empty_blob))
            {
                continue;
            }
                            ...
                            const struct object_id *compat_oid)
{
    ...
    fd = open(path.buf, O_WRONLY | O_CREAT | O_APPEND | O_CLOEXEC, 0666);
    if (fd < 0)
    {
        goto errout;
    }
                              ...
                              struct object_id           *dest)
{
    ...
    for (dir = repo->objects->odb; dir; dir = dir->next)
    {
        ...
        map = (to == repo->compat_hash_algo) ? loose_map->to_compat : loose_map->to_storage;
