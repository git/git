...
{
    struct commit_dist *a;
    struct commit_dist *b;
...
static enum bisect_error check_merge_bases(int rev_nr, struct commit **rev, int no_checkout)
{
    ...
    for (; result; result = result->next)
    {
        ...
        if (0 <= oid_array_lookup(&good_revs, mb))
        {
            continue;
        }
