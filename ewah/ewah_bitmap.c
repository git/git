...
static size_t add_empty_word(struct ewah_bitmap *self, int v)
{
    ...
    if (no_literal && rlw_get_run_bit(self->rlw) == v && run_len < RLW_LARGEST_RUNNING_COUNT)
    {
...
{
    const size_t dist =
        DIV_ROUND_UP(i + 1, BITS_IN_EWORD) - DIV_ROUND_UP(self->bit_size, BITS_IN_EWORD);
    ...
    struct ewah_bitmap *out)
{
    ...
    while (rlwit_word_size(&rlw_i) > 0 && rlwit_word_size(&rlw_j) > 0)
    {
        ...
        if (literals)
        {
            ...
            {
                ewah_add(out,
                         rlw_i.buffer[rlw_i.literal_word_start + k] ^ rlw_j.buffer[rlw_j.literal_word_start + k]);
            }
