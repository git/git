                             ...
                             struct strbuf *type)
{
     ...
     */
    if (!skip_prefix(type->buf, "application/x-", &p) || !skip_prefix(p, service, &p) || strcmp(p, "-advertisement"))
        return;
...
static struct discovery *discover_refs(const char *service, int for_push)
{
    ...
    if ((starts_with(url.buf, "http://") || starts_with(url.buf, "https://")) && git_env_bool("GIT_SMART_HTTP", 1))
    {
        maybe_smart = 1;
        if (!strchr(url.buf, '?'))
...
static void output_refs(struct ref *refs)
{
    ...
    for (posn = refs; posn; posn = posn->next)
    {
        if (posn->symref)
            ...
            printf("%s %s\n", hash_to_hex_algop(posn->old_oid.hash, options.hash_algo),
                   posn->name);
    }
    printf("\n");
    fflush(stdout);
