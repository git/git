    ...
    int                       combined_all_paths)
{
    ...
    struct combine_diff_path **tail = &curr;
    int                        i;
    int                        j;
    int                        cmp;
                               ...
                               long flags)
{
    ...
    while (len1 > 0 && len2 > 0)
    {
        ...
        {
            if ((flags & XDF_IGNORE_WHITESPACE_CHANGE) && (!XDL_ISSPACE(line1[len1]) || !XDL_ISSPACE(line2[len2])))
            {
                return 0;
            }
                                    ...
                                    unsigned long parent, long flags)
{
    ...
    struct lline             *baseend;
    struct lline             *newend = NULL;
    int                       i;
    int                       j;
        ...
        for (j = i + 1; j <= cnt; j++)
        {
            if (!(sline[j].flag & mark))
            {
                ...
                la                   = adjust_hunk_tail(sline, all_mask,
                                                        hunk_begin, j);
                la                   = (la + context < cnt + 1) ? (la + context) : cnt + 1;
                while (la && j <= --la)
                {
                            ...
                            struct rev_info *rev)
{
    ...
    else
    {
        ...
        else if (0 <= (fd = open(elem->path, O_RDONLY | O_CLOEXEC)))
        {
...
static struct combine_diff_path *combined_objfind(struct diff_options      *opt,
                                                  ...
                                                  int                       num_parent)
{
    struct combine_diff_path  *ret  = NULL;
    struct combine_diff_path **tail = &ret;
                        ...
                        struct rev_info        *rev)
{
     ...
     */
    need_generic_pathscan = opt->skip_stat_unmatch || opt->flags.follow_renames || opt->break_opt != -1 || opt->detect_rename || (opt->pickaxe_opts & (DIFF_PICKAXE_KINDS_MASK & ~DIFF_PICKAXE_KIND_OBJFIND)) || opt->filter;
    ...
    while (paths)
    {
        ...
        for (i = 0; i < num_parent; i++)
        {
            if (rev->combined_all_paths && filename_changed(tmp->parent[i].status))
            {
