...
static char *url_normalize_1(const char *url, struct url_info *out_info, char allow_globs)
{
    ...
    if (!spanned || !isalpha(url[0]) || spanned + 3 > url_len || url[spanned] != ':' || url[spanned + 1] != '/' || url[spanned + 2] != '/')
    {
        if (out_info)
        {
            out_info->url = NULL;
    ...
    if (colon_ptr < slash_ptr)
    {
        ...
        else if (slash_ptr - url == 2 && starts_with(norm.buf, "http:") && !strncmp(url, "80", 2))
        {
            /* Skip http :80 as it's the default */
        }
        else if (slash_ptr - url == 3 && starts_with(norm.buf, "https:") && !strncmp(url, "443", 3))
        {
