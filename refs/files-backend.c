...
stat_ref:
    ...
    if (S_ISLNK(st.st_mode))
    {
        ...
        if (starts_with(sb_contents.buf, "refs/") && !check_refname_format(sb_contents.buf, 0))
        {
    ...
    if (S_ISDIR(st.st_mode))
    {
         ...
         * packed ref:
         */
        if (skip_packed_refs || refs_read_raw_ref(refs->packed_ref_store, refname, oid, referent, type, &ignore_errno))
        {
            myerr = EISDIR;
            goto out;
        }
                             ...
                             int *failure_errno)
{
     ...
     */
    if (parse_oid_hex_algop(buf, oid, &p, algop) || (*p != '\0' && !isspace(*p)))
    {
        *type |= REF_ISBROKEN;
...
static int files_ref_iterator_advance(struct ref_iterator *ref_iterator)
{
    ...
    {
        if (iter->flags & DO_FOR_EACH_PER_WORKTREE_ONLY && parse_worktree_ref(iter->iter0->refname, NULL, NULL, NULL) != REF_WORKTREE_CURRENT)
        {
            continue;
        }
        ...
        if (!(iter->flags & DO_FOR_EACH_INCLUDE_BROKEN) && !ref_resolves_to_object(iter->iter0->refname, iter->repo, iter->iter0->oid, iter->iter0->flags))
        {
            continue;
        }
...
{
    struct files_ref_store    *refs;
    struct ref_iterator       *loose_iter;
    struct ref_iterator       *packed_iter;
    struct ref_iterator       *overlay_iter;
    struct files_ref_iterator *iter;
                                           ...
                                           struct strbuf          *err)
{
     ...
     * our refname.
     */
    if (is_null_oid(&lock->old_oid) && refs_verify_refname_available(refs->packed_ref_store, refname, NULL, NULL, err))
    {
        goto error_return;
    }
                           ...
                           struct pack_refs_opts *opts)
{
    ...
    if (parse_worktree_ref(refname, NULL, NULL, NULL) != REF_WORKTREE_SHARED)
    {
        return 0;
    }
    ...
    for_each_string_list_item(item, opts->includes) if (!wildmatch(item->string, refname, 0))
    {
        return 1;
    }
                                    ...
                                    const char *logmsg, int copy)
{
     ...
     * its current value.
     */
    if (!copy && refs_resolve_ref_unsafe(&refs->base, newrefname, RESOLVE_REF_READING | RESOLVE_REF_NO_RECURSE, NULL, NULL) && refs_delete_ref(&refs->base, NULL, newrefname, NULL, REF_NO_DEREF))
    {
        if (errno == EISDIR)
        {
            struct strbuf path = STRBUF_INIT;
    ...
    if (write_ref_to_lockfile(refs, lock, &orig_oid, 0, &err) || commit_ref_update(refs, lock, &orig_oid, logmsg, &err))
    {
...
rollback:
    ...
    if (write_ref_to_lockfile(refs, lock, &orig_oid, 0, &err) || commit_ref_update(refs, lock, &orig_oid, NULL, &err))
    {
...
static int open_or_create_logfile(const char *path, void *cb)
{
    ...
    *fd = open(path, O_APPEND | O_WRONLY | O_CREAT | O_CLOEXEC, 0666);
    return (*fd < 0) ? -1 : 0;
}
                         ...
                         int *logfd, struct strbuf *err)
{
    ...
    {
        *logfd = open(logfile, O_APPEND | O_WRONLY | O_CLOEXEC);
        if (*logfd < 0)
        {
                                 ...
                                 int skip_oid_verification, struct strbuf *err)
{
    ...
    if (write_in_full(fd, oid_to_hex(oid), refs->base.repo->hash_algo->hexsz) < 0 || write_in_full(fd, &term, 1) < 0 || fsync_component(FSYNC_COMPONENT_REFERENCE, get_lock_file_fd(&lock->lk)) < 0 || close_ref_gently(lock) < 0)
    {
        strbuf_addf(err,
...
static char *find_beginning_of_line(const char *bob, char *scan)
{
    while (bob < scan && *(--scan) != '\n')
    {
                             ...
                             struct strbuf          *err)
{
    ...
    if ((update->flags & REF_LOG_ONLY) || (update->flags & REF_SKIP_CREATE_REFLOG) || (update->flags & REF_IS_PRUNING) || (update->flags & REF_UPDATE_VIA_HEAD))
    {
        return 0;
    }

    if (strcmp(update->refname, head_ref) != 0)
    {
        return 0;
    }
     ...
     */
    if (strcmp(new_update->refname, "HEAD") != 0)
    {
                               ...
                               struct strbuf          *err)
{
    ...
    else if ((update->flags & REF_HAVE_NEW) && !(update->flags & REF_DELETING) && !(update->flags & REF_LOG_ONLY))
    {
        if (!(update->type & REF_ISSYMREF) && oideq(&lock->old_oid, &update->new_oid))
        {
            /*
                                      ...
                                      struct ref_transaction *transaction)
{
    ...
    if (backend_data)
    {
        if (backend_data->packed_transaction && ref_transaction_abort(backend_data->packed_transaction, &err))
        {
                                     ...
                                     struct strbuf          *err)
{
    ...
    for (i = 0; i < transaction->nr; i++)
    {
        ...
        if ((update->flags & REF_IS_PRUNING) && !(update->flags & REF_NO_DEREF))
        {
    ...
    for (i = 0; i < transaction->nr; i++)
    {
        ...
        if (update->flags & REF_DELETING && !(update->flags & REF_LOG_ONLY) && !(update->flags & REF_IS_PRUNING))
        {
            /*
                                    ...
                                    struct strbuf          *err)
{
    ...
    for (i = 0; i < transaction->nr; i++)
    {
        ...
        if (update->flags & REF_NEEDS_COMMIT || update->flags & REF_LOG_ONLY)
        {
    ...
    for (i = 0; i < transaction->nr; i++)
    {
        ...
        if (update->flags & REF_DELETING && !(update->flags & REF_LOG_ONLY))
        {
            update->flags |= REF_DELETED_RMDIR;
            if (!(update->type & REF_ISPACKED) || update->type & REF_ISSYMREF)
            {
                               ...
                               void                         *policy_cb_data)
{
    ...
    if (!cb.dry_run)
    {
        ...
        if ((expire_flags & EXPIRE_REFLOGS_UPDATE_REF) && !is_null_oid(&cb.last_kept_oid))
        {
            int         type;
            const char *ref;
        ...
        else if (update && (write_in_full(get_lock_file_fd(&lock->lk), oid_to_hex(&cb.last_kept_oid), refs->base.repo->hash_algo->hexsz) < 0 || write_str_in_full(get_lock_file_fd(&lock->lk), "\n") < 0 || close_ref_gently(lock) < 0))
        {
                               ...
                               files_fsck_refs_fn  *fsck_refs_fn)
{
    ...
    while ((iter_status = dir_iterator_advance(iter)) == ITER_OK)
    {
        ...
        if (S_ISREG(iter->st.st_mode) || S_ISLNK(iter->st.st_mode))
        {
            if (o->verbose)
                      ...
                      struct fsck_options *o)
{
    ...
    return files_fsck_refs(ref_store, o) | refs->packed_ref_store->be->fsck(refs->packed_ref_store, o);
}
