...
static unsigned parse_color_moved_ws(const char *arg)
{
    ...
    if ((ret & COLOR_MOVED_WS_ALLOW_INDENTATION_CHANGE) && (ret & XDF_WHITESPACE_FLAGS))
    {
                                ...
                                struct moved_entry_list *entry_list)
{
    ...
    for (n = 0; n < o->emitted_symbols->nr; n++)
    {
        ...
        {
            if (!adjust_last_block(o, n, block_length) && block_length > 1)
            {
                /*
...
static void dim_moved_lines(struct diff_options *o)
{
    ...
    for (n = 0; n < o->emitted_symbols->nr; n++)
    {
        struct emitted_diff_symbol *prev = (n != 0) ? &o->emitted_symbols->buf[n - 1] : NULL;
        struct emitted_diff_symbol *l    = &o->emitted_symbols->buf[n];
        struct emitted_diff_symbol *next =
            (n < o->emitted_symbols->nr - 1) ? &o->emitted_symbols->buf[n + 1] : NULL;
                                         ...
                                         struct emitted_diff_symbol *eds)
{
    ...
    switch (s)
    {
        ...
        case DIFF_SYMBOL_CONTEXT:
            ...
            break;
        case DIFF_SYMBOL_PLUS:
            switch (flags & (DIFF_SYMBOL_MOVED_LINE | DIFF_SYMBOL_MOVED_LINE_ALT | DIFF_SYMBOL_MOVED_LINE_UNINTERESTING))
            {
                ...
                case DIFF_SYMBOL_MOVED_LINE | DIFF_SYMBOL_MOVED_LINE_ALT:
                    ...
                    break;
                case DIFF_SYMBOL_MOVED_LINE | DIFF_SYMBOL_MOVED_LINE_UNINTERESTING:
...
{
    if ((diff_words->last_minus == 0 && diff_words->current_plus == diff_words->plus.text.ptr) || (diff_words->current_plus > diff_words->plus.text.ptr && *(diff_words->current_plus - 1) == '\n'))
    {
        return 1;
    }
...
static int fn_out_consume(void *priv, char *line, unsigned long len)
{
    ...
    {
        enum diff_symbol s =
            ecbdata->diff_words->type == DIFF_WORDS_PORCELAIN ? DIFF_SYMBOL_WORDS_PORCELAIN : DIFF_SYMBOL_WORDS;
        if (line[0] == '-')
        {
...
static void show_stats(struct diffstat_t *data, struct diff_options *options)
{
    ...
    for (i = 0; i < data->nr; i++)
    {
        ...
        if (file->is_unmerged || (!file->is_interesting && (added + deleted == 0)))
        {
            total_files--;
            continue;
        }
...
static void show_shortstats(struct diffstat_t *data, struct diff_options *options)
{
    ...
    for (i = 0; i < data->nr; i++)
    {
        ...
        if (data->files[i]->is_unmerged || (!data->files[i]->is_interesting && (added + deleted == 0)))
        {
            total_files--;
        }
...
static void show_dirstat(struct diff_options *options)
{
    ...
    for (i = 0; i < q->nr; i++)
    {
        ...
        if (p->one->oid_valid && p->two->oid_valid && oideq(&p->one->oid, &p->two->oid))
        {
            /*
                         ...
                         int                   complete_rewrite)
{
    ...
    if (o->submodule_format == DIFF_SUBMODULE_LOG && (!one->mode || S_ISGITLINK(one->mode)) && (!two->mode || S_ISGITLINK(two->mode)) && (!diff_filepair_is_phoney(one, two)))
    {
    ...
    else if (!o->flags.text && ((!textconv_one && diff_filespec_is_binary(o->repo, one)) || (!textconv_two && diff_filespec_is_binary(o->repo, two))))
    {
        struct strbuf sb = STRBUF_INIT;
        if (!one->data && !two->data && S_ISREG(one->mode) && S_ISREG(two->mode) && !o->flags.binary)
        {
                             ...
                             struct diff_filepair *p)
{
    ...
    may_differ = !(one->oid_valid && two->oid_valid && oideq(&one->oid, &two->oid));
                           ...
                           const struct diff_populate_filespec_options *options)
{
    ...
    if (!s->oid_valid || reuse_worktree_file(r->index, s->path, &s->oid, 0))
    {
         ...
         * is probably fine.
         */
        if (check_binary && s->size > big_file_threshold && s->is_binary == -1)
        {
            s->is_binary = 1;
            return 0;
        }
                                               ...
                                               struct diff_filespec *one)
{
    ...
    if (!S_ISGITLINK(one->mode) && (!one->oid_valid || reuse_worktree_file(r->index, one->path, &one->oid, 1)))
    {
        struct stat st;
...
static void run_diff(struct diff_filepair *p, struct diff_options *o)
{
    ...
    if (!pgm && DIFF_FILE_VALID(one) && DIFF_FILE_VALID(two) && (S_IFMT & one->mode) != (S_IFMT & two->mode))
    {
        /*
...
int diff_check_follow_pathspec(struct pathspec *ps, int die_on_error)
{
    ...
    forbidden_magic = ps->items[0].magic & ~(PATHSPEC_FROMTOP | PATHSPEC_LITERAL);
    if (forbidden_magic)
    {
        if (die_on_error)
        {
...
{
    unsigned check_mask = DIFF_FORMAT_NAME | DIFF_FORMAT_NAME_STATUS | DIFF_FORMAT_CHECKDIFF | DIFF_FORMAT_NO_OUTPUT;
    /*
...
static void diff_flush_patch(struct diff_filepair *p, struct diff_options *o)
{
    int include_conflict_headers =
        (additional_headers(o, p->one->path) && !o->pickaxe_opts && (!o->filter || filter_bit_tst(DIFF_STATUS_UNMERGED, o)));
...
{
    return (((p->status == DIFF_STATUS_MODIFIED) && ((p->score && filter_bit_tst(DIFF_STATUS_FILTER_BROKEN, options)) || (!p->score && filter_bit_tst(DIFF_STATUS_MODIFIED, options)))) || ((p->status != DIFF_STATUS_MODIFIED) && filter_bit_tst(p->status, options)));
}
...
{
    int output_formats_to_prefetch = DIFF_FORMAT_DIFFSTAT | DIFF_FORMAT_NUMSTAT | DIFF_FORMAT_PATCH | DIFF_FORMAT_SHORTSTAT | DIFF_FORMAT_DIRSTAT;
