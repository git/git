...
static int set_scalar_config(const struct scalar_config *config, int reconfigure)
{
    ...
    if ((reconfigure && config->overwrite_on_reconfigure) || git_config_get_string(config->key, &value))
    {
...
{
    return fsmonitor_ipc__is_supported() && fsm_settings__get_reason(the_repository) == FSMONITOR_REASON_OK;
}
...
static char *remote_default_branch(const char *url)
{
    ...
    if (!pipe_command(&cp, NULL, 0, &out, 0, NULL, 0))
    {
        ...
        while (*line)
        {
            ...
            if (!skip_prefix(line, "ref: ", &p) || !strip_suffix_mem(line, &len, "\tHEAD"))
            {
                line = eol + (*eol == '\n');
                continue;
            }
    ...
    warning(_(
        "failed to get default branch name from remote; "
        "using local default"));
    strbuf_reset(&out);
...
static int cmd_clone(int argc, const char **argv)
{
    ...
    if (set_config("remote.origin.url=%s", url) || set_config("remote.origin.fetch="
                                                              "+refs/heads/%s:refs/remotes/origin/%s",
                                                              single_branch ? branch : "*", single_branch ? branch : "*")
        || set_config("remote.origin.promisor=true") || set_config("remote.origin.partialCloneFilter=blob:none"))
    {
...
static int remove_deleted_enlistment(struct strbuf *path)
{
    ...
    if (run_git("config", "--global",
                "--unset", "--fixed-value",
                "maintenance.repo", path->buf, NULL)
        < 0)
    {
        res = -1;
    }
...
static int cmd_run(int argc, const char **argv)
{
    ...
    {
        for (i = 0; tasks[i].arg && strcmp(tasks[i].arg, argv[0]) != 0; i++)
        {
...
static int cmd_unregister(int argc, const char **argv)
{
    ...
    if (argc == 1)
    {
        ...
        struct strbuf workdir_path = STRBUF_INIT;
