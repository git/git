                              ...
                              struct object_array     *reachable)
{
    ...
    for (i = get_max_object_index(); 0 < i; i--)
    {
        ...
        if (o && o->type == OBJ_COMMIT && (o->flags & TMP_MARK))
        {
            add_object_array(o, NULL, reachable);
            o->flags &= ~TMP_MARK;
        }
                         ...
                         const struct object_id *oid)
{
    ...
    if (capabilities)
    {
        ...
        packet_fwrite_fmt(stdout, "%s %s%c%s%s%s%s%s%s%s object-format=%s agent=%s\n",
                          ...
                          0, capabilities,
                          (data->allow_uor & ALLOW_TIP_SHA1) ? " allow-tip-sha1-in-want" : "",
                          (data->allow_uor & ALLOW_REACHABLE_SHA1) ? " allow-reachable-sha1-in-want" : "",
                          ...
                          struct object_array *want_obj)
{
    ...
    if (skip_prefix(line, "want-ref ", &refname_nons))
    {
        ...
        if (ref_is_hidden(refname_nons, refname.buf, hidden_refs) || refs_read_ref(get_main_ref_store(the_repository), refname.buf, &oid))
        {
                         ...
                         struct upload_pack_data *data)
{
    while (packet_reader_read(request) == PACKET_READ_NORMAL)
    {
        ...
        if (data->allow_sideband_all && !strcmp(arg, "sideband-all"))
        {
