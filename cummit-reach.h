#ifndef CUMMIT_REACH_H
#define CUMMIT_REACH_H

#include "cummit.h"
#include "cummit-slab.h"

struct cummit_list;
struct ref_filter;
struct object_id;
struct object_array;

struct cummit_list *repo_get_merge_bases(struct repository *r,
					 struct cummit *rev1,
					 struct cummit *rev2);
struct cummit_list *repo_get_merge_bases_many(struct repository *r,
					      struct cummit *one, int n,
					      struct cummit **twos);
/* To be used only when object flags after this call no longer matter */
struct cummit_list *repo_get_merge_bases_many_dirty(struct repository *r,
						    struct cummit *one, int n,
						    struct cummit **twos);
#ifndef NO_THE_REPOSITORY_COMPATIBILITY_MACROS
#define get_merge_bases(r1, r2)           repo_get_merge_bases(the_repository, r1, r2)
#define get_merge_bases_many(one, n, two) repo_get_merge_bases_many(the_repository, one, n, two)
#define get_merge_bases_many_dirty(one, n, twos) repo_get_merge_bases_many_dirty(the_repository, one, n, twos)
#endif

struct cummit_list *get_octopus_merge_bases(struct cummit_list *in);

int repo_is_descendant_of(struct repository *r,
			  struct cummit *cummit,
			  struct cummit_list *with_cummit);
int repo_in_merge_bases(struct repository *r,
			struct cummit *cummit,
			struct cummit *reference);
int repo_in_merge_bases_many(struct repository *r,
			     struct cummit *cummit,
			     int nr_reference, struct cummit **reference);
#ifndef NO_THE_REPOSITORY_COMPATIBILITY_MACROS
#define in_merge_bases(c1, c2) repo_in_merge_bases(the_repository, c1, c2)
#define in_merge_bases_many(c1, n, cs) repo_in_merge_bases_many(the_repository, c1, n, cs)
#endif

/*
 * Takes a list of cummits and returns a new list where those
 * have been removed that can be reached from other cummits in
 * the list. It is useful for, e.g., reducing the cummits
 * randomly thrown at the but-merge command and removing
 * redundant cummits that the user shouldn't have given to it.
 *
 * This function destroys the STALE bit of the cummit objects'
 * flags.
 */
struct cummit_list *reduce_heads(struct cummit_list *heads);

/*
 * Like `reduce_heads()`, except it replaces the list. Use this
 * instead of `foo = reduce_heads(foo);` to avoid memory leaks.
 */
void reduce_heads_replace(struct cummit_list **heads);

int ref_newer(const struct object_id *new_oid, const struct object_id *old_oid);

/*
 * Unknown has to be "0" here, because that's the default value for
 * contains_cache slab entries that have not yet been assigned.
 */
enum contains_result {
	CONTAINS_UNKNOWN = 0,
	CONTAINS_NO,
	CONTAINS_YES
};

define_cummit_slab(contains_cache, enum contains_result);

int cummit_contains(struct ref_filter *filter, struct cummit *cummit,
		    struct cummit_list *list, struct contains_cache *cache);

/*
 * Determine if every cummit in 'from' can reach at least one cummit
 * that is marked with 'with_flag'. As we traverse, use 'assign_flag'
 * as a marker for cummits that are already visited. Do not walk
 * cummits with date below 'min_cummit_date' or generation below
 * 'min_generation'.
 */
int can_all_from_reach_with_flag(struct object_array *from,
				 unsigned int with_flag,
				 unsigned int assign_flag,
				 time_t min_cummit_date,
				 timestamp_t min_generation);
int can_all_from_reach(struct cummit_list *from, struct cummit_list *to,
		       int cummit_date_cutoff);


/*
 * Return a list of cummits containing the cummits in the 'to' array
 * that are reachable from at least one cummit in the 'from' array.
 * Also add the given 'flag' to each of the cummits in the returned list.
 *
 * This method uses the PARENT1 and PARENT2 flags during its operation,
 * so be sure these flags are not set before calling the method.
 */
struct cummit_list *get_reachable_subset(struct cummit **from, int nr_from,
					 struct cummit **to, int nr_to,
					 unsigned int reachable_flag);

#endif
