...
test_expect_success 'setup repo with a git repo inside it' '
	...
	(
		...
		echo a > a &&
		svn_cmd add .git a &&
		svn_cmd cummit -m "create a nested git repo" &&
		svn_cmd up &&
		echo hi >> .git/a &&
		svn_cmd cummit -m "modify .git/a" &&
		svn_cmd up
	)
'
...
test_expect_success 'SVN-side change outside of .git' '
	(
		cd s &&
		echo b >> a &&
		svn_cmd cummit -m "SVN-side change outside of .git" &&
		svn_cmd up &&
...
test_expect_success 'SVN-side change inside of .git' '
	(
		cd s &&
		git add a &&
		git cummit -m "add a inside an SVN repo" &&
		git log &&
		svn_cmd add --force .git &&
		svn_cmd cummit -m "SVN-side change inside of .git" &&
		svn_cmd up &&
...
test_expect_success 'SVN-side change in and out of .git' '
	(
		...
		git add a &&
		git cummit -m "add a inside an SVN repo" &&
		svn_cmd cummit -m "SVN-side change in and out of .git" &&
		svn_cmd up &&
