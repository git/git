...
# but will never intentionally create packfiles with
...
test_expect_success 'lookup in duplicated pack' '
	but cat-file --batch-check <input >actual &&
...
test_expect_success 'index-pack can reject packs with duplicates' '
	...
	test_must_fail but index-pack --strict --stdin <dups.pack &&
	test_expect_code 1 but cat-file -e $LO_SHA1
'
