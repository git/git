...
test_expect_success 'setup helper scripts' '
	cat >dump <<-\EOF &&
	whoami=$(echo $0 | sed s/.*but-credential-//)
	echo >&2 "$whoami: $*"
	OIFS=$IFS
	IFS==
	...
	write_script but-credential-useless <<-\EOF &&
	. ./dump
	exit 0
	EOF

	write_script but-credential-quit <<-\EOF &&
	. ./dump
	echo quit=1
	EOF

	write_script but-credential-verbatim <<-\EOF &&
	user=$1; shift
	pass=$1; shift
	. ./dump
...
test_expect_success 'but-credential respects core.askPass' '
...
test_expect_success 'match multiple configured helpers with URLs' '
	test_config credential.https://example.com/repo.but.helper "verbatim \"\" \"\"" &&
	test_config credential.https://example.com.helper "$HELPER" &&
	check fill <<-\EOF
	protocol=https
	host=example.com
...
test_expect_success 'match percent-encoded values' '
	test_config credential.https://example.com/%2566.but.helper "$HELPER" &&
	check fill <<-\EOF
	url=https://example.com/%2566.but
	--
	protocol=https
	host=example.com
	username=foo
...
test_expect_success 'match percent-encoded UTF-8 values in path' '
	...
	test_config credential.https://example.com/perú.but.helper "$HELPER" &&
	check fill <<-\EOF
	url=https://example.com/per%C3%BA.but
	--
	protocol=https
	host=example.com
	path=perú.but
...
test_expect_success 'match percent-encoded values in username' '
	test_config credential.https://user%2fname@example.com/foo/bar.but.helper "$HELPER" &&
	check fill <<-\EOF
	url=https://user%2fname@example.com/foo/bar.but
	--
	protocol=https
	host=example.com
	username=foo
...
test_expect_success 'fetch with multiple path components' '
	...
	test_config credential.https://example.com/foo/repo.but.helper "verbatim foo bar" &&
	check fill <<-\EOF
	url=https://example.com/foo/repo.but
	--
	protocol=https
	host=example.com
	username=foo
...
test_expect_success 'last matching username wins' '
	test_config credential.https://example.com/path.but.username bob &&
	test_config credential.https://example.com.username alice &&
	test_config credential.https://example.com.helper "verbatim \"\" bar" &&
	check fill <<-\EOF
	url=https://example.com/path.but
	--
	protocol=https
	host=example.com
	username=alice
...
test_expect_success 'url parser rejects embedded newlines' '
	test_must_fail but credential fill 2>stderr <<-\EOF &&
...
test_expect_success 'url parser handles bare query marker' '
	check_host_and_path https://example.com?foo.but example.com ?foo.but
'
...
test_expect_success 'url parser not confused by encoded markers' '
	check_host_and_path https://example.com%23%3f%2f/foo.but \
		"example.com#?/" foo.but
'

test_expect_success 'credential config with partial URLs' '
	echo "echo password=yep" | write_script but-credential-yep &&
	test_write_lines url=https://user@example.com/repo.but >stdin &&
	for partial in \
		example.com \
		user@example.com \
		https:// \
		...
		https://user@example.com/ \
		https://example.com/repo.but \
		https://user@example.com/repo.but \
		/repo.but
	do
		but -c credential.$partial.helper=yep \
