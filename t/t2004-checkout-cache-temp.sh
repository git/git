...
test_description='but checkout-index --temp test.

With --temp flag, but checkout-index writes to temporary merge files
...
test_expect_success 'setup' '
	...
	but update-index --add path0 path1 path3 path4 asubdir/path5 &&
	t1=$(but write-tree) &&
	rm -f path* .merge_* actual .but/index &&
	echo tree2path0 >path0 &&
	echo tree2path1 >path1 &&
	echo tree2path2 >path2 &&
	echo tree2path4 >path4 &&
	but update-index --add path0 path1 path2 path4 &&
	t2=$(but write-tree) &&
	rm -f path* .merge_* actual .but/index &&
	echo tree2path0 >path0 &&
	...
	but update-index --add path0 path1 path2 path3 &&
	t3=$(but write-tree)
'
...
test_expect_success 'checkout one stage 0 to temporary file' '
	rm -f path* .merge_* actual .but/index &&
	but read-tree $t1 &&
	but checkout-index --temp -- path1 >actual &&
...
test_expect_success 'checkout all stage 0 to temporary files' '
	rm -f path* .merge_* actual .but/index &&
	but read-tree $t1 &&
	but checkout-index -a --temp >actual &&
...
test_expect_success 'setup 3-way merge' '
	rm -f path* .merge_* actual .but/index &&
	but read-tree -m $t1 $t2 $t3
'

test_expect_success 'checkout one stage 2 to temporary file' '
	rm -f path* .merge_* actual &&
	but checkout-index --stage=2 --temp -- path1 >actual &&
	test_line_count = 1 actual &&
	test $(cut "-d	" -f2 actual) = path1 &&
	p=$(cut "-d	" -f1 actual) &&
	test -f $p &&
...
test_expect_success 'checkout all stage 2 to temporary files' '
	rm -f path* .merge_* actual &&
	but checkout-index --all --stage=2 --temp >actual &&
...
test_expect_success 'checkout all stages of unknown path' '
	rm -f path* .merge_* actual &&
	test_must_fail but checkout-index --stage=all --temp \
...
test_expect_success 'checkout all stages/one file to nothing' '
	rm -f path* .merge_* actual &&
	but checkout-index --stage=all --temp -- path0 >actual 2>stderr &&
...
test_expect_success 'checkout all stages/one file to temporary files' '
	rm -f path* .merge_* actual &&
	but checkout-index --stage=all --temp -- path1 >actual &&
	test_line_count = 1 actual &&
	test $(cut "-d	" -f2 actual) = path1 &&
...
test_expect_success 'checkout some stages/one file to temporary files' '
	rm -f path* .merge_* actual &&
	but checkout-index --stage=all --temp -- path2 >actual &&
	test_line_count = 1 actual &&
...
test_expect_success 'checkout all stages/all files to temporary files' '
	rm -f path* .merge_* actual &&
	but checkout-index -a --stage=all --temp >actual &&
	test_line_count = 5 actual
'
...
test_expect_success 'checkout --temp within subdir' '
	(
		cd asubdir &&
		but checkout-index -a --stage=all >actual &&
		test_line_count = 1 actual &&
...
test_expect_success 'checkout --temp symlink' '
	...
	but checkout-index --temp -a >actual &&
	test_line_count = 1 actual &&
...
test_expect_success 'emit well-formed relative path' '
	rm -f path* .merge_* actual .but/index &&
	>path0123456789 &&
	but update-index --add path0123456789 &&
	(
		cd asubdir &&
		but checkout-index --temp -- ../path0123456789 >actual &&
		test_line_count = 1 actual &&
