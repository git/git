...
test_description='but apply with new style GNU diff with empty context
...
test_expect_success 'apply --numstat' '
	...
	but apply --numstat diff.output >actual &&
	{
		echo "0	1	file1" &&
		echo "0	1	file2"
	} >expect &&
...
test_expect_success 'apply --apply' '
	...
	but update-index file1 file2 &&
	but apply --index diff.output &&
