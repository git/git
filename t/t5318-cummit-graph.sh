...
test_expect_success 'usage' '
	...
	test_expect_code 129 but cummit-graph write verify
'

test_expect_success 'usage shown without sub-command' '
	test_expect_code 129 but cummit-graph 2>err &&
	! grep error: err
'
...
test_expect_success 'usage shown with an error on unknown sub-command' '
	...
	EOF
	test_expect_code 129 but cummit-graph unknown 2>stderr &&
...
test_expect_success 'write graph with no packs' '
	cd "$TRASH_DIRECTORY/full" &&
	but cummit-graph write --object-dir $objdir &&
...
test_expect_success 'exit with correct error on bad input to --stdin-packs' '
	...
	test_expect_code 1 but cummit-graph write --stdin-packs <in 2>stderr &&
...
graph_but_behavior 'no graph' full cummits/3 cummits/1

test_expect_success 'exit with correct error on bad input to --stdin-cummits' '
	cd "$TRASH_DIRECTORY/full" &&
	# invalid, non-hex OID
	echo HEAD >in &&
	test_expect_code 1 but cummit-graph write --stdin-cummits <in 2>stderr &&
	...
	but rev-parse HEAD HEAD^{tree} >in &&
	but cummit-graph write --stdin-cummits <in &&
...
graph_but_behavior 'graph exists' full cummits/3 cummits/1
...
test_expect_success 'cummit-graph write with the --no-progress option' '
	cd "$TRASH_DIRECTORY/full" &&
	but cummit-graph write --no-progress 2>err &&
	test_must_be_empty err
'
...
test_expect_success 'cummit-graph write --stdin-cummits progress off for redirected stderr' '
	...
	but cummit-graph write --stdin-cummits <in 2>err &&
	test_must_be_empty err
'
...
test_expect_success 'cummit-graph write --stdin-cummits with the --no-progress option' '
	cd "$TRASH_DIRECTORY/full" &&
	but rev-parse cummits/5 >in &&
	but cummit-graph write --stdin-cummits --no-progress <in 2>err &&
	test_must_be_empty err
'

test_expect_success 'cummit-graph verify progress off for redirected stderr' '
	cd "$TRASH_DIRECTORY/full" &&
	but cummit-graph verify 2>err &&
	test_must_be_empty err
'
...
test_expect_success 'cummit-graph verify with the --no-progress option' '
	cd "$TRASH_DIRECTORY/full" &&
	but cummit-graph verify --no-progress 2>err &&
	test_must_be_empty err
'
...
graph_but_behavior 'merge 1 vs 2' full merge/1 merge/2
graph_but_behavior 'merge 1 vs 3' full merge/1 merge/3
graph_but_behavior 'merge 2 vs 3' full merge/2 merge/3

test_expect_success 'Add one more cummit' '
	cd "$TRASH_DIRECTORY/full" &&
	test_cummit 8 &&
	but branch cummits/8 &&
...
graph_but_behavior 'mixed mode, cummit 8 vs merge 1' full cummits/8 merge/1
graph_but_behavior 'mixed mode, cummit 8 vs merge 2' full cummits/8 merge/2
...
graph_but_behavior 'full graph, cummit 8 vs merge 1' full cummits/8 merge/1
graph_but_behavior 'full graph, cummit 8 vs merge 2' full cummits/8 merge/2
...
graph_but_behavior 'cleared graph, cummit 8 vs merge 1' full cummits/8 merge/1
graph_but_behavior 'cleared graph, cummit 8 vs merge 2' full cummits/8 merge/2

test_expect_success 'build graph from latest pack with closure' '
	cd "$TRASH_DIRECTORY/full" &&
	cat new-idx | but cummit-graph write --stdin-packs &&
	test_path_is_file $objdir/info/cummit-graph &&
	graph_read_expect "9" "generation_data extra_edges"
'

graph_but_behavior 'graph from pack, cummit 8 vs merge 1' full cummits/8 merge/1
graph_but_behavior 'graph from pack, cummit 8 vs merge 2' full cummits/8 merge/2

test_expect_success 'build graph from cummits with closure' '
	cd "$TRASH_DIRECTORY/full" &&
	but tag -a -m "merge" tag/merge merge/2 &&
	but rev-parse tag/merge >cummits-in &&
	but rev-parse merge/1 >>cummits-in &&
	cat cummits-in | but cummit-graph write --stdin-cummits &&
	test_path_is_file $objdir/info/cummit-graph &&
	graph_read_expect "6" "generation_data"
'

graph_but_behavior 'graph from cummits, cummit 8 vs merge 1' full cummits/8 merge/1
graph_but_behavior 'graph from cummits, cummit 8 vs merge 2' full cummits/8 merge/2

test_expect_success 'build graph from cummits with append' '
	cd "$TRASH_DIRECTORY/full" &&
	but rev-parse merge/3 | but cummit-graph write --stdin-cummits --append &&
	test_path_is_file $objdir/info/cummit-graph &&
	graph_read_expect "10" "generation_data extra_edges"
'

graph_but_behavior 'append graph, cummit 8 vs merge 1' full cummits/8 merge/1
graph_but_behavior 'append graph, cummit 8 vs merge 2' full cummits/8 merge/2
...
test_expect_success 'setup bare repo' '
	cd "$TRASH_DIRECTORY" &&
	but clone --bare --no-local full bare &&
	cd bare &&
	but config core.cummitGraph true &&
	baredir="./objects"
'

graph_but_behavior 'bare repo, cummit 8 vs merge 1' bare cummits/8 merge/1
graph_but_behavior 'bare repo, cummit 8 vs merge 2' bare cummits/8 merge/2
...
graph_but_behavior 'bare repo with graph, cummit 8 vs merge 1' bare cummits/8 merge/1
graph_but_behavior 'bare repo with graph, cummit 8 vs merge 2' bare cummits/8 merge/2

test_expect_success 'perform fast-forward merge in full repo' '
	cd "$TRASH_DIRECTORY/full" &&
	but checkout -b merge-5-to-8 cummits/5 &&
	but merge cummits/8 &&
	but show-ref -s merge-5-to-8 >output &&
	but show-ref -s cummits/8 >expect &&
	test_cmp expect output
'

test_expect_success 'check that gc computes cummit-graph' '
	cd "$TRASH_DIRECTORY/full" &&
	but cummit --allow-empty -m "blank" &&
	but cummit-graph write --reachable &&
	cp $objdir/info/cummit-graph cummit-graph-before-gc &&
	but reset --hard HEAD~1 &&
	but config gc.writecummitGraph true &&
	but gc &&
...
test_expect_success 'replace-objects invalidates cummit-graph' '
	...
	but clone full replace &&
	(
		cd replace &&
		but cummit-graph write --reachable &&
		test_path_is_file .but/objects/info/cummit-graph &&
		but replace HEAD~1 HEAD~2 &&
		graph_but_two_modes "cummit-graph verify" &&
		but -c core.cummitGraph=false log >expect &&
		but -c core.cummitGraph=true log >actual &&
		test_cmp expect actual &&
		but cummit-graph write --reachable &&
		but -c core.cummitGraph=false --no-replace-objects log >expect &&
		but -c core.cummitGraph=true --no-replace-objects log >actual &&
		test_cmp expect actual &&
...
test_expect_success 'cummit grafts invalidate cummit-graph' '
	...
	but clone full graft &&
	(
		cd graft &&
		but cummit-graph write --reachable &&
		test_path_is_file .but/objects/info/cummit-graph &&
		H1=$(but rev-parse --verify HEAD~1) &&
		H3=$(but rev-parse --verify HEAD~3) &&
		echo "$H1 $H3" >.but/info/grafts &&
		but -c core.cummitGraph=false log >expect &&
		but -c core.cummitGraph=true log >actual &&
		test_cmp expect actual &&
		but cummit-graph write --reachable &&
		...
		rm -rf .but/objects/info/cummit-graph &&
		but cummit-graph write --reachable &&
		test_path_is_missing .but/objects/info/cummit-graph
	)
'
...
test_expect_success 'replace-objects invalidates cummit-graph' '
	...
	but clone --depth 2 "file://$TRASH_DIRECTORY/full" shallow &&
	(
		cd shallow &&
		but cummit-graph write --reachable &&
		test_path_is_missing .but/objects/info/cummit-graph &&
		but fetch origin --unshallow &&
		but cummit-graph write --reachable &&
		test_path_is_file .but/objects/info/cummit-graph
	)
'
...
test_expect_success 'warn on improper hash version' '
	...
	(
		...
		test_cummit 1 &&
		but cummit-graph write --reachable &&
		mv .but/objects/info/cummit-graph ../cg-sha1
	) &&
	...
	(
		...
		test_cummit 1 &&
		but cummit-graph write --reachable &&
		mv .but/objects/info/cummit-graph ../cg-sha256
	) &&
	(
		cd sha1 &&
		mv ../cg-sha256 .but/objects/info/cummit-graph &&
		but log -1 2>err &&
		test_i18ngrep "cummit-graph hash version 2 does not match version 1" err
	) &&
	(
		cd sha256 &&
		mv ../cg-sha1 .but/objects/info/cummit-graph &&
		but log -1 2>err &&
...
test_expect_success 'but cummit-graph verify' '
	cd "$TRASH_DIRECTORY/full" &&
	but rev-parse cummits/8 | but -c cummitGraph.generationVersion=1 cummit-graph write --stdin-cummits &&
	but cummit-graph verify >output &&
...
# starting at <zero_pos>, then runs 'but cummit-graph verify'
...
test_expect_success 'but fsck (checks cummit-graph when config set to true)' '
	cd "$TRASH_DIRECTORY/full" &&
	but fsck &&
	corrupt_graph_and_verify $GRAPH_BYTE_FOOTER "\00" \
		"incorrect checksum" &&
	cp cummit-graph-pre-write-test $objdir/info/cummit-graph &&
	test_must_fail but -c core.cummitGraph=true fsck
'

test_expect_success 'but fsck (ignores cummit-graph when config set to false)' '
	cd "$TRASH_DIRECTORY/full" &&
	but fsck &&
	corrupt_graph_and_verify $GRAPH_BYTE_FOOTER "\00" \
		"incorrect checksum" &&
	cp cummit-graph-pre-write-test $objdir/info/cummit-graph &&
	but -c core.cummitGraph=false fsck
'

test_expect_success 'but fsck (checks cummit-graph when config unset)' '
	cd "$TRASH_DIRECTORY/full" &&
	test_when_finished "but config core.cummitGraph true" &&
...
test_expect_success 'setup non-the_repository tests' '
	...
	but -C repo config core.cummitGraph true &&
	but -C repo rev-parse two | \
		but -C repo cummit-graph write --stdin-cummits
'

test_expect_success 'parse_cummit_in_graph works for non-the_repository' '
	test-tool repository parse_cummit_in_graph \
		repo/.but repo "$(but -C repo rev-parse two)" >actual &&
	{
		but -C repo log --pretty=format:"%ct " -1 &&
		but -C repo rev-parse one
	} >expect &&
	test_cmp expect actual &&

	test-tool repository parse_cummit_in_graph \
		repo/.but repo "$(but -C repo rev-parse one)" >actual &&
	but -C repo log --pretty="%ct" -1 one >expect &&
	test_cmp expect actual
'
...
test_expect_success 'get_cummit_tree_in_graph works for non-the_repository' '
	test-tool repository get_cummit_tree_in_graph \
		repo/.but repo "$(but -C repo rev-parse two)" >actual &&
	but -C repo rev-parse two^{tree} >expect &&
	test_cmp expect actual &&

	test-tool repository get_cummit_tree_in_graph \
		repo/.but repo "$(but -C repo rev-parse one)" >actual &&
	but -C repo rev-parse one^{tree} >expect &&
	test_cmp expect actual
'
...
test_expect_success 'corrupt cummit-graph write (broken parent)' '
	...
	(
		...
		broken cummit
		EOF
		broken="$(but hash-object -w -t cummit --literally broken)" &&
		but cummit-tree -p "$broken" -m "good cummit" "$empty" >good &&
		test_must_fail but cummit-graph write --stdin-cummits \
			<good 2>test_err &&
...
test_expect_success 'corrupt cummit-graph write (missing tree)' '
	...
	(
		...
		EOF
		broken="$(but hash-object -w -t cummit --literally broken)" &&
		but cummit-tree -p "$broken" -m "good" "$tree" >good &&
		test_must_fail but cummit-graph write --stdin-cummits \
			<good 2>test_err &&
		test_i18ngrep "unable to parse cummit" test_err
	)
...
graph_but_behavior 'generation data overflow chunk repo' repo left right
