...
rules, found in <Documentation/but read-tree.txt>.
...
read_tree_twoway () {
    but read-tree -m "$1" "$2" && but ls-files --stage
}
...
test_expect_success '4 - carry forward local addition.' '
	...
	but checkout-index -u -f -q -a &&
	but update-index --add yomin &&
	read_tree_twoway $treeH $treeM &&
	but ls-files --stage >4.out &&
	test_must_fail but diff --no-index M.out 4.out >4diff.out &&
...
test_expect_success '5 - carry forward local addition.' '
	...
	test_must_fail but diff --no-index M.out 5.out >5diff.out &&
...
test_expect_success '14 - unchanged in two heads.' '
	...
	test_must_fail but diff --no-index M.out 14.out >14diff.out &&
...
test_expect_success '15 - unchanged in two heads.' '
	...
	test_must_fail but diff --no-index M.out 15.out >15diff.out &&
...
test_expect_success 'a/b (untracked) vs a case setup.' '
	...
	echo treeM $treeM &&
	but ls-tree $treeM &&
	but ls-files --stage >treeM.out &&

	rm -f a &&
	but update-index --remove a &&
	mkdir a &&
	: >a/b &&
	treeH=$(but write-tree) &&
	echo treeH $treeH &&
...
test_expect_success 'read-tree supports the super-prefix' '
	...
	EOF
	test_must_fail but --super-prefix fictional/ read-tree -u -m "$treeH" "$treeM" 2>actual &&
...
test_expect_success 'a/b vs a, plus c/d case setup.' '
	...
	mkdir a &&
	: >a/b &&
	but update-index --add --remove a a/b &&
	treeH=$(but write-tree) &&
	echo treeH $treeH &&
	but ls-tree $treeH
'
...
test_expect_success 'a/b vs a, plus c/d case test.' '
	...
	but ls-files --stage | tee >treeMcheck.out &&
...
test_expect_success '-m references the correct modified tree' '
	echo >file-a &&
	echo >file-b &&
	but add file-a file-b &&
	but cummit -a -m "test for correct modified tree" &&
	but branch initial-mod &&
	echo b >file-b &&
	but cummit -a -m "B" &&
	echo a >file-a &&
	but add file-a &&
	but ls-tree $(but write-tree) file-a >expect &&
	read_tree_must_succeed -m HEAD initial-mod &&
	but ls-tree $(but write-tree) file-a >actual &&
	test_cmp expect actual
'
