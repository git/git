...
test_expect_success 'get bloom filters for cummit with no changes' '
	but init &&
	but cummit --allow-empty -m "c0" &&
	cat >expect <<-\EOF &&
	Filter_Length:1
	Filter_Data:00|
	EOF
	test-tool bloom get_filter_for_cummit "$(but rev-parse HEAD)" >actual &&
	test_cmp expect actual
'

test_expect_success 'get bloom filter for cummit with 10 changes' '
	rm actual &&
	rm expect &&
	mkdir smallDir &&
	...
	but add smallDir &&
	but cummit -m "cummit with 10 changes" &&
	cat >expect <<-\EOF &&
	Filter_Length:14
...
test_expect_success EXPENSIVE 'get bloom filter for cummit with 513 changes' '
	rm actual &&
	rm expect &&
	mkdir bigDir &&
	...
	but add bigDir &&
	but cummit -m "cummit with 513 changes" &&
	cat >expect <<-\EOF &&
	Filter_Length:1
	Filter_Data:ff|
	EOF
