...
test_description='but svn multiple branch and tag paths in the svn repo'
. ./lib-but-svn.sh
...
test_expect_success 'clone multiple branch and tag paths' '
	but svn clone -T trunk \
		      -b b_one/* --branches b_two/* \
		      -t tags_A/* --tags tags_B \
		      "$svnrepo/project" but_project &&
	( cd but_project &&
		but rev-parse refs/remotes/origin/first &&
		but rev-parse refs/remotes/origin/second &&
		but rev-parse refs/remotes/origin/1 &&
		but rev-parse refs/remotes/origin/2 &&
		but rev-parse refs/remotes/origin/tags/1.0 &&
		but rev-parse refs/remotes/origin/tags/2.0 &&
		but rev-parse refs/remotes/origin/tags/3.0 &&
		but rev-parse refs/remotes/origin/tags/4.0 &&
		but rev-parse refs/remotes/origin/tags/v5 &&
		but rev-parse refs/remotes/origin/tags/v6 &&
		but rev-parse refs/remotes/origin/tags/v7 &&
		but rev-parse refs/remotes/origin/tags/v8
	)
'

test_expect_success 'Multiple branch or tag paths require -d' '
	( cd but_project &&
		test_must_fail but svn branch -m "No new branch" Nope &&
		test_must_fail but svn tag -m "No new tag" Tagless &&
		test_must_fail but rev-parse refs/remotes/origin/Nope &&
		test_must_fail but rev-parse refs/remotes/origin/tags/Tagless
	) &&
	( cd svn_project &&
		svn_cmd up &&
...
test_expect_success 'create new branches and tags' '
	( cd but_project &&
		but svn branch -m "New branch 1" -d b_one New1 ) &&
	( cd svn_project &&
		svn_cmd up && test -e b_one/New1/a.file ) &&

	( cd but_project &&
		but svn branch -m "New branch 2" -d b_two New2 ) &&
	( cd svn_project &&
		svn_cmd up && test -e b_two/New2/a.file ) &&

	( cd but_project &&
		but svn branch -t -m "New tag 1" -d tags_A Tag1 ) &&
	( cd svn_project &&
		svn_cmd up && test -e tags_A/Tag1/a.file ) &&

	( cd but_project &&
		but svn tag -m "New tag 2" -d tags_B Tag2 ) &&
	( cd svn_project &&
