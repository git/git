...
test_expect_success 'setup' '
	...
	blob_file1=$(but hash-object file1) &&
	blob_file2=$(but hash-object subdir/file2) &&
	but add . &&
	but cummit -m one
'
...
check_diff () {
	...
	expect=$1
	shift
	short_blob=$(but rev-parse --short $blob_file2)
	cat >expected <<-EOF
	diff --but a/$expect b/$expect
...
check_numstat () {
	...
	test_expect_success "--numstat $*" "
		...
		but -C '$dir' diff --numstat $* HEAD^ >actual &&
		test_cmp expected actual
	"
}
...
check_stat () {
	...
	test_expect_success "--stat $*" "
		but -C '$dir' diff --stat $* HEAD^ >actual &&
		test_cmp expected actual
	"
}
...
check_raw () {
	...
	test_expect_success "--raw $*" "
		but -C '$dir' diff --no-abbrev --raw $* HEAD^ >actual &&
		test_cmp expected actual
	"
}
...
check_diff_no_relative_option () {
	...
	shift
	test_expect_success "config diff.relative $relative_opt -p $*" "
		short_blob_file1=\$(but rev-parse --short $blob_file1) &&
		short_blob_file2=\$(but rev-parse --short $blob_file2) &&
		cat >expected <<-EOF &&
...
test_expect_success 'diff --relative without change in subdir' '
	but switch br2 &&
	test_when_finished "but merge --abort" &&
	test_must_fail but merge one &&
	but -C subdir diff --relative >out &&
	test_must_be_empty out &&
	but -C subdir diff --relative --name-only >out &&
	test_must_be_empty out
'
...
test_expect_failure 'diff --relative with change in subdir' '
	but switch br3 &&
	br1_blob=$(but rev-parse --short --verify br1:subdir/file0) &&
	br3_blob=$(but rev-parse --short --verify br3:subdir/file0) &&
	test_when_finished "but merge --abort" &&
	test_must_fail but merge br1 &&
	cat >expected <<-EOF &&
	diff --cc file0
