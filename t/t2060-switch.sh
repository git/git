...
test_expect_success 'setup' '
	test_cummit first &&
	but branch first-branch &&
	test_cummit second &&
	test_cummit third &&
	but remote add origin nohost:/nopath &&
	but update-ref refs/remotes/origin/foo first-branch
'

test_expect_success 'switch branch no arguments' '
	test_must_fail but switch
'

test_expect_success 'switch branch' '
	but switch first-branch &&
	test_path_is_missing second.t
'

test_expect_success 'switch and detach' '
	test_when_finished but switch main &&
	test_must_fail but switch main^{cummit} &&
	but switch --detach main^{cummit} &&
	test_must_fail but symbolic-ref HEAD
'

test_expect_success 'suggestion to detach' '
	test_must_fail but switch main^{cummit} 2>stderr &&
...
test_expect_success 'switch and create branch' '
	test_when_finished but switch main &&
	but switch -c temp main^ &&
	test_cmp_rev main^ refs/heads/temp &&
	echo refs/heads/temp >expected-branch &&
	but symbolic-ref HEAD >actual-branch &&
	test_cmp expected-branch actual-branch
'

test_expect_success 'force create branch from HEAD' '
	test_when_finished but switch main &&
	but switch --detach main &&
	test_must_fail but switch -c temp &&
	but switch -C temp &&
	test_cmp_rev main refs/heads/temp &&
	echo refs/heads/temp >expected-branch &&
	but symbolic-ref HEAD >actual-branch &&
...
test_expect_success 'new orphan branch from empty' '
	test_when_finished but switch main &&
	test_must_fail but switch --orphan new-orphan HEAD &&
	but switch --orphan new-orphan &&
	test_cummit orphan &&
	but cat-file cummit refs/heads/new-orphan >cummit &&
	! grep ^parent cummit &&
...
test_expect_success 'orphan branch works with --discard-changes' '
	test_when_finished but switch main &&
	echo foo >foo.txt &&
	but switch --discard-changes --orphan new-orphan2 &&
	but ls-files >tracked-files &&
...
test_expect_success 'guess and create branch' '
	test_when_finished but switch main &&
	test_must_fail but switch --no-guess foo &&
	test_config checkout.guess false &&
	test_must_fail but switch foo &&
...
test_expect_success 'not switching when something is in progress' '
	test_when_finished rm -f .but/MERGE_HEAD &&
	# fake a merge-in-progress
	cp .but/HEAD .but/MERGE_HEAD &&
	test_must_fail but switch -d @^
'

test_expect_success 'tracking info copied with autoSetupMerge=inherit' '
	# default config does not copy tracking info
	but switch -c foo-no-inherit foo &&
	...
	but switch --track=inherit -c foo2 foo &&
