...
test_description='but svn fetch deleted tag 2'
...
test_expect_success 'fetch deleted tags from same revision with no checksum error' '
	...
	but diff --exit-code main:subdir3/file origin/tags/mytag:file &&
	but diff --exit-code main:subdir2/file origin/tags/mytag^:file &&
	but diff --exit-code main:subdir1/file origin/tags/mytag^^:file
'
