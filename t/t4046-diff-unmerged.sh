...
test_expect_success setup '
	for i in 0 1 2 3
	do
		blob=$(echo $i | but hash-object --stdin) &&
		eval "blob$i=$blob" &&
	...
	but update-index --index-info <ls-files-s.expect &&
	but ls-files -s >ls-files-s.actual &&
...
test_expect_success 'diff-files -0' '
	...
	but diff-files -0 >diff-files-0.actual &&
...
test_expect_success 'diff-files -1' '
	...
	but diff-files -1 >diff-files-1.actual &&
...
test_expect_success 'diff-files -2' '
	...
	but diff-files -2 >diff-files-2.actual &&
	test_cmp diff-files-2.expect diff-files-2.actual &&
	but diff-files >diff-files-default-2.actual &&
...
test_expect_success 'diff-files -3' '
	...
	but diff-files -3 >diff-files-3.actual &&
