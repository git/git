...
create_new_pack() {
    rm -rf .but &&
    but init &&
    blob_1=$(but hash-object -t blob -w file_1) &&
    blob_2=$(but hash-object -t blob -w file_2) &&
    blob_3=$(but hash-object -t blob -w file_3) &&
    pack=$(printf "$blob_1\n$blob_2\n$blob_3\n" |
          but pack-objects $@ .but/objects/pack/pack) &&
    pack=".but/objects/pack/pack-${pack}" &&
    but verify-pack -v ${pack}.pack
}
...
do_corrupt_object() {
    ofs=$(but show-index < ${pack}.idx | grep $1 | cut -f1 -d" ") &&
    ofs=$(($ofs + $2)) &&
    chmod +w ${pack}.pack &&
    dd of=${pack}.pack bs=1 conv=notrunc seek=$ofs &&
    test_must_fail but verify-pack ${pack}.pack
}
...
test_expect_success \
    ...
    'create_test_files &&
     create_new_pack &&
     but prune-packed &&
     but cat-file blob $blob_1 > /dev/null &&
     but cat-file blob $blob_2 > /dev/null &&
     but cat-file blob $blob_3 > /dev/null'

test_expect_success \
    'create corruption in header of first object' \
    'do_corrupt_object $blob_1 0 < zero &&
     test_must_fail but cat-file blob $blob_1 > /dev/null &&
     test_must_fail but cat-file blob $blob_2 > /dev/null &&
     test_must_fail but cat-file blob $blob_3 > /dev/null'

test_expect_success \
    '... but having a loose copy allows for full recovery' \
    'mv ${pack}.idx tmp &&
     but hash-object -t blob -w file_1 &&
     mv tmp ${pack}.idx &&
     but cat-file blob $blob_1 > /dev/null &&
     but cat-file blob $blob_2 > /dev/null &&
     but cat-file blob $blob_3 > /dev/null'
...
test_expect_success \
    ...
    'but prune-packed &&
     test_must_fail but cat-file blob $blob_2 > /dev/null &&
     mv ${pack}.idx tmp &&
     but hash-object -t blob -w file_2 &&
     mv tmp ${pack}.idx &&
     test_must_fail but cat-file blob $blob_1 > /dev/null &&
     but cat-file blob $blob_2 > /dev/null &&
     but cat-file blob $blob_3 > /dev/null'
...
test_expect_success \
    '... but having a loose copy allows for full recovery' \
    'mv ${pack}.idx tmp &&
     but hash-object -t blob -w file_3 &&
     mv tmp ${pack}.idx &&
     but cat-file blob $blob_1 > /dev/null &&
     but cat-file blob $blob_2 > /dev/null &&
     but cat-file blob $blob_3 > /dev/null'
