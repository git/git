...
test_expect_success 'Use --numbered' '
	...
	but format-patch --numbered --stdout HEAD~1 >patch1 &&
...
test_expect_success 'format.numbered = true' '
	...
	but config format.numbered true &&
	but format-patch --stdout HEAD~2 >patch2 &&
...
test_expect_success 'format.numbered && single patch' '
	...
	but format-patch --stdout HEAD^ > patch3 &&
...
test_expect_success 'format.numbered && --no-numbered' '
	...
	but format-patch --no-numbered --stdout HEAD~2 >patch4 &&
...
test_expect_success 'format.numbered && --keep-subject' '
	...
	but format-patch --keep-subject --stdout HEAD^ >patch4a &&
...
test_expect_success 'format.numbered = auto' '
	...
	but config format.numbered auto &&
	but format-patch --stdout HEAD~2 > patch5 &&
...
test_expect_success 'format.numbered = auto && single patch' '
	...
	but format-patch --stdout HEAD^ > patch6 &&
...
test_expect_success 'format.numbered = auto && --no-numbered' '
	...
	but format-patch --no-numbered --stdout HEAD~2 > patch7 &&
