...
test_expect_success GPG 'create repositories with signed cummits' '
	...
	(
		cd signed &&
		echo 2 >b && git add b &&
		test_tick && git cummit -S -m "signed"
	) &&
	...
	(
		cd unsigned &&
		echo 3 >c && git add c &&
		test_tick && git cummit -m "unsigned"
	) &&
	...
	(
		cd bad &&
		echo 4 >d && git add d &&
		test_tick && git cummit -S -m "bad" &&
		git cat-file commit HEAD >raw &&
		sed -e "s/^bad/forged bad/" raw >forged &&
		git hash-object -w -t cummit forged >forged.cummit &&
		git checkout $(cat forged.cummit)
	) &&
	...
	(
		cd untrusted &&
		echo 5 >e && git add e &&
		test_tick && git cummit -SB7227189 -m "untrusted"
	)
'
