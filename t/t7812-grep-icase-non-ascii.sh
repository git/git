...
test_expect_success REGEX_LOCALE 'grep literal string, no -F' '
	but grep -i "TILRAUN: Halló Heimur!" &&
	but grep -i "TILRAUN: HALLÓ HEIMUR!"
'

test_expect_success GETTEXT_LOCALE,PCRE 'grep pcre utf-8 icase' '
	but grep --perl-regexp    "TILRAUN: H.lló Heimur!" &&
	but grep --perl-regexp -i "TILRAUN: H.lló Heimur!" &&
	but grep --perl-regexp -i "TILRAUN: H.LLÓ HEIMUR!"
'
...
test_expect_success GETTEXT_LOCALE,PCRE 'grep pcre utf-8 string with "+"' '
	...
	but add file2 &&
	but grep -l --perl-regexp "TILRAUN: H.lló+ Heimur!" >actual &&
	echo file >expected &&
	echo file2 >>expected &&
...
test_expect_success REGEX_LOCALE 'grep literal string, with -F' '
	but grep -i -F "TILRAUN: Halló Heimur!" &&
	but grep -i -F "TILRAUN: HALLÓ HEIMUR!"
'
...
test_expect_success REGEX_LOCALE 'grep string with regex, with -F' '
	...
	but add file3 &&
	but grep -i -F "TILRAUN: Halló Heimur [abc]!" file3
'

test_expect_success REGEX_LOCALE 'pickaxe -i on non-ascii' '
	but cummit -m first &&
	but log --format=%f -i -S"TILRAUN: HALLÓ HEIMUR!" >actual &&
	echo first >expected &&
	test_cmp expected actual
'
...
test_expect_success GETTEXT_LOCALE,LIBPCRE2 'PCRE v2: grep non-ASCII from invalid UTF-8 data with -i' '
	test_might_fail but grep -hi "Æ" invalid-0x80 >actual &&
	test_might_fail but grep -hi "(*NO_JIT)Æ" invalid-0x80 >actual
'
...
test_expect_success GETTEXT_LOCALE,LIBPCRE2,PCRE2_MATCH_INVALID_UTF 'PCRE v2: grep non-ASCII from invalid UTF-8 data with -i' '
	but grep -hi "Æ" invalid-0x80 >actual &&
	test_cmp expected actual &&
	but grep -hi "(*NO_JIT)Æ" invalid-0x80 >actual &&
	test_cmp expected actual
'
...
test_expect_success GETTEXT_LOCALE,LIBPCRE2,PCRE2_MATCH_INVALID_UTF 'PCRE v2: grep non-ASCII from invalid UTF-8 data with -i (PCRE2 bug #2642)' '
	but grep -hi "Æ" invalid-0xe5 >actual &&
	test_cmp invalid-0xe5 actual &&
	but grep -hi "(*NO_JIT)Æ" invalid-0xe5 >actual &&
	test_cmp invalid-0xe5 actual &&

	# Only the case of grepping the ASCII part in a way that
	# relies on -i fails
	but grep -hi "aÆ" invalid-0xe5 >actual &&
	test_cmp invalid-0xe5 actual &&
	but grep -hi "(*NO_JIT)aÆ" invalid-0xe5 >actual &&
	test_cmp invalid-0xe5 actual
'

test_expect_success GETTEXT_LOCALE,LIBPCRE2 'PCRE v2: grep non-literal ASCII from UTF-8' '
	but grep --perl-regexp -h -o -e ll. file >actual &&
	echo "lló" >expected &&
	test_cmp expected actual
'
