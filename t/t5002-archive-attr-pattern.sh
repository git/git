...
test_description='but archive attribute pattern tests'
...
test_expect_success 'setup' '
	...
	but add not-ignored-dir &&

	mkdir ignored-only-if-dir &&
	echo ignored by ignored dir >ignored-only-if-dir/ignored-by-ignored-dir &&
	echo ignored-only-if-dir/ export-ignore >>.but/info/attributes &&
	but add ignored-only-if-dir &&

	mkdir -p ignored-without-slash &&
	echo "ignored without slash" >ignored-without-slash/foo &&
	but add ignored-without-slash/foo &&
	echo "ignored-without-slash export-ignore" >>.but/info/attributes &&

	mkdir -p wildcard-without-slash &&
	echo "ignored without slash" >wildcard-without-slash/foo &&
	but add wildcard-without-slash/foo &&
	echo "wild*-without-slash export-ignore" >>.but/info/attributes &&

	mkdir -p deep/and/slashless &&
	echo "ignored without slash" >deep/and/slashless/foo &&
	but add deep/and/slashless/foo &&
	echo "deep/and/slashless export-ignore" >>.but/info/attributes &&

	mkdir -p deep/with/wildcard &&
	echo "ignored without slash" >deep/with/wildcard/foo &&
	but add deep/with/wildcard/foo &&
	echo "deep/*t*/wildcard export-ignore" >>.but/info/attributes &&

	mkdir -p one-level-lower/two-levels-lower/ignored-only-if-dir &&
	echo ignored by ignored dir >one-level-lower/two-levels-lower/ignored-only-if-dir/ignored-by-ignored-dir &&
	but add one-level-lower &&
