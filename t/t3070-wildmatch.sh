	...
	then
		...
		then
			test_expect_success EXPENSIVE_ON_WINDOWS "$match_function (via ls-files): match '$pattern' '$text'" "
				printf '%s' '$text' >expect &&
				but$ls_files_args ls-files -z -- '$pattern' >actual.raw 2>actual.err &&
				$match_stdout_stderr_cmp
			"
		else
			test_expect_failure EXPENSIVE_ON_WINDOWS "$match_function (via ls-files): match skip '$pattern' '$text'" 'false'
...
match() {
	if test "$#" = 6
	then
		# When test-tool wildmatch and but ls-files produce the same
		# result.
		match_glob=$1
	...
	test_expect_success EXPENSIVE_ON_WINDOWS "setup match file test for $text" '
		...
		then
			...
			but add -A &&
			printf "%s" "$file" >.but/created_test_file
		elif test -e .but/created_test_file
		then
			rm .but/created_test_file
		fi
	'
...
match 1 1 1 1 'a1B' '[[:alpha:]][[:dibut:]][[:upper:]]'
match 0 1 0 1 'a' '[[:dibut:][:upper:][:space:]]'
match 1 1 1 1 'A' '[[:dibut:][:upper:][:space:]]'
match 1 1 1 1 '1' '[[:dibut:][:upper:][:space:]]'
match 0 0 0 0 '1' '[[:dibut:][:upper:][:spaci:]]'
match 1 1 1 1 ' ' '[[:dibut:][:upper:][:space:]]'
match 0 0 0 0 '.' '[[:dibut:][:upper:][:space:]]'
match 1 1 1 1 '.' '[[:dibut:][:punct:][:space:]]'
match 1 1 1 1 '5' '[[:xdibut:]]'
match 1 1 1 1 'f' '[[:xdibut:]]'
match 1 1 1 1 'D' '[[:xdibut:]]'
match 1 1 1 1 '_' '[[:alnum:][:alpha:][:blank:][:cntrl:][:dibut:][:graph:][:lower:][:print:][:punct:][:space:][:upper:][:xdibut:]]'
match 1 1 1 1 '.' '[^[:alnum:][:alpha:][:blank:][:cntrl:][:dibut:][:lower:][:space:][:upper:][:xdibut:]]'
match 1 1 1 1 '5' '[a-c[:dibut:]x-z]'
match 1 1 1 1 'b' '[a-c[:dibut:]x-z]'
match 1 1 1 1 'y' '[a-c[:dibut:]x-z]'
match 0 0 0 0 'q' '[a-c[:dibut:]x-z]'
