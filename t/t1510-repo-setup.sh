...
1. GIT_DIR is relative to user's cwd. --but-dir is equivalent to
   GIT_DIR.
...
5. GIT_WORK_TREE/core.worktree was originally meant to work only if
   GIT_DIR is set, but earlier but didn't enforce it, and some scripts
...
9. If .but discovery is done inside a repo, the repo becomes a bare
   repo. .but discovery is performed if GIT_DIR is not set.
...
test_repo () {
	(
		...
		rm -f trace &&
		GIT_TRACE_SETUP="$(pwd)/trace" but symbolic-ref HEAD >/dev/null &&
...
# try_repo #c GIT_WORK_TREE GIT_DIR core.worktree .butfile? core.bare \
...
test_expect_success '#9: GIT_WORK_TREE accepted with butfile' '
	mkdir -p 9/wt &&
...
test_expect_success '#10: GIT_DIR can point to butfile' '
...
test_expect_success '#13: core.worktree+GIT_WORK_TREE accepted (with butfile)' '
	# or: you cannot intimidate away the lack of GIT_DIR setting
...
test_expect_success '#14: core.worktree with GIT_DIR pointing to butfile' '
...
# case #15: GIT_WORK_TREE overrides core.worktree (butfile case).
...
test_expect_success '#18: bare .but named by GIT_DIR has no worktree' '
...
test_expect_success '#20a: core.worktree without GIT_DIR accepted (inside .but)' '
	# Unlike case #16a.
...
test_expect_success '#22a: core.worktree = GIT_DIR = .but dir' '
	# like case #6.
...
test_expect_success '#22b: core.worktree child of .but, GIT_DIR=.but' '
...
test_expect_success '#22c: core.worktree = .but/.., GIT_DIR=.but' '
...
test_expect_success '#25: GIT_WORK_TREE accepted if GIT_DIR unset (bare butfile case)' '
...
test_expect_success '#26: bare repo has no worktree (GIT_DIR -> butfile case)' '
...
# Case #27: GIT_DIR + GIT_WORK_TREE suppresses bareness (with butfile).
...
# Case #29: GIT_WORK_TREE(+core.worktree) overrides core.bare (butfile case).
...
test_expect_success '#30: core.worktree and core.bare conflict (butfile version)' '
	...
	(
		cd 30 &&
		test_must_fail env GIT_DIR=.but but status 2>result
	) &&
