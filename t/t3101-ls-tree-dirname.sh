...
test_description='but ls-tree directory and filenames handling.

This test runs but ls-tree with the following in a tree.
...
test_expect_success 'ls-tree filter 1.txt' '
	but ls-tree $tree 1.txt >current &&
	cat >expected <<\EOF &&
100644 blob X	1.txt
EOF
	test_output
...
test_expect_success 'ls-tree filter path1/b/c/1.txt' '
	but ls-tree $tree path1/b/c/1.txt >current &&
	cat >expected <<\EOF &&
100644 blob X	path1/b/c/1.txt
EOF
	test_output
...
test_expect_success 'ls-tree filter all 1.txt files' '
	but ls-tree $tree 1.txt path0/a/b/c/1.txt \
...
test_expect_success 'ls-tree filter directories' '
	but ls-tree $tree path3 path2 path0/a/b/c path1/b/c path0/a >current &&
	cat >expected <<\EOF &&
...
test_expect_success 'ls-tree filter odd names' '
	but ls-tree $tree 1.txt ./1.txt .//1.txt \
...
test_expect_success 'ls-tree filter missing files and extra slashes' '
	but ls-tree $tree 1.txt/ abc.txt \
...
test_expect_success 'ls-tree filter is leading path match' '
	but ls-tree $tree pa path3/a >current &&
	>expected &&
	test_output
'
...
test_expect_success 'ls-tree --abbrev=5' '
	but ls-tree --abbrev=5 $tree >current &&
...
do
	...
	test_expect_success "ls-tree $opt -r" '
		but ls-tree $opt -r $tree >current &&
		cat >expected <<-\EOF &&
		1.txt
		2.txt
		path0/a/b/c/1.txt
