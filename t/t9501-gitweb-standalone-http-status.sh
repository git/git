...
test_expect_success 'snapshots: tgz only default format enabled' '
...
test_expect_success 'snapshots: all enabled in default, use default disabled value' '
	gitweb_run "p=.git;a=snapshot;h=HEAD;sf=tgz" &&
	grep "Status: 200 OK" gitweb.output &&
	gitweb_run "p=.git;a=snapshot;h=HEAD;sf=tbz2" &&
	grep "Status: 200 OK" gitweb.output &&
...
test_expect_success 'snapshots: zip explicitly disabled' '
	gitweb_run "p=.git;a=snapshot;h=HEAD;sf=zip" &&
	grep "403 - Snapshot format not allowed" gitweb.output
'
...
test_expect_success 'snapshots: tgz explicitly enabled' '
	gitweb_run "p=.git;a=snapshot;h=HEAD;sf=tgz" &&
	grep "Status: 200 OK" gitweb.output
'
