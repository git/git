#!/bin/sh

test_description='test main ref store api'

BUT_TEST_DEFAULT_INITIAL_BRANCH_NAME=main
export BUT_TEST_DEFAULT_INITIAL_BRANCH_NAME

. ./test-lib.sh

RUN="test-tool ref-store main"


test_expect_success 'setup' '
	test_cummit one
'

test_expect_success REFFILES 'pack_refs(PACK_REFS_ALL | PACK_REFS_PRUNE)' '
	N=`find .but/refs -type f | wc -l` &&
	test "$N" != 0 &&
	$RUN pack-refs PACK_REFS_PRUNE,PACK_REFS_ALL &&
	N=`find .but/refs -type f` &&
	test -z "$N"
'

test_expect_success 'create_symref(FOO, refs/heads/main)' '
	$RUN create-symref FOO refs/heads/main nothing &&
	echo refs/heads/main >expected &&
	but symbolic-ref FOO >actual &&
	test_cmp expected actual
'

test_expect_success 'delete_refs(FOO, refs/tags/new-tag)' '
	but tag -a -m new-tag new-tag HEAD &&
	but rev-parse FOO -- &&
	but rev-parse refs/tags/new-tag -- &&
	m=$(but rev-parse main) &&
	$RUN delete-refs REF_NO_DEREF nothing FOO refs/tags/new-tag &&
	test_must_fail but rev-parse --symbolic-full-name FOO &&
	test_must_fail but rev-parse FOO -- &&
	test_must_fail but rev-parse refs/tags/new-tag --
'

# In reftable, we keep the reflogs around for deleted refs.
test_expect_success !REFFILES 'delete-reflog(FOO, refs/tags/new-tag)' '
	$RUN delete-reflog FOO &&
	$RUN delete-reflog refs/tags/new-tag
'

test_expect_success 'rename_refs(main, new-main)' '
	but rev-parse main >expected &&
	$RUN rename-ref refs/heads/main refs/heads/new-main &&
	but rev-parse new-main >actual &&
	test_cmp expected actual &&
	test_cummit recreate-main
'

test_expect_success 'for_each_ref(refs/heads/)' '
	$RUN for-each-ref refs/heads/ | cut -d" " -f 2- >actual &&
	cat >expected <<-\EOF &&
	main 0x0
	new-main 0x0
	EOF
	test_cmp expected actual
'

test_expect_success 'for_each_ref() is sorted' '
	$RUN for-each-ref refs/heads/ | cut -d" " -f 2- >actual &&
	sort actual > expected &&
	test_cmp expected actual
'

test_expect_success 'resolve_ref(new-main)' '
	SHA1=`but rev-parse new-main` &&
	echo "$SHA1 refs/heads/new-main 0x0" >expected &&
	$RUN resolve-ref refs/heads/new-main 0 >actual &&
	test_cmp expected actual
'

test_expect_success 'verify_ref(new-main)' '
	$RUN verify-ref refs/heads/new-main
'

test_expect_success 'for_each_reflog()' '
	$RUN for-each-reflog | sort -k2 | cut -d" " -f 2- >actual &&
	cat >expected <<-\EOF &&
	HEAD 0x1
	refs/heads/main 0x0
	refs/heads/new-main 0x0
	EOF
	test_cmp expected actual
'

test_expect_success 'for_each_reflog_ent()' '
	$RUN for-each-reflog-ent HEAD >actual &&
	head -n1 actual | grep one &&
	tail -n1 actual | grep recreate-main
'

test_expect_success 'for_each_reflog_ent_reverse()' '
	$RUN for-each-reflog-ent-reverse HEAD >actual &&
	head -n1 actual | grep recreate-main &&
	tail -n1 actual | grep one
'

test_expect_success 'reflog_exists(HEAD)' '
	$RUN reflog-exists HEAD
'

test_expect_success 'delete_reflog(HEAD)' '
	$RUN delete-reflog HEAD &&
	test_must_fail but reflog exists HEAD
'

test_expect_success REFFILES 'create-reflog(HEAD)' '
	$RUN create-reflog HEAD &&
	but reflog exists HEAD
'

test_expect_success 'delete_ref(refs/heads/foo)' '
	but checkout -b foo &&
	FOO_SHA1=`but rev-parse foo` &&
	but checkout --detach &&
	test_cummit bar-cummit &&
	but checkout -b bar &&
	BAR_SHA1=`but rev-parse bar` &&
	$RUN update-ref updating refs/heads/foo $BAR_SHA1 $FOO_SHA1 0 &&
	echo $BAR_SHA1 >expected &&
	but rev-parse refs/heads/foo >actual &&
	test_cmp expected actual
'

test_expect_success 'delete_ref(refs/heads/foo)' '
	SHA1=`but rev-parse foo` &&
	but checkout --detach &&
	$RUN delete-ref msg refs/heads/foo $SHA1 0 &&
	test_must_fail but rev-parse refs/heads/foo --
'

test_done
