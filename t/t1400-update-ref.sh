...
test_description='Test but update-ref and basic ref logging'
. ./test-lib.sh
...
create_test_cummits ()
{
	...
	do
		test_tick &&
		T=$(but write-tree) &&
		sha1=$(echo $name | but cummit-tree $T) &&
		eval $prfx$name=$sha1
	done
}
...
test_expect_success "create $m with oldvalue verification" '
	but update-ref $m $B $A &&
	test $B = $(but show-ref -s --verify $m)
'
test_expect_success "fail to delete $m with stale ref" '
	test_must_fail but update-ref -d $m $A &&
	test $B = "$(but show-ref -s --verify $m)"
'
test_expect_success "delete $m" '
	test_when_finished "but update-ref -d $m" &&
	but update-ref -d $m $B &&
	test_must_fail but show-ref --verify -q $m
'
...
test_expect_success "fail to create $n" '
	test_when_finished "rm -f .but/$n_dir" &&
	touch .but/$n_dir &&
	test_must_fail but update-ref $n $A
'
...
test_expect_success "create $m (by HEAD) with oldvalue verification" '
	but update-ref HEAD $B $A &&
	test $B = $(but show-ref -s --verify $m)
'
test_expect_success "fail to delete $m (by HEAD) with stale ref" '
	test_must_fail but update-ref -d HEAD $A &&
	test $B = $(but show-ref -s --verify $m)
'
test_expect_success "delete $m (by HEAD)" '
	test_when_finished "but update-ref -d $m" &&
	but update-ref -d HEAD $B &&
	test_must_fail but show-ref --verify -q $m
'
...
test_expect_success "deleting current branch adds message to HEAD's log" '
	test_when_finished "but update-ref -d $m" &&
	but update-ref $m $A &&
	but symbolic-ref HEAD $m &&
	but update-ref -m delete-$m -d $m &&
	test_must_fail but show-ref --verify -q $m &&
	grep "delete-$m$" .but/logs/HEAD
'
...
test_expect_success "deleting by HEAD adds message to HEAD's log" '
	...
	but update-ref $m $A &&
	but symbolic-ref HEAD $m &&
	but update-ref -m delete-by-head -d HEAD &&
	test_must_fail but show-ref --verify -q $m &&
	grep "delete-by-head$" .but/logs/HEAD
'

test_expect_success 'update-ref does not create reflogs by default' '
	test_when_finished "but update-ref -d $outside" &&
	but update-ref $outside $A &&
	but rev-parse $A >expect &&
	but rev-parse $outside >actual &&
	test_cmp expect actual &&
	test_must_fail but reflog exists $outside
'

test_expect_success 'update-ref creates reflogs with --create-reflog' '
	test_when_finished "but update-ref -d $outside" &&
	but update-ref --create-reflog $outside $A &&
	but rev-parse $A >expect &&
	but rev-parse $outside >actual &&
	test_cmp expect actual &&
	but reflog exists $outside
'

test_expect_success 'creates no reflog in bare repository' '
	but -C $bare update-ref $m $bareA &&
	but -C $bare rev-parse $bareA >expect &&
	but -C $bare rev-parse $m >actual &&
	test_cmp expect actual &&
	test_must_fail but -C $bare reflog exists $m
'

test_expect_success 'core.logAllRefUpdates=true creates reflog in bare repository' '
	test_when_finished "but -C $bare config --unset core.logAllRefUpdates && \
		rm $bare/logs/$m" &&
	but -C $bare config core.logAllRefUpdates true &&
	but -C $bare update-ref $m $bareB &&
	but -C $bare rev-parse $bareB >expect &&
	but -C $bare rev-parse $m >actual &&
	test_cmp expect actual &&
	but -C $bare reflog exists $m
'
...
test_expect_success 'core.logAllRefUpdates=always creates reflog for ORIG_HEAD' '
	test_config core.logAllRefUpdates always &&
	but update-ref ORIG_HEAD $A &&
	but reflog exists ORIG_HEAD
'

test_expect_success '--no-create-reflog overrides core.logAllRefUpdates=always' '
	test_config core.logAllRefUpdates true &&
	test_when_finished "but update-ref -d $outside" &&
	but update-ref --no-create-reflog $outside $A &&
	but rev-parse $A >expect &&
	but rev-parse $outside >actual &&
	test_cmp expect actual &&
	test_must_fail but reflog exists $outside
...
test_expect_success 'delete symref without dereference' '
	...
	but add foo.c &&
	but cummit -m foo &&
	but symbolic-ref SYMREF $m &&
	but update-ref --no-deref -d SYMREF &&
	but show-ref --verify -q $m &&
	test_must_fail but show-ref --verify -q SYMREF &&
	test_must_fail but symbolic-ref SYMREF
'
...
test_expect_success 'update-ref -d is not confused by self-reference' '
	but symbolic-ref refs/heads/self refs/heads/self &&
	test_when_finished "rm -f .but/refs/heads/self" &&
	test_path_is_file .but/refs/heads/self &&
	test_must_fail but update-ref -d refs/heads/self &&
	test_path_is_file .but/refs/heads/self
'
...
test_expect_success 'update-ref --no-deref -d can delete self-reference' '
	...
	test_when_finished "rm -f .but/refs/heads/self" &&
	test_path_is_file .but/refs/heads/self &&
	but update-ref --no-deref -d refs/heads/self &&
	test_must_fail but show-ref --verify -q refs/heads/self
'

test_expect_success 'update-ref --no-deref -d can delete reference to bad ref' '
	>.but/refs/heads/bad &&
	test_when_finished "rm -f .but/refs/heads/bad" &&
	but symbolic-ref refs/heads/ref-to-bad refs/heads/bad &&
	test_when_finished "but update-ref -d refs/heads/ref-to-bad" &&
	test_path_is_file .but/refs/heads/ref-to-bad &&
	but update-ref --no-deref -d refs/heads/ref-to-bad &&
	test_must_fail but show-ref --verify -q refs/heads/ref-to-bad
'

test_expect_success '(not) create HEAD with old sha1' '
	test_must_fail but update-ref HEAD $A $B
'
test_expect_success "(not) prior created .but/$m" '
	test_when_finished "but update-ref -d $m" &&
	test_must_fail but show-ref --verify -q $m
'
...
test_expect_success '(not) change HEAD with wrong SHA1' '
	test_must_fail but update-ref HEAD $B $Z
'
test_expect_success "(not) changed .but/$m" '
	test_when_finished "but update-ref -d $m" &&
	! test $B = $(but show-ref -s --verify $m)
'
...
test_expect_success 'empty directory removal' '
	but branch d1/d2/r1 HEAD &&
	but branch d1/r2 HEAD &&
	test_path_is_file .but/refs/heads/d1/d2/r1 &&
	test_path_is_file .but/logs/refs/heads/d1/d2/r1 &&
	but branch -d d1/d2/r1 &&
	test_must_fail but show-ref --verify -q refs/heads/d1/d2 &&
	test_must_fail but show-ref --verify -q logs/refs/heads/d1/d2 &&
	test_path_is_file .but/refs/heads/d1/r2 &&
	test_path_is_file .but/logs/refs/heads/d1/r2
'

test_expect_success 'symref empty directory removal' '
	but branch e1/e2/r1 HEAD &&
	but branch e1/r2 HEAD &&
	but checkout e1/e2/r1 &&
	test_when_finished "but checkout main" &&
	test_path_is_file .but/refs/heads/e1/e2/r1 &&
	test_path_is_file .but/logs/refs/heads/e1/e2/r1 &&
	but update-ref -d HEAD &&
	test_must_fail but show-ref --verify -q refs/heads/e1/e2 &&
	test_must_fail but show-ref --verify -q logs/refs/heads/e1/e2 &&
	test_path_is_file .but/refs/heads/e1/r2 &&
	test_path_is_file .but/logs/refs/heads/e1/r2 &&
	test_path_is_file .but/logs/HEAD
'
...
test_expect_success 'Query "main@{May 25 2005}" (before history)' '
	...
	but rev-parse --verify "main@{May 25 2005}" >o 2>e &&
	echo "$C" >expect &&
	test_cmp expect o &&
...
test_expect_success 'Query main@{2005-05-25} (before history)' '
	test_when_finished "rm -f o e" &&
	but rev-parse --verify main@{2005-05-25} >o 2>e &&
	echo "$C" >expect &&
	test_cmp expect o &&
	echo "warning: log for '\''main'\'' only goes back to $ed" >expect &&
	test_cmp expect e
...
test_expect_success 'Query "main@{May 26 2005 23:31:59}" (1 second before history)' '
	test_when_finished "rm -f o e" &&
	but rev-parse --verify "main@{May 26 2005 23:31:59}" >o 2>e &&
	echo "$C" >expect &&
	test_cmp expect o &&
	echo "warning: log for '\''main'\'' only goes back to $ed" >expect &&
	test_cmp expect e
...
test_expect_success 'Query "main@{May 26 2005 23:32:00}" (exactly history start)' '
	test_when_finished "rm -f o e" &&
	but rev-parse --verify "main@{May 26 2005 23:32:00}" >o 2>e &&
	echo "$C" >expect &&
	test_cmp expect o &&
...
test_expect_success 'Query "main@{May 26 2005 23:32:30}" (first non-creation change)' '
	test_when_finished "rm -f o e" &&
	but rev-parse --verify "main@{May 26 2005 23:32:30}" >o 2>e &&
	echo "$A" >expect &&
	test_cmp expect o &&
	test_must_be_empty e
'
test_expect_success 'Query "main@{2005-05-26 23:33:01}" (middle of history with gap)' '
	test_when_finished "rm -f o e" &&
	but rev-parse --verify "main@{2005-05-26 23:33:01}" >o 2>e &&
	echo "$B" >expect &&
	test_cmp expect o &&
...
test_expect_success 'Query "main@{2005-05-26 23:38:00}" (middle of history)' '
	test_when_finished "rm -f o e" &&
	but rev-parse --verify "main@{2005-05-26 23:38:00}" >o 2>e &&
	echo "$Z" >expect &&
	test_cmp expect o &&
	test_must_be_empty e
'
test_expect_success 'Query "main@{2005-05-26 23:43:00}" (exact end of history)' '
	test_when_finished "rm -f o e" &&
	but rev-parse --verify "main@{2005-05-26 23:43:00}" >o 2>e &&
	echo "$E" >expect &&
	test_cmp expect o &&
	test_must_be_empty e
'
test_expect_success 'Query "main@{2005-05-28}" (past end of history)' '
	test_when_finished "rm -f o e" &&
	but rev-parse --verify "main@{2005-05-28}" >o 2>e &&
	echo "$D" >expect &&
	test_cmp expect o &&
	test_i18ngrep -F "warning: log for ref $m unexpectedly ended on $ld" e
'

rm -f .but/$m .but/logs/$m expect
...
test_expect_success 'but cat-file blob main:F (expect OTHER)' '
	test OTHER = $(but cat-file blob main:F)
'
test_expect_success 'but cat-file blob main@{2005-05-26 23:30}:F (expect TEST)' '
	test TEST = $(but cat-file blob "main@{2005-05-26 23:30}:F")
'
test_expect_success 'but cat-file blob main@{2005-05-26 23:42}:F (expect OTHER)' '
	test OTHER = $(but cat-file blob "main@{2005-05-26 23:42}:F")
'

# Test adding and deleting pseudorefs

test_expect_success 'given old value for missing pseudoref, do not create' '
	test_must_fail but update-ref PSEUDOREF $A $B 2>err &&
	test_must_fail but rev-parse PSEUDOREF &&
	test_i18ngrep "unable to resolve reference" err
'

test_expect_success 'create pseudoref' '
	but update-ref PSEUDOREF $A &&
	test $A = $(but rev-parse PSEUDOREF)
'

test_expect_success 'overwrite pseudoref with no old value given' '
	but update-ref PSEUDOREF $B &&
	test $B = $(but rev-parse PSEUDOREF)
'

test_expect_success 'overwrite pseudoref with correct old value' '
	but update-ref PSEUDOREF $C $B &&
	test $C = $(but rev-parse PSEUDOREF)
'

test_expect_success 'do not overwrite pseudoref with wrong old value' '
	test_must_fail but update-ref PSEUDOREF $D $E 2>err &&
	test $C = $(but rev-parse PSEUDOREF) &&
	test_i18ngrep "cannot lock ref.*expected" err
'

test_expect_success 'delete pseudoref' '
	but update-ref -d PSEUDOREF &&
	test_must_fail but rev-parse PSEUDOREF
'

test_expect_success 'do not delete pseudoref with wrong old value' '
	but update-ref PSEUDOREF $A &&
	test_must_fail but update-ref -d PSEUDOREF $B 2>err &&
	test $A = $(but rev-parse PSEUDOREF) &&
	test_i18ngrep "cannot lock ref.*expected" err
'

test_expect_success 'delete pseudoref with correct old value' '
	but update-ref -d PSEUDOREF $A &&
	test_must_fail but rev-parse PSEUDOREF
'

test_expect_success 'create pseudoref with old OID zero' '
	but update-ref PSEUDOREF $A $Z &&
	test $A = $(but rev-parse PSEUDOREF)
'

test_expect_success 'do not overwrite pseudoref with old OID zero' '
	test_when_finished but update-ref -d PSEUDOREF &&
	test_must_fail but update-ref PSEUDOREF $B $Z 2>err &&
	test $A = $(but rev-parse PSEUDOREF) &&
...
test_expect_success '-z fails without --stdin' '
	test_must_fail but update-ref -z $m $m $m 2>err &&
	test_i18ngrep "usage: but update-ref" err
'

test_expect_success 'stdin works with no input' '
	>stdin &&
	but update-ref --stdin <stdin &&
	but rev-parse --verify -q $m
'
...
test_expect_success 'stdin create ref works' '
	echo "create $a $m" >stdin &&
	but update-ref --stdin <stdin &&
	but rev-parse $m >expect &&
	but rev-parse $a >actual &&
	test_cmp expect actual
'
...
test_expect_success 'stdin creates reflogs with --create-reflog' '
	test_when_finished "but update-ref -d $outside" &&
	echo "create $outside $m" >stdin &&
	but update-ref --create-reflog --stdin <stdin &&
	but rev-parse $m >expect &&
	but rev-parse $outside >actual &&
	test_cmp expect actual &&
	but reflog exists $outside
...
test_expect_success 'stdin create ref works with path with space to blob' '
	...
	but update-ref --stdin <stdin &&
	but rev-parse "$m:$pws" >expect &&
	but rev-parse refs/blobs/pws >actual &&
	test_cmp expect actual &&
	but update-ref -d refs/blobs/pws
'
...
test_expect_success 'stdin update ref fails with wrong old value' '
	...
	test_must_fail but rev-parse --verify -q $c
'
...
test_expect_success 'stdin update symref works option no-deref' '
	but symbolic-ref TESTSYMREF $b &&
	cat >stdin <<-EOF &&
	option no-deref
	update TESTSYMREF $a $b
	EOF
	but update-ref --stdin <stdin &&
	but rev-parse TESTSYMREF >expect &&
	but rev-parse $a >actual &&
	test_cmp expect actual &&
	but rev-parse $m~1 >expect &&
	but rev-parse $b >actual &&
...
test_expect_success 'stdin delete symref works option no-deref' '
	...
	EOF
	but update-ref --stdin <stdin &&
	test_must_fail but rev-parse --verify -q TESTSYMREF &&
	but rev-parse $m~1 >expect &&
	but rev-parse $b >actual &&
	test_cmp expect actual
'
...
test_expect_success 'stdin update symref works flag --no-deref' '
	but symbolic-ref TESTSYMREFONE $b &&
	but symbolic-ref TESTSYMREFTWO $b &&
	cat >stdin <<-EOF &&
	update TESTSYMREFONE $a $b
	update TESTSYMREFTWO $a $b
	EOF
	but update-ref --no-deref --stdin <stdin &&
	but rev-parse TESTSYMREFONE TESTSYMREFTWO >expect &&
	but rev-parse $a $a >actual &&
	test_cmp expect actual &&
	but rev-parse $m~1 >expect &&
	but rev-parse $b >actual &&
	test_cmp expect actual
...
test_expect_success 'stdin delete symref works flag --no-deref' '
	...
	EOF
	but update-ref --no-deref --stdin <stdin &&
	test_must_fail but rev-parse --verify -q TESTSYMREFONE &&
	test_must_fail but rev-parse --verify -q TESTSYMREFTWO &&
	but rev-parse $m~1 >expect &&
	but rev-parse $b >actual &&
	test_cmp expect actual
'
...
test_expect_success 'stdin delete ref works with right old value' '
	...
	but update-ref --stdin <stdin &&
	test_must_fail but rev-parse --verify -q $b
'
...
test_expect_success 'stdin verify succeeds for missing reference' '
	...
	but update-ref --stdin <stdin &&
	test_must_fail but rev-parse --verify -q refs/heads/missing
'
...
test_expect_success 'stdin verify fails for wrong value' '
	...
	test_must_fail but update-ref --stdin <stdin &&
	but rev-parse $m >actual &&
	test_cmp expect actual
'
...
test_expect_success 'stdin verify fails for mistaken empty value' '
	M=$(but rev-parse $m) &&
	test_when_finished "but update-ref $m $M" &&
	but rev-parse $m >expect &&
	echo "verify $m" >stdin &&
	test_must_fail but update-ref --stdin <stdin &&
	but rev-parse $m >actual &&
...
test_expect_success 'stdin delete refs works with packed and loose refs' '
	...
	EOF
	but update-ref --stdin <stdin &&
	test_must_fail but rev-parse --verify -q $a &&
	test_must_fail but rev-parse --verify -q $b &&
	test_must_fail but rev-parse --verify -q $c
'
...
test_expect_success 'stdin -z emits warning with empty new value' '
	...
	but update-ref -z --stdin <stdin 2>err &&
...
test_expect_success 'stdin -z verify fails for wrong value' '
	...
	test_must_fail but update-ref -z --stdin <stdin &&
	but rev-parse $m >actual &&
	test_cmp expect actual
'
...
test_expect_success 'fails with duplicate HEAD update' '
	but branch target1 $A &&
	but checkout target1 &&
	cat >stdin <<-EOF &&
	...
	test_cmp expect actual &&
	echo "$A" >expect &&
	but rev-parse refs/heads/target1 >actual &&
	test_cmp expect actual
'

test_expect_success 'fails with duplicate ref update via symref' '
	but branch target2 $A &&
	but symbolic-ref refs/heads/symref2 refs/heads/target2 &&
	cat >stdin <<-EOF &&
	...
	but symbolic-ref refs/heads/symref2 >actual &&
	test_cmp expect actual &&
	echo "$A" >expect &&
	but rev-parse refs/heads/target2 >actual &&
	test_cmp expect actual
'
...
(
	...
	run_with_limited_open_files but update-ref --stdin <large_input &&
	but rev-parse --verify -q refs/heads/33
)
'
...
(
	...
	run_with_limited_open_files but update-ref --stdin <large_input &&
	test_must_fail but rev-parse --verify -q refs/heads/33
)
'

test_expect_success 'handle per-worktree refs in refs/bisect' '
	but cummit --allow-empty -m "initial cummit" &&
	but worktree add -b branch worktree &&
	(
		cd worktree &&
		but cummit --allow-empty -m "test cummit"  &&
		but for-each-ref >for-each-ref.out &&
		! grep refs/bisect for-each-ref.out &&
		but update-ref refs/bisect/something HEAD &&
		but rev-parse refs/bisect/something >../worktree-head &&
		but for-each-ref | grep refs/bisect/something
	) &&
	but show-ref >actual &&
	! grep 'refs/bisect' actual &&
	test_must_fail but rev-parse refs/bisect/something &&
	but update-ref refs/bisect/something HEAD &&
	but rev-parse refs/bisect/something >main-head &&
...
test_expect_success 'transaction handles empty cummit' '
	...
	cummit
	EOF
	but update-ref --stdin <stdin >actual &&
...
test_expect_success 'transaction exits on multiple aborts' '
	...
	abort
	EOF
	test_must_fail but update-ref --stdin <stdin >actual 2>err &&
	printf "%s: ok\n" abort >expect &&
	test_cmp expect actual &&
...
test_expect_success 'transaction exits on start after prepare' '
	...
	start
	EOF
	test_must_fail but update-ref --stdin <stdin 2>err >actual &&
	printf "%s: ok\n" prepare >expect &&
	test_cmp expect actual &&
...
test_expect_success 'transaction can handle abort' '
	...
	printf "%s: ok\n" start abort >expect &&
	test_cmp expect actual &&
	test_must_fail but show-ref --verify -q $b
'
...
test_expect_success 'transaction can cummit multiple times' '
	...
	test_cmp expect actual &&
	echo "$A" >expect &&
	but rev-parse refs/heads/branch-1 >actual &&
	test_cmp expect actual &&
	echo "$B" >expect &&
	but rev-parse refs/heads/branch-2 >actual &&
	test_cmp expect actual
'
...
test_expect_success 'transaction can create and delete' '
	...
	but update-ref --stdin <stdin >actual &&
	printf "%s: ok\n" start cummit start cummit >expect &&
	test_must_fail but show-ref --verify refs/heads/create-and-delete
'
...
test_expect_success 'transaction can cummit after abort' '
	...
	echo "$A" >expect &&
	but rev-parse refs/heads/abort >actual &&
	test_cmp expect actual
'
...
test_expect_success 'transaction cannot restart ongoing transaction' '
	...
	cummit
	EOF
	test_must_fail but update-ref --stdin <stdin >actual &&
	test_must_fail but show-ref --verify refs/heads/restart
'

test_expect_success PIPE 'transaction flushes status updates' '
	mkfifo in out &&
	(but update-ref --stdin <in >out &) &&
	...
	test_must_fail but update-ref refs/heads/flush $B 2>stderr &&
...
test_expect_success 'directory not created deleting packed ref' '
	but branch d1/d2/r1 HEAD &&
	but pack-refs --all &&
	test_path_is_missing .but/refs/heads/d1/d2 &&
	but update-ref -d refs/heads/d1/d2/r1 &&
	test_path_is_missing .but/refs/heads/d1/d2 &&
	test_path_is_missing .but/refs/heads/d1
'
