...
test_but_co () {
	# Usage: test_but_co BRANCH_NAME
	(cd module-but && but checkout "$1")
}

test_cmp_branch_file () {
	# Usage: test_cmp_branch_file BRANCH_NAME PATH
	# The branch must already be checked out of CVS and but.
	test_cmp module-cvs-"$1"/"$2" module-but/"$2"
}

test_cmp_branch_tree () {
	# Usage: test_cmp_branch_tree BRANCH_NAME
	# Check BRANCH_NAME out of CVS and but and make sure that all
	...
	(
		cd module-but
		find . -type d -name .but -prune -o -type f -print
	) | sort >module-but-"$1".list &&
	test_cmp module-cvs-"$1".list module-but-"$1".list &&
