...
test_expect_success \
    ...
    'but diff-tree $tree_O $tree_A >.test-a &&
     cmp -s .test-a .test-plain-OA'

test_expect_success \
    'diff-tree of known trees.' \
    'but diff-tree -r $tree_O $tree_A >.test-a &&
     cmp -s .test-a .test-recursive-OA'

test_expect_success \
    'diff-tree of known trees.' \
    'but diff-tree $tree_O $tree_B >.test-a &&
     cmp -s .test-a .test-plain-OB'

test_expect_success \
    'diff-tree of known trees.' \
    'but diff-tree -r $tree_O $tree_B >.test-a &&
     cmp -s .test-a .test-recursive-OB'

test_expect_success \
    'diff-tree of known trees.' \
    'but diff-tree $tree_A $tree_B >.test-a &&
     cmp -s .test-a .test-plain-AB'

test_expect_success \
    'diff-tree of known trees.' \
    'but diff-tree -r $tree_A $tree_B >.test-a &&
     cmp -s .test-a .test-recursive-AB'

test_expect_success \
    'diff-tree --stdin of known trees.' \
    'echo $tree_A $tree_B | but diff-tree --stdin > .test-a &&
...
test_expect_success \
    'diff-tree --stdin of known trees.' \
    'echo $tree_A $tree_B | but diff-tree -r --stdin > .test-a &&
...
test_expect_success \
    ...
    'but read-tree $tree_A &&
     but diff-index --cached $tree_O >.test-a &&
     cmp -s .test-a .test-recursive-OA'

test_expect_success \
    'diff-cache O with B in cache' \
    'but read-tree $tree_B &&
     but diff-index --cached $tree_O >.test-a &&
     cmp -s .test-a .test-recursive-OB'
...
test_expect_success \
    ...
    'but read-tree $tree_B &&
     but diff-index --cached $tree_A >.test-a &&
     cmp -s .test-a .test-recursive-AB'
...
test_expect_success \
    ...
    'rm -fr Z [A-Z][A-Z] &&
     ...
     but read-tree --reset $tree_O &&
     test_must_fail but update-index --refresh -q &&
     but diff-files >.test-a &&
...
test_expect_success \
    ...
    'but diff-tree $tree_O $tree_A >.test-a &&
    but diff-tree -R $tree_A $tree_O >.test-b &&
    cmp -s .test-a .test-b'

test_expect_success \
    'diff-tree -r O A == diff-tree -r -R A O' \
    'but diff-tree -r $tree_O $tree_A >.test-a &&
    but diff-tree -r -R $tree_A $tree_O >.test-b &&
    cmp -s .test-a .test-b'

test_expect_success \
    'diff-tree B A == diff-tree -R A B' \
    'but diff-tree $tree_B $tree_A >.test-a &&
    but diff-tree -R $tree_A $tree_B >.test-b &&
    cmp -s .test-a .test-b'

test_expect_success \
    'diff-tree -r B A == diff-tree -r -R A B' \
    'but diff-tree -r $tree_B $tree_A >.test-a &&
    but diff-tree -r -R $tree_A $tree_B >.test-b &&
    cmp -s .test-a .test-b'

test_expect_success \
    'diff can read from stdin' \
    'test_must_fail but diff --no-index -- MN - < NN |
