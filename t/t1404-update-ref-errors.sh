...
test_description='Test but update-ref error handling'
. ./test-lib.sh
...
test_update_rejected () {
	...
	but for-each-ref $prefix >unchanged &&
	if $pack
	then
		but pack-refs --all
	fi &&
	printf "create $prefix/%s $C\n" $create >input &&
	test_must_fail but update-ref --stdin <input 2>output.err &&
	test_i18ngrep -F "$error" output.err &&
	but for-each-ref $prefix >actual &&
...
df_test() {
	...
	but update-ref "$delref" $C &&
	if $symadd
	then
		addname="$prefix/s/symadd" &&
		but symbolic-ref "$addname" "$addref"
	else
		addname="$addref"
	fi &&
	if $symdel
	then
		...
		but symbolic-ref "$delname" "$delref"
	else
		delname="$delref"
	fi &&
	...
	test_must_fail but update-ref --stdin <commands 2>output.err &&
	test_cmp expected-err output.err &&
	printf "%s\n" "$C $delref" >expected-refs &&
	but for-each-ref --format="%(objectname) %(refname)" $prefix/r >actual-refs &&
...
test_expect_success REFFILES 'empty directory should not fool rev-parse' '
	...
	but update-ref $prefix/foo $C &&
	but pack-refs --all &&
	mkdir -p .but/$prefix/foo/bar/baz &&
	echo "$C" >expected &&
	but rev-parse $prefix/foo >actual &&
...
test_expect_success REFFILES 'empty directory should not fool for-each-ref' '
	...
	but update-ref $prefix/foo $C &&
	but for-each-ref $prefix >expected &&
	but pack-refs --all &&
	mkdir -p .but/$prefix/foo/bar/baz &&
	but for-each-ref $prefix >actual &&
	test_cmp expected actual
...
test_expect_success 'missing old value blocks update' '
	...
	test_must_fail but update-ref --stdin 2>output.err &&
...
test_expect_success 'missing old value blocks indirect update' '
	...
	but symbolic-ref $prefix/symref $prefix/foo &&
	cat >expected <<-EOF &&
...
test_expect_success REFFILES 'non-empty directory blocks create' '
	...
	mkdir -p .but/$prefix/foo/bar &&
	: >.but/$prefix/foo/bar/baz.lock &&
	test_when_finished "rm -f .but/$prefix/foo/bar/baz.lock" &&
	cat >expected <<-EOF &&
	fatal: cannot lock ref $SQ$prefix/foo$SQ: there is a non-empty directory $SQ.but/$prefix/foo$SQ blocking reference $SQ$prefix/foo$SQ
	EOF
	printf "%s\n" "update $prefix/foo $C" |
	test_must_fail but update-ref --stdin 2>output.err &&
	test_cmp expected output.err &&
...
test_expect_success REFFILES 'broken reference blocks create' '
	...
	mkdir -p .but/$prefix &&
	echo "gobbledigook" >.but/$prefix/foo &&
	test_when_finished "rm -f .but/$prefix/foo" &&
	cat >expected <<-EOF &&
...
test_expect_success REFFILES 'non-empty directory blocks indirect create' '
	...
	test_when_finished "rm -f .but/$prefix/foo/bar/baz.lock" &&
	cat >expected <<-EOF &&
	fatal: cannot lock ref $SQ$prefix/symref$SQ: there is a non-empty directory $SQ.but/$prefix/foo$SQ blocking reference $SQ$prefix/foo$SQ
	EOF
...
test_expect_success REFFILES 'no bogus intermediate values during delete' '
	...
	but update-ref $prefix/foo $C &&
	but pack-refs --all &&
	but update-ref $prefix/foo $D &&
	but for-each-ref $prefix >unchanged &&
	# Now try to update the reference, but hold the `packed-refs` lock
	# for a while to see what happens while the process is blocked:
	: >.but/packed-refs.lock &&
	test_when_finished "rm -f .but/packed-refs.lock" &&
	{
		...
		but -c core.packedrefstimeout=30000 update-ref -d $prefix/foo &
	} &&
	pid2=$! &&
	...
	sha1=$(but rev-parse --verify --quiet $prefix/foo || echo undefined) &&
	case "$sha1" in
	$D)
	...
	*)
	...
	rm -f .but/packed-refs.lock &&
	wait $pid2 &&
	test_must_be_empty out &&
	test_must_fail but rev-parse --verify --quiet $prefix/foo
'
...
test_expect_success REFFILES 'delete fails cleanly if packed-refs file is locked' '
	...
	: >.but/packed-refs.lock &&
	test_when_finished "rm -f .but/packed-refs.lock" &&
	test_must_fail but update-ref -d $prefix/foo >out 2>err &&
	but for-each-ref $prefix >actual &&
...
test_expect_success REFFILES 'delete fails cleanly if packed-refs.new write fails' '
	...
	: >.but/packed-refs.new &&
	test_when_finished "rm -f .but/packed-refs.new" &&
	test_must_fail but update-ref -d $prefix/foo &&
	but for-each-ref $prefix >actual &&
	test_cmp unchanged actual
'
