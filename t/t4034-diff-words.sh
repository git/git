...
cat >post.simple <<-\EOF
	...
	aeff = aeff * ( aaa )
EOF
pre=$(but rev-parse --short $(but hash-object pre.simple))
post=$(but rev-parse --short $(but hash-object post.simple))
cat >expect.letter-runs-are-words <<-EOF
	<BOLD>diff --but a/pre b/post<RESET>
...
word_diff () {
	pre=$(but rev-parse --short $(but hash-object pre)) &&
	post=$(but rev-parse --short $(but hash-object post)) &&
	test_must_fail but diff --no-index "$@" pre post >output &&
...
test_language_driver () {
	...
	test_expect_success "diff driver '$lang'" '
		...
		echo "* diff='"$lang"'" >.butattributes &&
		word_diff --color-words
	'
}

test_expect_success setup '
	but config diff.color.old red &&
	but config diff.color.new green &&
	but config diff.color.func magenta
'
...
test_expect_success 'set up a diff driver' '
	but config diff.testdriver.wordRegex "[^[:space:]]" &&
...
test_expect_success 'option overrides .butattributes' '
	cp expect.letter-runs-are-words expect &&
	word_diff --color-words="[a-z]+"
'
...
test_expect_success 'set up diff.wordRegex option' '
	but config diff.wordRegex "[[:alnum:]]+"
'
...
test_expect_success '.butattributes override config' '
	cp expect.non-whitespace-is-word expect &&
	word_diff --color-words
'
...
test_expect_success 'setup history with two files' '
	...
	echo "a bx; c" >a.tex &&
	echo "a bx; c" >z.txt &&
	but cummit -mmodified -a
'

test_expect_success 'wordRegex for the first file does not apply to the second' '
	echo "*.tex diff=tex" >.butattributes &&
	test_config diff.tex.wordRegex "[a-z]+|." &&
	cat >expect <<-\EOF &&
		diff --but a/a.tex b/a.tex
		--- a/a.tex
		+++ b/a.tex
		@@ -1 +1 @@
		a [-b-]{+bx+}; c
		diff --but a/z.txt b/z.txt
		--- a/z.txt
		+++ b/z.txt
		@@ -1 +1 @@
		a [-b;-]{+bx;+} c
	EOF
	but diff --word-diff HEAD~ >actual &&
