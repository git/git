...
test_description='but apply filename consistency check'
...
test_expect_success 'apply diff with inconsistent filenames in headers' '
	test_must_fail but apply bad1.patch 2>err &&
	test_i18ngrep "inconsistent new filename" err &&
	test_must_fail but apply bad2.patch 2>err &&
...
test_expect_success 'apply diff with new filename missing from headers' '
	...
	+1
	EOF
	test_must_fail but apply missing_new_filename.diff 2>err &&
...
test_expect_success 'apply diff with old filename missing from headers' '
	...
	-1
	EOF
	test_must_fail but apply missing_old_filename.diff 2>err &&
	test_i18ngrep "lacks filename information" err
'
