...
test_description='but mailinfo and but mailsplit test'
...
test_expect_success 'split sample box' \
	'but mailsplit -o. "$DATA/sample.mbox" >last &&
	last=$(cat last) &&
	echo total is $last &&
	test $(cat last) = 18'
...
check_mailinfo () {
	mail=$1 opt=$2
	mo="$mail$opt"
	but mailinfo -u $opt "msg$mo" "patch$mo" <"$mail" >"info$mo" &&
...
test_expect_success 'split box with rfc2047 samples' \
	'mkdir rfc2047 &&
	but mailsplit -orfc2047 "$DATA/rfc2047-samples.mbox" \
	  >rfc2047/last &&
...
do
	test_expect_success "mailinfo $mail" '
		but mailinfo -u "$mail-msg" "$mail-patch" <"$mail" >"$mail-info" &&
		echo msg &&
...
test_expect_success 'respect NULs' '
	...
	but mailsplit -d3 -o. "$DATA/nul-plain" &&
	test_cmp "$DATA/nul-plain" 001 &&
	(cat 001 | but mailinfo msg patch) &&
...
test_expect_success 'Preserve NULs out of MIME encoded message' '
	...
	but mailsplit -d5 -o. "$DATA/nul-b64.in" &&
	test_cmp "$DATA/nul-b64.in" 00001 &&
	but mailinfo msg patch <00001 &&
...
test_expect_success 'mailinfo on from header without name works' '
	...
	mkdir info-from &&
	but mailsplit -oinfo-from "$DATA/info-from.in" &&
	test_cmp "$DATA/info-from.in" info-from/0001 &&
	but mailinfo info-from/msg info-from/patch \
...
test_expect_success 'mailinfo finds headers after embedded From line' '
	mkdir embed-from &&
	but mailsplit -oembed-from "$DATA/embed-from.in" &&
	test_cmp "$DATA/embed-from.in" embed-from/0001 &&
	but mailinfo embed-from/msg embed-from/patch \
...
test_expect_success 'mailinfo on message with quoted >From' '
	mkdir quoted-from &&
	but mailsplit -oquoted-from "$DATA/quoted-from.in" &&
	test_cmp "$DATA/quoted-from.in" quoted-from/0001 &&
	but mailinfo quoted-from/msg quoted-from/patch \
...
test_expect_success 'mailinfo unescapes with --mboxrd' '
	mkdir mboxrd &&
	but mailsplit -omboxrd --mboxrd \
		"$DATA/sample.mboxrd" >last &&
	test x"$(cat last)" = x2 &&
	for i in 0001 0002
	do
		but mailinfo mboxrd/msg mboxrd/patch \
		  <mboxrd/$i >mboxrd/out &&
	...
	but mailsplit -f2 -omboxrd --mboxrd <sp >last &&
	test x"$(cat last)" = x1 &&
	but mailinfo mboxrd/msg mboxrd/patch <mboxrd/0003 &&
	test_cmp expect mboxrd/msg
'

test_expect_success 'mailinfo handles rfc2822 quoted-string' '
	mkdir quoted-string &&
	but mailinfo /dev/null /dev/null <"$DATA/quoted-string.in" \
		>quoted-string/info &&
...
test_expect_success 'mailinfo handles rfc2822 comment' '
	mkdir comment &&
	but mailinfo /dev/null /dev/null <"$DATA/comment.in" \
		>comment/info &&
...
test_expect_success 'mailinfo with mailinfo.scissors config' '
	...
	(
		mkdir sub &&
		cd sub &&
		but mailinfo ../msg0014.sc ../patch0014.sc <../0014 >../info0014.sc
	) &&
...
test_expect_success 'mailinfo handles unusual header whitespace' '
	but mailinfo /dev/null /dev/null >actual <<-\EOF &&
...
check_quoted_cr_mail () {
	mail="$1" && shift &&
	but mailinfo -u "$@" "$mail.msg" "$mail.patch" \
...
test_expect_success 'split base64 email with quoted-cr' '
	mkdir quoted-cr &&
	but mailsplit -oquoted-cr "$DATA/quoted-cr.mbox" >quoted-cr/last &&
