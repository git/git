...
test_description='but log with invalid commit headers'
...
test_expect_success 'setup' '
	...
	but cat-file commit HEAD |
	sed "/^author /s/>/>-<>/" >broken_email.cummit &&
	but hash-object -w -t cummit broken_email.cummit >broken_email.hash &&
	but update-ref refs/heads/broken_email $(cat broken_email.hash)
'
...
test_expect_success 'but log with broken author email' '
	{
	...
	but log broken_email >actual.out 2>actual.err &&

	test_cmp expect.out actual.out &&
	test_must_be_empty actual.err
'

test_expect_success 'but log --format with broken author email' '
	echo "A U Thor+author@example.com+Thu Apr 7 15:13:13 2005 -0700" >expect.out &&

	but log --format="%an+%ae+%ad" broken_email >actual.out 2>actual.err &&
...
test_expect_success 'unparsable dates produce sentinel value (%ad)' '
	cummit=$(munge_author_date HEAD totally_bogus) &&
	echo >expect &&
	but log -1 --format=%ad $cummit >actual &&
	test_cmp expect actual
'
