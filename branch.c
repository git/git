                                                  ...
                                                  const char *origin, struct string_list *remotes)
{
    ...
    for_each_string_list_item(item, remotes) if (git_config_set_multivar_gently(key.buf, item->string, CONFIG_REGEX_NONE, 0) < 0)
    {
        goto out_err;
    }
    ...
    if (flag & BRANCH_CONFIG_VERBOSE)
    {
        ...
        if (remotes->nr == 1)
        {
             ...
             * upstream branch.
             */
            printf_ln(rebasing ? _("branch '%s' set up to track '%s' by rebasing.") : _("branch '%s' set up to track '%s'."),
...
static void prepare_checked_out_branches(void)
{
    ...
    while (worktrees[i])
    {
        ...
        if (wt_status_check_rebase(wt, &state) && (state.rebase_in_progress || state.rebase_interactive_in_progress) && state.branch)
        {
        ...
        if (wt_status_check_bisect(wt, &state) && state.bisecting_from)
        {
            struct strbuf ref = STRBUF_INIT;
