                                             ...
                                             const struct conv_attrs  *ca)
{
    ...
    packed_item_size = sizeof(struct pc_item_fixed_portion) + ce->ce_namelen + (ca->working_tree_encoding ? strlen(ca->working_tree_encoding) : 0);
                     ...
                     int *checkout_counter)
{
    ...
    if (parallel_checkout.status != PC_ACCEPTING_ENTRIES || !is_eligible_for_parallel_checkout(ce, ca))
        return -1;
                   ...
                   struct checkout               *state)
{
     ...
     */
    if (dir_sep && !has_dirs_only_path(path.buf, dir_sep - path.buf, state->base_dir_len))
    {
...
static void send_one_item(int fd, struct parallel_checkout_item *pc_item)
{
    ...
    size_t                        working_tree_encoding_len = working_tree_encoding ? strlen(working_tree_encoding) : 0;

    /*
     * Any changes in the calculation of the message size must also be made
     * in is_eligible_for_parallel_checkout().
     */
    len_data = sizeof(struct pc_item_fixed_portion) + name_len + working_tree_encoding_len;
