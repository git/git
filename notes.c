                             ...
                             struct leaf_node *entry)
{
    ...
    *p = SET_PTR_TYPE(NULL, PTR_TYPE_NULL);
    ...
    while (i > 0 && !note_tree_consolidate(parent_stack[i], parent_stack[i - 1], GET_NIBBLE(i - 1, entry->key_oid.hash)))
    {
        i--;
    }
}
                            ...
                            combine_notes_fn combine_notes)
{
    ...
    assert(GET_PTR_TYPE(*p) == PTR_TYPE_NOTE || GET_PTR_TYPE(*p) == PTR_TYPE_SUBTREE);
    if (is_null_oid(&entry->val_oid))
...
{
    struct non_note *p = t->prev_non_note;
    struct non_note *n;
    ...
    redo:
        ...
        switch (GET_PTR_TYPE(p))
        {
            ...
            case PTR_TYPE_SUBTREE:
                 ...
                 */
                if (n < 2 * fanout && flags & FOR_EACH_NOTE_YIELD_SUBTREES)
                {
                ...
                if (n >= 2 * fanout || !(flags & FOR_EACH_NOTE_DONT_UNPACK_SUBTREES))
                {
...
{
    return full_path[0] == tws->path[0] && full_path[1] == tws->path[1] && full_path[2] == '/';
}

static void write_tree_entry(struct strbuf *buf, unsigned int mode,
                             const char *path, unsigned int path_len, const unsigned char *hash)
{
                                  ...
                                  const struct object_id *oid)
{
    ...
    while (tws && 3 * n < path_len && matches_tree_write_stack(tws, path + 3 * n))
    {
        n++;
        tws = tws->next;
    }
                           ...
                           void *cb_data)
{
    ...
    return write_each_non_note_until(note_path, d) || write_each_note_helper(d->root, note_path, mode, note_oid);
}
                              ...
                              const struct object_id *new_oid)
{
    ...
    enum object_type cur_type;
    enum object_type new_type;
    int              ret;
                ...
                combine_notes_fn combine_notes, int flags)
{
    ...
    if (flags & NOTES_INIT_EMPTY || repo_get_oid_treeish(the_repository, notes_ref, &object_oid))
    {
        return;
    }
...
void load_display_notes(struct display_notes_opt *opt)
{
    ...
    if (!opt || opt->use_default_notes > 0 || (opt->use_default_notes == -1 && !opt->extra_notes_refs.nr))
    {
...
int write_notes_tree(struct notes_tree *t, struct object_id *result)
{
    ...
    flags = FOR_EACH_NOTE_DONT_UNPACK_SUBTREES | FOR_EACH_NOTE_YIELD_SUBTREES;
    ret   = for_each_note(t, flags, write_each_note, &cb_data) || write_each_non_note_until(NULL, &cb_data) || tree_write_stack_finish_subtree(&root) || write_object_file(root.buf.buf, root.buf.len, OBJ_TREE, result);
                        ...
                        struct strbuf *sb, const char *output_encoding, int raw)
{
    ...
    if (output_encoding && *output_encoding && !is_encoding_utf8(output_encoding))
    {
