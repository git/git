...
#define FILTER_REFS_KIND_MASK                                                \
 (FILTER_REFS_REGULAR | FILTER_REFS_DETACHED_HEAD | FILTER_REFS_PSEUDOREFS | \
...
struct ref_filter {
	...
	unsigned int with_commit_tag_algo:1, match_as_path:1, ignore_case:1,
		detached:1;
...
#define REF_FILTER_INIT                                \
 {                                                     \
  .points_at = OID_ARRAY_INIT, .exclude = STRVEC_INIT, \
 }
#define REF_FORMAT_INIT                           \
 {                                                \
  .use_color = -1, .bases = STRING_LIST_INIT_DUP, \
...
#define _OPT_MERGED_NO_MERGED(option, filter, h)                               \
 {                                                                             \
  OPTION_CALLBACK, 0, option, (filter), N_("commit"), (h),                     \
	  PARSE_OPT_LASTARG_DEFAULT | PARSE_OPT_NONEG, parse_opt_merge_filter, \
	  (intptr_t) "HEAD"                                                    \
 }
...
#define OPT_REF_SORT(var) \
 OPT_STRING_LIST(0, "sort", (var), N_("key"), N_("field name to sort on"))
#define OPT_REF_FILTER_EXCLUDE(var)                       \
 OPT_STRVEC(0, "exclude", &(var)->exclude, N_("pattern"), \
	    N_("exclude refs which match pattern"))
 ...
 */
void filter_is_base(struct repository *r, struct ref_format *format,
		    struct ref_array *array);
