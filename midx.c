                                                          ...
                                                          int         local)
{
    ...
    {
        const char *end;
        size_t      avail = m->chunk_pack_names_len - (cur_pack_name - (const char *)m->chunk_pack_names);
                             ...
                             struct multi_pack_index *midx_chain)
{
    if (midx_chain)
    {
        ...
        midx->num_packs_in_base   = midx_chain->num_packs + midx_chain->num_packs_in_base;
        midx->num_objects_in_base = midx_chain->num_objects + midx_chain->num_objects_in_base;
    }
...
uint32_t nth_midxed_pack_int_id(struct multi_pack_index *m, uint32_t pos)
{
    ...
    return m->num_packs_in_base + get_be32(m->chunk_object_offsets + (off_t)pos * MIDX_CHUNK_OFFSET_WIDTH);
}
                    ...
                    struct multi_pack_index *m)
{
    ...
    if (oidset_size(&p->bad_objects) && oidset_contains(&p->bad_objects, oid))
    {
        return 0;
    }
                                ...
                                const char *file_name, void *_data)
{
    ...
    if (!(starts_with(file_name, "multi-pack-index-") && ends_with(file_name, data->ext)))
    {
        return;
    }
...
int verify_midx_file(struct repository *r, const char *object_dir, unsigned flags)
{
    ...
    for (i = 0; i < m->num_objects + m->num_objects_in_base; i++)
    {
        ...
        if (i > 0 && pairs[i - 1].pack_int_id != pairs[i].pack_int_id && nth_midxed_pack(m, pairs[i - 1].pack_int_id))
        {
