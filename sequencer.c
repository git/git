...
static void append_todo_help(unsigned edit_todo, unsigned keep_empty,
			     struct strbuf *buf)
{
	const char *msg = _("\nCommands:\n"
...
int edit_todo_list(unsigned flags)
{
	...
	write_message(buf.buf, buf.len, todo_file, 0);
...
int complete_action(struct replay_opts *opts, unsigned flags,
		    ...
		    unsigned autosquash, unsigned keep_empty, unsigned verbose)
{
	...
	if (check_todo_list()) {
		checkout_onto(opts, onto_name, onto, orig_head, verbose);
		return 1;
	}
	...
	if (opts->allow_ff && skip_unnecessary_picks(&onto))
		return error("Could not skip unnecessary pick commands");
