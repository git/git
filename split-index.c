...
{
    const unsigned int ondisk_flags = CE_STAGEMASK | CE_VALID | CE_EXTENDED_FLAGS;
    ...
    ret = memcmp(&a->ce_stat_data, &b->ce_stat_data,
                 offsetof(struct cache_entry, name) - offsetof(struct cache_entry, oid))
              != 0
          || !oideq(&a->oid, &b->oid);
    a->ce_flags = ce_flags;
...
void prepare_to_write_split_index(struct index_state *istate)
{
    ...
    if (si->base)
    {
        ...
        for (i = 0; i < istate->cache_nr; i++)
        {
            ...
            if (ce->ce_namelen != base->ce_namelen || strcmp(ce->name, base->name) != 0)
            {
                ce->index = 0;
                continue;
            }
        ...
        {
            ce = si->base->cache[i];
            if ((ce->ce_flags & CE_REMOVE) || !(ce->ce_flags & CE_MATCHED))
            {
...
{
    if (ce->index && istate->split_index && istate->split_index->base && ce->index <= istate->split_index->base->cache_nr && ce == istate->split_index->base->cache[ce->index - 1])
    {
...
{
    if (old_entry->index && istate->split_index && istate->split_index->base && old_entry->index <= istate->split_index->base->cache_nr)
    {
