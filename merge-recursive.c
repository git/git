...
static int path_hashmap_cmp(const void *cmp_data        UNUSED,
                            ...
                            const void                 *keydata)
{
    const struct path_hashmap_entry *a;
    const struct path_hashmap_entry *b;
...
__attribute__((format(printf, 2, 3))) static int err(struct merge_options *opt, const char *err, ...)
{
    va_list params;
...
{
    return (!opt->priv->call_depth && opt->verbosity >= v) || opt->verbosity >= 5;
}

__attribute__((format(printf, 3, 4))) static void output(struct merge_options *opt, int v, const char *fmt, ...)
{
    va_list ap;
                                     ...
                                     struct string_list   *entries)
{
    ...
    for (i = 0; i < df_sorted_entries.nr; i++)
    {
         ...
         */
        if (last_file && len > last_len && memcmp(path, last_file, last_len) == 0 && path[last_len] == '/')
        {
    ...
    if (update_working_directory)
    {
        if (ignore_case)
        {
            ...
            ce = index_file_exists(opt->repo->index, path, strlen(path),
                                   ignore_case);
            if (ce && ce_stage(ce) == 0 && strcmp(path, ce->name) != 0)
            {
                return 0;
            }
        }
                      ...
                      int check_working_copy, int empty_ok)
{
    ...
    if (pos < istate->cache_nr && !strncmp(dirpath.buf, istate->cache[pos]->name, dirpath.len))
    {
    ...
    strbuf_release(&dirpath);
    return check_working_copy && !lstat(path, &st) && S_ISDIR(st.st_mode) && !(empty_ok && is_empty_dir(path)) && !has_symlink_leading_path(path, strlen(path));
}
...
static int would_lose_untracked(struct merge_options *opt, const char *path)
{
    ...
    while (pos < istate->cache_nr && !strcmp(path, istate->cache[pos]->name))
    {
        /*
...
static int make_room_for_path(struct merge_options *opt, const char *path)
{
    ...
    for (i = 0; i < opt->priv->df_conflict_file_set.nr; i++)
    {
        ...
        if (df_pathlen < pathlen && path[df_pathlen] == '/' && strncmp(path, df_path, df_pathlen) == 0)
        {
            output(opt, 3,
                             ...
                             int                         update_wd)
{
    ...
    if (update_wd)
    {
        ...
        if (S_ISREG(contents->mode) || (!has_symlinks && S_ISLNK(contents->mode)))
        {
            ...
            fd = open(path, O_WRONLY | O_TRUNC | O_CREAT | O_CLOEXEC, mode);
            if (fd < 0)
            {
...
update_index:
    if (!ret && update_cache)
    {
        int refresh = (!opt->priv->call_depth && contents->mode != S_IFGITLINK);
                      ...
                      const int                   extra_marker_size)
{
    ...
    if (strcmp(a->path, b->path) != 0 || strcmp(a->path, o->path) != 0)
    {
                           ...
                           const struct object_id *b)
{
    ...
    if (!(commit_base = lookup_commit_reference(&subrepo, base)) || !(commit_a = lookup_commit_reference(&subrepo, a)) || !(commit_b = lookup_commit_reference(&subrepo, b)))
    {
    ...
    switch (parent_count)
    {
        ...
        case 1:
            ...
            output(opt, 2, _("If this is correct simply add it to the index "
                             "for example\n"
                             "by using:\n\n"
                                   ...
                                   struct merge_file_info     *result)
{
    ...
    else
    {
        ...
        else if (S_ISREG(a->mode))
        {
            ...
            if (!ret && write_object_file(result_buf.ptr, result_buf.size, OBJ_BLOB, &result->blob.oid))
            {
                                 ...
                                 struct rename_conflict_info *ci)
{
    ...
    int                         mark_conflicted = (opt->detect_directory_renames == MERGE_DIRECTORY_RENAMES_CONFLICT);
                                ...
                                const char *change, const char *change_past)
{
    ...
    if (dir_in_way(opt->repo->index, path, !opt->priv->call_depth, 0) || (!opt->priv->call_depth && would_lose_untracked(opt, path)))
    {
                                ...
                                struct rename_conflict_info *ci)
{
    ...
    const char                 *delete_branch = (opt->branch1 == ren->branch ? opt->branch2 : opt->branch1);
                                 ...
                                 struct diff_filespec *b)
{
    ...
    if (!mfi.clean && !opt->priv->call_depth && update_stages(opt, collide_path, NULL, a, b))
    {
        return -1;
    }
    free(alt_path);
                             ...
                             struct rename_conflict_info *ci)
{
    ...
    const char *add_branch    = (opt->branch1 == rename_branch ? opt->branch2 : opt->branch1);
                                     ...
                                     struct rename_conflict_info *ci)
{
    ...
    if (!mfi.clean && mfi.blob.mode == a->mode && oideq(&mfi.blob.oid, &a->oid))
    {
        /*
                                    ...
                                    char **old_dir, char **new_dir)
{
    ...
    *new_dir = NULL;
    ...
    while (*--end_of_new == *--end_of_old && end_of_old != old_path && end_of_new != new_path)
    {
     ...
     */
    if (end_of_old == old_path && end_of_new == new_path && *end_of_old == *end_of_new)
    {
        return; /* Note: *old_dir and *new_dir are still NULL */
    }
     ...
     */
    if (end_of_new == new_path && end_of_old != old_path && end_of_old[-1] == '/')
    {
        *old_dir = xstrndup(old_path, --end_of_old - old_path);
        *new_dir = xstrdup("");
        return;
                                             ...
                                             struct tree          *merge)
{
                           ...
                           ent /* member name */)
    {
        ...
        if (merge_ent && !head_ent->non_unique_new_dir && !merge_ent->non_unique_new_dir && !strbuf_cmp(&head_ent->new_dir, &merge_ent->new_dir))
        {
                           ...
                           ent /* member name */)
    {
        ...
        else if (head_ent && !head_ent->non_unique_new_dir && !merge_ent->non_unique_new_dir)
        {
                           ...
                           struct string_list   *b_renames)
{
    ...
    for (i = 0; i < a_renames->nr; i++)
    {
        ...
        string_list_insert(&a_by_dst, sre->pair->two->path)->util = (void *)sre;
    }
    for (i = 0; i < b_renames->nr; i++)
    {
        sre                                                       = b_renames->items[i].util;
        string_list_insert(&b_by_dst, sre->pair->two->path)->util = (void *)sre;
    }

    for (i = 0, j = 0; i < a_renames->nr || j < b_renames->nr;)
    {
        struct string_list      *renames1;
        struct string_list      *renames2Dst;
        struct rename           *ren1 = NULL;
        struct rename           *ren2 = NULL;
        const char              *ren1_src;
        const char              *ren1_dst;
        ...
        else
        {
            ...
            struct diff_filespec src_other;
            struct diff_filespec dst_other;
            int                  try_merge;
            ...
            if (oideq(&src_other.oid, null_oid()) && ren1->dir_rename_original_type == 'A')
            {
            ...
            else if ((dst_other.mode == ren1->pair->two->mode) && oideq(&dst_other.oid, &ren1->pair->two->oid))
            {
                /*
...
{
    struct diff_queue_struct *head_pairs;
    struct diff_queue_struct *merge_pairs;
    struct hashmap           *dir_re_head;
    struct hashmap           *dir_re_merge;
    int                       clean = 1;
    ...
    if ((opt->detect_directory_renames == MERGE_DIRECTORY_RENAMES_TRUE) || (opt->detect_directory_renames == MERGE_DIRECTORY_RENAMES_CONFLICT && !opt->priv->call_depth))
    {
                          ...
                          int renormalize, const char *path)
{
    ...
    if (read_oid_strbuf(opt, &o->oid, &obuf) || read_oid_strbuf(opt, &a->oid, &abuf))
    {
        goto error_return;
    }
    /*
     ...
     */
    if (renormalize_buffer(idx, path, obuf.buf, obuf.len, &obuf) | renormalize_buffer(idx, path, abuf.buf, abuf.len, &abuf))
    {
                                ...
                                struct rename_conflict_info *ci)
{
    ...
    if (ci && dir_in_way(opt->repo->index, path, !opt->priv->call_depth, S_ISGITLINK(ci->ren1->pair->two->mode)))
    {
     ...
     */
    if (mfi->clean && was_tracked_and_matches(opt, path, &mfi->blob) && !df_conflict_remains)
    {
        int                 pos;
        struct cache_entry *ce;
                                ...
                                struct rename_conflict_info *ci)
{
    ...
    if (clean && opt->detect_directory_renames == MERGE_DIRECTORY_RENAMES_CONFLICT && ren->dir_rename_original_dest)
    {
                               ...
                               struct rename        *ren)
{
    ...
    other_branch = (ren->branch == opt->branch1 ? opt->branch2 : opt->branch1);
    if (is_add)
    {
                                          ...
                                          struct rename        *ren)
{
    ...
    assert(ren->dir_rename_original_type == 'A' || ren->dir_rename_original_type == 'R');
    ...
    {
        msg = _(
            "Path updated: %s renamed to %s in %s, inside a "
            "directory that was renamed in %s; moving it to %s.");
    }
    else if (ren->dir_rename_original_type == 'R' && !clean)
    {
        msg = _(
            "CONFLICT (file location): %s renamed to %s in %s, "
            "inside a directory that was renamed in %s, "
            "suggesting it should perhaps be moved to %s.");
    }
    else
                         ...
                         const char *path, struct stage_data *entry)
{
    ...
    else if (o_valid && (!a_valid || !b_valid))
    {
        ...
        if ((!a_valid && !b_valid) || (!b_valid && blob_unchanged(opt, o, a, normalize, path)) || (!a_valid && blob_unchanged(opt, o, b, normalize, path)))
        {
    ...
    else if ((!o_valid && a_valid && !b_valid) || (!o_valid && !a_valid && b_valid))
    {
        /* Case B: Added in one. */
                                ...
                                struct tree         **result)
{
    ...
    if (opt->priv->call_depth && !(*result = write_in_core_index_as_tree(opt->repo)))
    {
        return -1;
    }
                    ...
                    struct commit           **result)
{
    ...
    assert(opt->ancestor == NULL || !strcmp(opt->ancestor, "constructed merge base"));
...
static void merge_recursive_config(struct merge_options *opt, int ui)
{
    ...
    if (!git_config_get_string("merge.directoryrenames", &value))
    {
        ...
        if (0 <= boolval)
        {
            opt->detect_directory_renames = boolval ? MERGE_DIRECTORY_RENAMES_TRUE : MERGE_DIRECTORY_RENAMES_NONE;
        }
...
int parse_merge_opt(struct merge_options *opt, const char *s)
{
    ...
    else if (skip_prefix(s, "find-renames=", &arg) || skip_prefix(s, "rename-threshold=", &arg))
    {
