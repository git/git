...
static wchar_t *normalize_ntpath(wchar_t *wbuf)
{
    ...
    if (wbuf[0] == '\\')
    {
        ...
        if (!wcsncmp(wbuf, L"\\??\\", 4) || !wcsncmp(wbuf, L"\\\\?\\", 4))
            wbuf += 4;
...
int mingw_unlink(const char *pathname)
{
    ...
    while (ret == -1 && is_file_in_use_error(GetLastError()) && ask_yes_no_if_possible("Unlink of file '%s' failed. "
                                                                                       "Should I try again?",
                                                                                       pathname))
...
int mingw_rmdir(const char *pathname)
{
    ...
    while (ret == -1 && errno == EACCES && is_file_in_use_error(GetLastError()) && ask_yes_no_if_possible("Deletion of directory '%s' failed. "
                                                                                                          "Should I try again?",
                                                                                                          pathname))
...
{
    return (is_dir_sep(filename[0]) && is_dir_sep(filename[1]) && filename[2] == '.' && is_dir_sep(filename[3]) && !strncasecmp(filename + 4, "pipe", 4) && is_dir_sep(filename[8]) && filename[9]);
}
...
int mingw_utime(const char *file_name, const struct utimbuf *times)
{
    ...
    osfilehandle = CreateFileW(wfilename,
                               ...
                               NULL,
                               OPEN_EXISTING,
                               (attrs != INVALID_FILE_ATTRIBUTES && (attrs & FILE_ATTRIBUTE_DIRECTORY)) ? FILE_FLAG_BACKUP_SEMANTICS : 0,
                               NULL);
                                            ...
                                            const char *component)
{
    ...
    if (*len < component_len + 1 || !is_dir_sep(path[*len - component_len - 1]) || fspathncmp(path + *len - component_len, component, component_len))
        return 0;
    *len -= component_len + 1;
...
static int is_msys2_sh(const char *cmd)
{
    ...
    if (!strcmp(cmd, "sh"))
    {
        ...
        else
        {
            ...
            ret = match_last_path_component(p, &len, "sh.exe") && match_last_path_component(p, &len, "bin") && match_last_path_component(p, &len, "usr");
            free(p);
        }
        return ret;
    }
                              ...
                              int prepend_cmd, int fhin, int fhout, int fherr)
{
    ...
    const char *(*quote_arg)(const char *arg) =
        is_msys2_sh(cmd ? cmd : *argv) ? quote_arg_msys2 : quote_arg_msvc;
    ...
    if (si.StartupInfo.hStdOutput != INVALID_HANDLE_VALUE && si.StartupInfo.hStdOutput != si.StartupInfo.hStdInput)
        stdhandles[stdhandles_count++] = si.StartupInfo.hStdOutput;
    if (si.StartupInfo.hStdError != INVALID_HANDLE_VALUE && si.StartupInfo.hStdError != si.StartupInfo.hStdInput && si.StartupInfo.hStdError != si.StartupInfo.hStdOutput)
    ...
    if (strace_env)
    {
        ...
        free(p);
        if (!strcmp("1", strace_env) || !strcasecmp("yes", strace_env) || !strcasecmp("true", strace_env))
    ...
    if (!ret && restrict_handle_inheritance && stdhandles_count)
    {
        ...
        if (err != ERROR_NO_SYSTEM_RESOURCES &&
             ...
             * handle inheritance.
             */
            !(err == ERROR_INVALID_PARAMETER && GetVersion() >> 16 < 9200) && !getenv("SUPPRESS_HANDLE_INHERITANCE_WARNING"))
        {
            DWORD fl = 0;
            int   i;
...
repeat:
    ...
    if (gle == ERROR_ACCESS_DENIED && ask_yes_no_if_possible("Rename from '%s' to '%s' failed. "
                                                             "Should I try again?",
                                                             pold, pnew))
        goto repeat;
...
int xutftowcsn(wchar_t *wcs, const char *utfs, size_t wcslen, int utflen)
{
    ...
    while (upos < utflen)
    {
        ...
        else if (c >= 0xe0 && c < 0xf0 && upos + 1 < utflen && !(c == 0xe0 && utf[upos] < 0xa0) && /* over-long encoding */
        ...
        else if (c >= 0xf0 && c < 0xf5 && upos + 2 < utflen && wpos + 1 < wcslen && !(c == 0xf0 && utf[upos] < 0x90) && /* over-long encoding */
                 !(c == 0xf4 && utf[upos] >= 0x90) &&                                                                   /* > \u10ffff */
                 (utf[upos] & 0xc0) == 0x80 && (utf[upos + 1] & 0xc0) == 0x80 && (utf[upos + 2] & 0xc0) == 0x80)
        {
...
static int acls_supported(const char *path)
{
    ...
    if (offset && xutftowcsn(wroot, path, MAX_PATH, offset) > 0 && GetVolumeInformationW(wroot, NULL, 0, NULL, NULL, &file_system_flags, NULL, 0))
...
int is_path_owned_by_current_sid(const char *path, struct strbuf *report)
{
    ...
    else if (sid && IsValidSid(sid))
    {
        ...
        if (current_user_sid && IsValidSid(current_user_sid) && EqualSid(sid, current_user_sid))
            result = 1;
        else if (IsWellKnownSid(sid, WinBuiltinAdministratorsSid) && CheckTokenMembership(NULL, sid, &is_member) && is_member)
            /*
             ...
             */
            result = 1;
        else if (report && IsWellKnownSid(sid, WinWorldSid) && !acls_supported(path))
        {
            /*
        ...
        switch (c)
        {
            ...
            segment_start:
                switch (*path)
                {
                    case 'a':
                    case 'A': /* AUX */
                        if (((c = path[++i]) != 'u' && c != 'U') || ((c = path[++i]) != 'x' && c != 'X'))
                        {
                        not_a_reserved_name:
                            path += i;
                            continue;
                    ...
                    case 'C':
                        ...
                        { /* CON */
                            c = path[i + 1];
                            if ((c == 'i' || c == 'I') && ((c = path[i + 2]) == 'n' || c == 'N') && path[i + 3] == '$')
                                i += 3; /* CONIN$ */
                            else if ((c == 'o' || c == 'O') && ((c = path[i + 2]) == 'u' || c == 'U') && ((c = path[i + 3]) == 't' || c == 'T') && path[i + 4] == '$')
                                i += 4; /* CONOUT$ */
                        }
                        else
                            goto not_a_reserved_name;
                    ...
                    case 'l':
                    case 'L': /* LPT<N> */
                        if (((c = path[++i]) != 'p' && c != 'P') || ((c = path[++i]) != 't' && c != 'T') || !isdigit(path[++i]))
                            goto not_a_reserved_name;
                        break;
                    case 'n':
                    case 'N': /* NUL */
                        if (((c = path[++i]) != 'u' && c != 'U') || ((c = path[++i]) != 'l' && c != 'L') || (allow_literal_nul && !path[i + 1] && p == path))
                            goto not_a_reserved_name;
                        break;
                    case 'p':
                    case 'P': /* PRN */
                        if (((c = path[++i]) != 'r' && c != 'R') || ((c = path[++i]) != 'n' && c != 'N'))
                            goto not_a_reserved_name;
                        break;
                    default:
                        continue;
