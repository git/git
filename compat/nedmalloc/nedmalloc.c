...
struct threadcacheblk_t {    /* Keep less than 16 bytes on 32 bit systems and 32 bytes on 64 bit systems */
...
static NOINLINE threadcache *AllocCache(nedpool *p) THROWSPEC
{
    ...
    if (THREADCACHEMAXCACHES == n) {    /* List exhausted, so disable for this thread */
...
static void *threadcache_malloc(nedpool *p, threadcache *tc, size_t *size) THROWSPEC
{
    ...
    blk = *binsptr;
    if (!blk || blk->size < *size) {    /* Bump it up a bin */
...
{    /* Gets called when thread's last used mspace is in use. The strategy
    ...
    if (end < p->threads) {
        ...
        if (end >= p->threads) {    /* Drat, must destroy it now */
            RELEASE_LOCK(&p->mutex);
...
void neddisablethreadcache(nedpool *p) THROWSPEC
{
    ...
    if (!mycache) {    /* Set to mspace 0 */
        if (TLSSET(p->mycache, (void *) -1)) abort();
    } else if (mycache > 0) {    /* Set to last used mspace */
...
#if THREADCACHEMAX
    if (tc && size <= THREADCACHEMAX) {    /* Use the thread cache */
        ret = threadcache_malloc(p, tc, &size);
    }
#endif
    if (!ret) {    /* Use this thread's mspace */
...
#if THREADCACHEMAX
    if (tc && rsize <= THREADCACHEMAX) {    /* Use the thread cache */
...
void *nedprealloc(nedpool *p, void *mem, size_t size) THROWSPEC
{
    ...
    GetThreadCache(&p, &tc, &mymspace, &size);
#if THREADCACHEMAX
    if (tc && size && size <= THREADCACHEMAX) {    /* Use the thread cache */
