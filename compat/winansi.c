...
static void warn_if_raster_font(void)
{
    ...
    else
    {
        ...
        HKEY hkey;
        if (ERROR_SUCCESS == RegOpenKeyExA(HKEY_CURRENT_USER, "Console", 0, KEY_READ, &hkey))
        {
...
static DWORD WINAPI console_thread(LPVOID unused)
{
    ...
    while (1)
    {
                      ...
                      NULL))
        {
            ...
            if (GetLastError() == ERROR_PIPE_NOT_CONNECTED || GetLastError() == ERROR_BROKEN_PIPE)
                break;
            /* ignore other errors */
            continue;
        }
...
int winansi_dup2(int oldfd, int newfd)
{
    ...
    if (!ret && newfd >= 0 && newfd <= 2)
        fd_is_interactive[newfd] = oldfd < 0 || oldfd > 2 ? 0 : fd_is_interactive[oldfd];
