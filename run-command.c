...
static void cleanup_children(int sig, int in_signal)
{
    ...
    while (children_to_clean)
    {
        ...
        if (p->process && !in_signal)
        {
            ...
            {
                trace_printf(
                    "trace: run_command: running exit handler for pid %" PRIuMAX, (uintmax_t)p->pid);
    ...
    for (i = 0; i < nr; i++)
    {
        ...
        if (io->type == POLLOUT)
        {
            ...
            len = write(io->fd, io->u.out.buf,
                        io->u.out.len <= MAX_IO_SIZE ? io->u.out.len : MAX_IO_SIZE);
            if (len < 0)
            {
                        ...
                        const struct run_process_parallel_opts *opts)
{
    ...
    if (start_command(&pp->children[i].process))
    {
        if (opts->start_failure)
            code = opts->start_failure(opts->ungroup ? NULL : &pp->children[i].err,
                                       opts->data,
                                       pp->children[i].data);
        else
            code = 0;
                             ...
                             int                                     output_timeout)
{
    ...
    for (size_t i = 0; i < opts->processes; i++)
    {
        if (pp->children[i].state == GIT_CP_WORKING && pp->pfd[i].revents & (POLLIN | POLLHUP))
        {
                               ...
                               const struct run_process_parallel_opts *opts)
{
    ...
    while (pp->nr_processes > 0)
    {
        ...
        if (opts->task_finished)
            code = opts->task_finished(code, opts->ungroup ? NULL : &pp->children[i].err, opts->data,
                                       pp->children[i].data);
        else
            code = 0;
...
void run_processes_parallel(const struct run_process_parallel_opts *opts)
{
    ...
    {
        for (i = 0;
             i < spawn_cap && !pp.shutdown && pp.nr_processes < opts->processes;
             i++)
        {
...
int prepare_auto_maintenance(int quiet, struct child_process *maint)
{
    ...
    if (!git_config_get_bool("maintenance.auto", &enabled) && !enabled)
        return 0;
...
void prepare_other_repo_env(struct strvec *env, const char *new_git_dir)
{
    ...
    {
        if (strcmp(*var, CONFIG_DATA_ENVIRONMENT) && strcmp(*var, CONFIG_COUNT_ENVIRONMENT))
            strvec_push(env, *var);
    }
